...
#: 배선 이전 판이 실제로 들고 있던 주장 — "래퍼가 캐시 계열 토큰을 읽지 않는다".
#: 가드가 잡아야 하는 것은 **이 주장**이지 "읽지 않는다"라는 낱말이 아니다.
_옛_주장 = re.compile(r"캐시[^.]{0,80}읽지\s*않는다")


def _한_줄로(text: str) -> str:
    """줄바꿈을 지워 문장이 줄에 걸쳐 있어도 한 덩어리로 본다."""
    return re.sub(r"\s+", " ", text)


def test_단가_문서가_캐시를_읽지_않는다는_옛_서술을_들고_있지_않다() -> None:
    """배선이 생겼는데 문서가 "캐시를 읽지 않는다"고 적혀 있으면 그 자체가 거짓 신고다.

    **낱말이 아니라 주장을 본다.** 처음에는 `"읽지 않는다" not in text` 였는데, 뒤에
    *"세 묶음을 같은 축의 시계열로 읽지 않는다"* 라는 **무관한 문장**이 들어오면서 가드가
    터졌다 — 이 저장소가 *"소스를 문자열로 스캔해 가드를 켜고 끄지 않는다"* 로 적어 둔
    실패의 문서 쪽 판본이다. 지금은 같은 문장 안에서 "캐시"와 함께 나오는 경우만 잡는다.
    """
    text = _한_줄로(_pricing_text())

    hit = _옛_주장.search(text)
    assert hit is None, hit.group(0) if hit else ""


def test_옛_주장_가드는_실제로_그_문장을_잡는다() -> None:
    """음성 대조 — 배선 이전 판이 들고 있던 문장을 넣으면 가드가 문다."""
    배선_이전_문장 = "생성 쪽 클라이언트가 응답의 캐시 계열 토큰(`cached_tokens`)을 읽지 않는다."
    assert _옛_주장.search(_한_줄로(배선_이전_문장)) is not None

    # 무관한 문장은 통과한다 — 낱말만 겹치는 경우.
    무관한_문장 = '세 묶음을 "같은 축의 시계열"로 읽지 않는다 — 계열 구성비를 보는 자리다.'
    assert _옛_주장.search(_한_줄로(무관한_문장)) is None
