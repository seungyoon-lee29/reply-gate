...
# ── 문서 ↔ 코드 — 자기참조를 끊는 두 번째 사본 ─────────────────────────────

#: 사람이 소유한 사본. **가드가 정의를 소유하고, 이 문서가 그것을 이름으로 다시 적는다.**
#:
#: 왜 두 벌인가 — 목록이 코드에만 있으면 "허용 목록의 모든 타입이 통과한다"는 검사가 무엇을
#: 통과시켜야 하는지를 **구현이 스스로 정한다**(사이클 5 감사 §3-3: *"구현자가 정한 목록으로
#: 구현자를 검증한다"*). 사람이 읽고 승인하는 사본이 하나 더 있어야 넓히는 변경이 리뷰에
#: 걸린다. 이 검사는 두 사본이 갈리는 순간 RED 를 낸다.
_보안_문서 = Path(sql_guard.__file__).resolve().parents[2] / "docs" / "security.md"

#: 그 사본이 시작하는 자리. 문서 전체에서 백틱 토큰을 긁으면 옆 문단의 반례(`regclass`)까지
#: 딸려 오므로, 절을 먼저 잘라낸다.
_사본_머리 = "**허용 타입 13종을 여기에 이름으로 적는다**"


def _사본_절(본문: str) -> str:
    """허용 타입을 이름으로 적은 문단 하나. 없으면 빈 문자열 — 검사가 그것을 잡는다."""
    if _사본_머리 not in 본문:
        return ""
    시작 = 본문.index(_사본_머리)
    return 본문[시작 : 본문.index("\n\n", 시작)]


def _사본이_적은_타입(본문: str) -> tuple[str, ...]:
    """**가드 목록에서 유도한다** — 가드의 이름 각각이 그 절에 실제로 적혀 있는가."""
    절 = _사본_절(본문)
    return tuple(name for name in ALLOWED_CAST_TYPES if f"`{name}`" in 절)


def test_보안_문서가_허용_타입_전부를_이름으로_적는다() -> None:
    """한쪽만 넓히면 여기서 걸린다 — 문서는 사람이 승인하는 사본이다."""
    본문 = _보안_문서.read_text(encoding="utf-8")

    assert _사본이_적은_타입(본문) == ALLOWED_CAST_TYPES
    assert len(ALLOWED_CAST_TYPES) == 13, "문서가 '13종'이라고 적는다 — 개수도 함께 움직인다"
    assert "regclass" not in _사본_절(본문), "반례가 허용 목록 문면에 섞이면 절이 거짓을 말한다"


def test_사본_대조가_빠진_이름과_사라진_절을_RED로_잡는다() -> None:
    """**음성 대조** — 대조기가 실제로 무언가를 잡는다(`tests/AGENTS.md` 불변식 3)."""
    온전한 = _보안_문서.read_text(encoding="utf-8")
    한_이름_빠짐 = 온전한.replace(f"`{ALLOWED_CAST_TYPES[-1]}`", "생략", 1)
    절_사라짐 = 온전한.replace(_사본_머리, "삭제된 머리말", 1)

    assert _사본이_적은_타입(한_이름_빠짐) != ALLOWED_CAST_TYPES
    assert set(ALLOWED_CAST_TYPES) - set(_사본이_적은_타입(한_이름_빠짐)) == {
        ALLOWED_CAST_TYPES[-1]
    }
    assert _사본이_적은_타입(절_사라짐) == ()
