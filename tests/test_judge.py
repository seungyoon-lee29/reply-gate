...
def test_시스템_프롬프트는_두_사유의_독립_판정을_못박는다() -> None:
    """뒷받침 판정과 모순 감지는 서로 독립이다 — 한쪽 성립이 다른 쪽 검사를 대체하지 않는다.

    docs/business-rules.md "기각 사유 2종" 은 두 사유의 발화 조건을 각각 따로 정의한다.
    지시가 그 독립성을 말하지 않으면 판정기가 한 사유를 찾은 뒤 나머지 검사를 건너뛰어도
    산출이 형식상 정합해 fail-closed 검증에 걸리지 않는다 — 사유별 지표만 조용히 낮아진다.
    """
    # 두 판정이 독립이라는 선언.
    assert "독립" in JUDGE_SYSTEM_PROMPT
    # 생략 금지 — "검사를 건너뛰지 않는다".
    assert "생략하지 않는다" in JUDGE_SYSTEM_PROMPT
    # 모순 감지가 뒷받침 판정 결과와 무관하게 항상 도는가.
    assert "뒷받침 판정 결과와 무관하게 항상 수행한다" in JUDGE_SYSTEM_PROMPT
    # 민감도 상향이 아니라는 것을 문면이 스스로 못박는다 — 기준은 그대로다.
    assert "기준을 낮추라는 것이 아니다" in JUDGE_SYSTEM_PROMPT
