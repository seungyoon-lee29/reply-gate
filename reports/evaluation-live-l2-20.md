...
- 실행 시각(UTC): `2026-08-19T09:11:33+00:00`
- 생성 LLM: 미실행
- 임베딩: 미실행
...
| `judge_model` | claude-haiku-4-5 | **선언된 실험 변인** |
| `judge_effort` | 기본값 |  |
...
- **L2 검출률: 83.3%** (5/6)
- **L2 오탐률: 20.0%** (1/5)
- 사유 목록까지 정확히 일치: 72.7% (8/11)
- claim 단위 판정 일치: 94.7% (18/19)
- 모순 근거쌍: 기대 4건 중 2건 검출 (기대 밖 검출 0건)
- 판정 토큰: 입력 27587 / 출력 3206 (픽스처당 2799.4)
...
| `contradictory_evidence` | 3 | 1 | 33.3% | 0 |

### 기대와 어긋난 픽스처

- `J07` (contradiction_uncited): 기대 reject[contradictory_evidence] → 실제 pass
- `J10` (direct_no_value): 기대 pass → 실제 reject[unsupported_claim]
- `J11` (combined): 기대 reject[unsupported_claim, contradictory_evidence] → 실제 reject[unsupported_claim]

## 회귀 가드 — 이중 기준선 두 줄 보고

- 이번 실측 세트: 2/3회 `evaluation-live-l2-19`, `evaluation-live-l2-20`
  - 조건 불일치로 세트에서 제외: `evaluation-live-l2-18` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-17` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-16` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-15` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-14` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-13` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-12` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-11` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-10` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-6` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-5` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-4` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-3` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-2` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5` · `evaluation-live-l2-1` — 조건이 다르다: judge_model: 기준선 `claude-haiku-4-5` → 이번 `claude-sonnet-5`
...
- 선언된 실험 변인(대조 진행): judge_model: 기준선 `claude-sonnet-5` → 이번 `claude-haiku-4-5` · judge_fixture_version: 기준선 `fixture-836322e97320` → 이번 `fixture-2b8f33392b36`
- 참고: 선언된 실험 변인이 있어 대조를 진행한다 — 차이: judge_model: 기준선 `claude-sonnet-5` → 이번 `claude-haiku-4-5` · judge_fixture_version: 기준선 `fixture-836322e97320` → 이번 `fixture-2b8f33392b36`
- 참고: 측정 3 은 무변경 검사 대상이 아니다 — 판정 층 개선의 성공이 무관한 축의 원복을 발동시키면 안 되기 때문이다. 기준선 100.0% / 이번 83.3% — 값이 달라졌고 이 실행이 판정 층 변경을 선언했다(`judge_model`, `judge_fixture_version`).
- 참고: 합산 일치율은 하한 경보선이고 사이클 판정은 케이스 단위가 맡는다.

### 직전 라이브 (경보) — **대조 불가**

- 사유: 선언되지 않은 조건 불일치 — 어긋난 항목: judge_prompt_version: 기준선 `judge-7fad677f9ac6` → 이번 `judge-e26f6c8e0b84`
- 기준선 산출물: `evaluation-live-l2-16`, `evaluation-live-l2-17`, `evaluation-live-l2-18`
- 기준선 출처: `reports/ 자동 탐색`
- **대조 불가 — 선언 없이 어긋난 항목**: judge_prompt_version: 기준선 `judge-7fad677f9ac6` → 이번 `judge-e26f6c8e0b84`
- 선언된 실험 변인(대조 진행): judge_model: 기준선 `claude-sonnet-5` → 이번 `claude-haiku-4-5`
- 참고: 선언된 실험 변인이 있어 대조를 진행한다 — 차이: judge_model: 기준선 `claude-sonnet-5` → 이번 `claude-haiku-4-5`
- 참고: 측정 3 은 무변경 검사 대상이 아니다 — 판정 층 개선의 성공이 무관한 축의 원복을 발동시키면 안 되기 때문이다. 기준선 100.0% / 이번 83.3% — 값이 달라졌고 이 실행이 판정 층 변경을 선언했다(`judge_model`, `judge_fixture_version`).
- 참고: 합산 일치율은 하한 경보선이고 사이클 판정은 케이스 단위가 맡는다.
