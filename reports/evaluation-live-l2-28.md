...
- 실행 시각(UTC): `2026-08-21T03:55:30+00:00`
...
- 지연 p50: 9516 ms / p95: 35684 ms (파이프라인 `run` 의 벽시계 시간 — 처리 기록 저장은 포함하지 않는다)
...
- 생성 계열 프롬프트 캐시(생성 입력 토큰과 별도 칸): write 1059 / read 0 — 위 '생성 입력'은 **캐시 적중분을 포함한 총 입력**이다(빼지도 더하지도 않는다)
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `intent` | 30 | 0 | 42028.6 | 1401.0 | 1248.2 | 2453.5 |
| `query_rewrite` | 21 | 9 | 35298.0 | 1680.9 | 1446.9 | 3730.1 |
| `inquiry_embedding` | 21 | 9 | 7627.1 | 363.2 | 181.5 | 830.0 |
| `policy_search` | 21 | 9 | 629.6 | 30.0 | 29.3 | 40.7 |
| `sql_generation` | 9 | 21 | 15303.7 | 1700.4 | 1645.3 | 2092.3 |
| `sql_execution` | 11 | 19 | 77.2 | 7.0 | 6.7 | 14.2 |
| `draft` | 22 | 8 | 46159.5 | 2098.2 | 1848.3 | 4221.8 |
| `gate` | 22 | 8 | 10.3 | 0.5 | 0.4 | 0.9 |
| `l2_judge` | 22 | 8 | 185855.9 | 8448.0 | 7339.2 | 26608.3 |
...
- `G09`: `sql:d3adde55-e444-4e33-b50d-0d6eec8f152b:1`
- `G10`: `sql:90416284-381c-474a-8a70-c4447ccd88d4:1`
- `G11`: `sql:796f3d8b-1921-4354-b3f5-663b74233b45:1`
- `G12`: `policy:support:4-3`, `policy:refund:2-4`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-3`, `sql:089dbdc2-8245-45b7-a9f5-6938bceedee5:1`
- `G13`: `sql:6d224661-9a2a-46cd-bcba-f35596fc7727:1`
- `G14`: `policy:exchange:3-1`, `policy:exchange:3-5`, `policy:exchange:3-4`, `policy:exchange:3-2`, `policy:refund:2-2`, `sql:f520e8ac-ad54-4367-ba13-dc68087e762a:1`
- `G15`: `policy:refund:2-5`, `policy:exchange:3-3`, `policy:shipping:1-7`, `policy:shipping:1-6`, `policy:refund:2-7`, `sql:0e7a13ec-e028-4fc3-ae0a-e9dfaece3368:1`
...
- `G27`: 없음
- `G28`: 없음
- `G29`: `sql:131b6561-d294-4de6-be0a-87a11aa7b884:1`
- `G30`: `sql:aeadb9ef-b735-4c5d-8429-784e223eaa73:1`
...
- `G09`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:d3adde55-e444-4e33-b50d-0d6eec8f152b:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G10`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:90416284-381c-474a-8a70-c4447ccd88d4:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G11`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-1` / 채택 근거 `sql:796f3d8b-1921-4354-b3f5-663b74233b45:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G13`: **근거 없이 답변 확정** — 정답 근거 `policy:refund:2-4` / 채택 근거 `sql:6d224661-9a2a-46cd-bcba-f35596fc7727:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- `G29`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:131b6561-d294-4de6-be0a-87a11aa7b884:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G30`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:aeadb9ef-b735-4c5d-8429-784e223eaa73:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- 판정 토큰: 입력 30042 / 출력 5672 (픽스처당 3246.7)
- 판정 호출 지연: 평균 6436.2 ms / p50 5467.1 ms / p95 17646.1 ms (측정 11건 / 미측정 0건 — 재시도와 판정 실패 호출을 포함한 총 벽시계다)
...
- 이번 실측 세트: 1/3회 `evaluation-live-l2-28`
