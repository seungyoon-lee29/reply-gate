...
- 실행 시각(UTC): `2026-08-19T04:02:30+00:00`
...
- 지연 p50: 12366 ms / p95: 22411 ms (파이프라인 `run` 의 벽시계 시간 — 처리 기록 저장은 포함하지 않는다)
...
- `G05`: `policy:support:4-3`, `policy:support:4-4`, `policy:support:4-5`, `policy:support:4-2`, `policy:refund:2-2`
...
- `G09`: `sql:a1c9e1d4-291c-4af1-b208-49870d92f86f:1`
- `G10`: `sql:c91fe83c-4f95-4869-b7d1-80ca108917e0:1`
- `G11`: `sql:fe25ff4b-1abc-4b29-972c-2bba46dce443:1`
- `G12`: `policy:support:4-3`, `policy:refund:2-4`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-3`, `sql:63e57d09-234f-45b3-a385-d290d07db023:1`
- `G13`: `sql:9e0b8dd7-0c60-4350-a985-9efec81c0fc8:1`
- `G14`: `sql:63a9dc46-71b7-45c4-b6ab-7efcfa77a501:1`
- `G15`: `policy:refund:2-5`, `policy:exchange:3-3`, `policy:refund:2-7`, `policy:shipping:1-7`, `policy:shipping:1-4`, `sql:270e0262-0153-4299-8ec3-4b9024bdd181:1`
...
- `G19`: `policy:support:4-2`, `policy:support:4-5`, `policy:support:4-1`, `policy:support:4-4`, `policy:support:4-7`
- `G20`: `policy:support:4-2`, `policy:support:4-1`, `policy:support:4-5`, `policy:support:4-4`, `policy:support:4-7`
- `G21`: `policy:shipping:1-5`, `policy:shipping:1-6`, `policy:shipping:1-1`, `policy:exchange:3-2`, `policy:shipping:1-2`
...
- `G29`: `sql:d0afaf8e-1aa2-40e1-9161-eed2e63ab6b3:1`
- `G30`: `sql:6b642b32-feba-4f72-9dff-68aeaa4f0b71:1`
...
- `G09`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:a1c9e1d4-291c-4af1-b208-49870d92f86f:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G10`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:c91fe83c-4f95-4869-b7d1-80ca108917e0:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G11`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-1` / 채택 근거 `sql:fe25ff4b-1abc-4b29-972c-2bba46dce443:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G13`: **근거 없이 답변 확정** — 정답 근거 `policy:refund:2-4` / 채택 근거 `sql:9e0b8dd7-0c60-4350-a985-9efec81c0fc8:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G14`: **근거 없이 답변 확정** — 정답 근거 `policy:exchange:3-5` / 채택 근거 `sql:63a9dc46-71b7-45c4-b6ab-7efcfa77a501:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G21`: **빈 정답 정상 인계** — 정답 근거 없음 / 채택 근거 `policy:shipping:1-5`, `policy:shipping:1-6`, `policy:shipping:1-1`, `policy:exchange:3-2`, `policy:shipping:1-2` / 근거 채택 후 L2 검출
...
- `G29`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:d0afaf8e-1aa2-40e1-9161-eed2e63ab6b3:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G30`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:6b642b32-feba-4f72-9dff-68aeaa4f0b71:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- 판정 토큰: 입력 29928 / 출력 5723 (픽스처당 3241.0)
...
- 이번 실측 세트: 3/3회 `evaluation-live-l2-2`, `evaluation-live-l2-3`, `evaluation-live-l2-7`
