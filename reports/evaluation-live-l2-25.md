...
- 실행 시각(UTC): `2026-08-19T09:21:26+00:00`
- 생성 LLM: 미실행
- 임베딩: 미실행
...
| `judge_prompt_caching` | on | **선언된 실험 변인** |
| `measurement_scope` | measurement_1_3_only |  |
| `generation_effort` | 기본값 |  |
| `l2_enabled` | on |  |

선언된 실험 변인: `judge_prompt_caching`, `judge_fixture_version`
...
- 판정 토큰: 입력 6832 / 출력 5488 (픽스처당 1120.0)
- 판정 프롬프트 캐시(판정 입력 토큰과 별도 칸): write 0 / read 23210 — 위 '판정 토큰 입력'은 **캐시 적중분을 제외한 비캐시 입력**이다
...
- 이번 실측 세트: 1/3회 `evaluation-live-l2-25`
  - 조건 불일치로 세트에서 제외: `evaluation-live-l2-18` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-17` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-16` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-15` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-14` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-13` — 조건이 다르다: judge_prompt_caching: 기준선 `on` → 이번 `off` · `evaluation-live-l2-12` — 조건이 다르다: judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · judge_prompt_caching: 기준선 `on` → 이번 `off` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-11` — 조건이 다르다: judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · judge_prompt_caching: 기준선 `on` → 이번 `off` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-10` — 조건이 다르다: judge_fixture_version: 기준선 `fixture-2b8f33392b36` → 이번 `fixture-836322e97320` · judge_prompt_caching: 기준선 `on` → 이번 `off` · measurement_scope: 기준선 `measurement_1_3_only` → 이번 `full` · `evaluation-live-l2-6` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-5` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-4` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-3` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개) · `evaluation-live-l2-2` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개) · `evaluation-live-l2-1` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개)
...
- 선언된 실험 변인(대조 진행): judge_fixture_version: 기준선 `fixture-836322e97320` → 이번 `fixture-2b8f33392b36` · judge_prompt_caching: 기준선 `off` → 이번 `on`
- 참고: 선언된 실험 변인이 있어 대조를 진행한다 — 차이: judge_fixture_version: 기준선 `fixture-836322e97320` → 이번 `fixture-2b8f33392b36` · judge_prompt_caching: 기준선 `off` → 이번 `on`
...
- 선언된 실험 변인(대조 진행): judge_prompt_caching: 기준선 `off` → 이번 `on`
- 참고: 선언된 실험 변인이 있어 대조를 진행한다 — 차이: judge_prompt_caching: 기준선 `off` → 이번 `on`
- 참고: 측정 3 은 무변경 검사 대상이 아니다 — 판정 층 개선의 성공이 무관한 축의 원복을 발동시키면 안 되기 때문이다. 기준선 100.0% / 이번 100.0% — 두 세트의 값이 같다.
- 참고: 합산 일치율은 하한 경보선이고 사이클 판정은 케이스 단위가 맡는다.
