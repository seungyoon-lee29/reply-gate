...
- 실행 시각(UTC): `2026-08-19T04:15:41+00:00`
...
- 지연 p50: 12859 ms / p95: 42098 ms (파이프라인 `run` 의 벽시계 시간 — 처리 기록 저장은 포함하지 않는다)
...
- 인계 사유: missing_order_ref 2건, order_not_found 2건, rejected_twice 4건
...
- `G09`: `sql:e32b927b-700c-487f-86ea-1bed1e94a3ca:1`
- `G10`: `sql:1b4d9ea3-80dd-4393-a457-496278f055bb:1`
- `G11`: `sql:4f23e971-981e-4580-805e-57ea75eccd88:1`
- `G12`: `policy:refund:2-4`, `policy:support:4-3`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-2`, `sql:8841be06-0614-40ad-8749-04ad80a01c7c:1`
- `G13`: `sql:8e27ad93-5bf2-4d8c-816a-bbe3d18eb080:1`
- `G14`: `sql:113ac034-1846-46e8-9ada-c0167a64b1f0:1`
- `G15`: `policy:refund:2-5`, `policy:exchange:3-3`, `policy:shipping:1-7`, `policy:shipping:1-6`, `policy:shipping:1-3`, `sql:ecafd618-6d7b-4455-9ad5-0c4759060ab6:1`
- `G16`: `policy:support:4-1`, `policy:support:4-2`, `policy:support:4-5`, `policy:support:4-4`, `policy:support:4-3`
- `G17`: `policy:support:4-5`, `policy:support:4-1`, `policy:support:4-2`, `policy:support:4-4`, `policy:support:4-7`
...
- `G21`: `policy:shipping:1-6`, `policy:shipping:1-2`, `policy:exchange:3-2`, `policy:shipping:1-4`, `policy:shipping:1-1`
...
- `G26`: 없음
- `G27`: 없음
- `G28`: `policy:support:4-3`, `policy:support:4-4`, `policy:support:4-5`, `policy:refund:2-4`, `policy:refund:2-6`
- `G29`: `sql:50a3f64e-d5f2-44d8-8722-fcf0e69b2272:1`
- `G30`: `sql:cd8eb914-4639-444e-b0a5-433298e004a3:1`
...
- `G09`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:e32b927b-700c-487f-86ea-1bed1e94a3ca:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G10`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:1b4d9ea3-80dd-4393-a457-496278f055bb:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G11`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-1` / 채택 근거 `sql:4f23e971-981e-4580-805e-57ea75eccd88:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G13`: **근거 없이 답변 확정** — 정답 근거 `policy:refund:2-4` / 채택 근거 `sql:8e27ad93-5bf2-4d8c-816a-bbe3d18eb080:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G14`: **근거 없이 답변 확정** — 정답 근거 `policy:exchange:3-5` / 채택 근거 `sql:113ac034-1846-46e8-9ada-c0167a64b1f0:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G21`: **빈 정답 정상 인계** — 정답 근거 없음 / 채택 근거 `policy:shipping:1-6`, `policy:shipping:1-2`, `policy:exchange:3-2`, `policy:shipping:1-4`, `policy:shipping:1-1` / 근거 채택 후 L2 검출
...
- `G28`: **빈 정답 정상 인계** — 정답 근거 없음 / 채택 근거 `policy:support:4-3`, `policy:support:4-4`, `policy:support:4-5`, `policy:refund:2-4`, `policy:refund:2-6` / 주문 단계 사전 인계 — 구조적 사유가 이긴다(계약상 정상)
- `G29`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:50a3f64e-d5f2-44d8-8722-fcf0e69b2272:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G30`: **검색 실패(전부 누락)** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:cd8eb914-4639-444e-b0a5-433298e004a3:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- 판정 토큰: 입력 29928 / 출력 5793 (픽스처당 3247.4)
...
- 이번 실측 세트: 3/3회 `evaluation-live-l2-3`, `evaluation-live-l2-7`, `evaluation-live-l2-8`
...
- 지문 미상 항목(기준선 또는 이번 실측에 값이 없다 — 같다고도 다르다고도 적지 않는다): `label_version`, `retrieval_labels_version`, `abstention_gate_statistic`, `abstention_tau`, `query_rewrite`, `embedding_dimensions`, `judge_prompt_version`, `judge_fixture_version`, `judge_prompt_caching`, `measurement_scope`
