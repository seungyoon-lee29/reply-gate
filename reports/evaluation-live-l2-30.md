...
- 실행 시각(UTC): `2026-08-21T04:09:49+00:00`
...
- 지연 p50: 9585 ms / p95: 34278 ms (파이프라인 `run` 의 벽시계 시간 — 처리 기록 저장은 포함하지 않는다)
...
- 생성 계열 프롬프트 캐시(생성 입력 토큰과 별도 칸): write 3288 / read 0 — 위 '생성 입력'은 **캐시 적중분을 포함한 총 입력**이다(빼지도 더하지도 않는다)
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `intent` | 30 | 0 | 41180.4 | 1372.7 | 1286.2 | 1698.3 |
| `query_rewrite` | 19 | 11 | 27823.3 | 1464.4 | 1359.6 | 2862.8 |
| `inquiry_embedding` | 19 | 11 | 5286.6 | 278.2 | 189.9 | 669.1 |
| `policy_search` | 19 | 11 | 592.0 | 31.2 | 31.6 | 43.2 |
| `sql_generation` | 9 | 21 | 17676.0 | 1964.0 | 1711.0 | 4021.5 |
| `sql_execution` | 11 | 19 | 92.7 | 8.4 | 7.6 | 16.7 |
| `draft` | 22 | 8 | 53930.6 | 2451.4 | 1921.4 | 4883.9 |
| `gate` | 22 | 8 | 14.0 | 0.6 | 0.5 | 1.4 |
| `l2_judge` | 22 | 8 | 223186.6 | 10144.8 | 7080.3 | 26873.7 |
...
- `G09`: `sql:2b873d79-ba61-47ef-bab1-b8b62e2b9f32:1`
- `G10`: `sql:5f9e483b-2624-40ac-85ae-45afd9de8146:1`
- `G11`: `sql:84c541cb-b2bc-45bd-9d6d-fb240cf4de48:1`
- `G12`: `policy:refund:2-4`, `policy:support:4-3`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-2`, `sql:714bcc39-17fd-464d-9d6c-bff0111c0666:1`
- `G13`: `sql:8d39c941-a116-4beb-80a3-cf3659fde7b6:1`
- `G14`: `sql:16fdd8fc-3b7d-4a3b-b63c-d05edb74b73c:1`
- `G15`: `policy:exchange:3-3`, `policy:refund:2-5`, `policy:shipping:1-6`, `policy:shipping:1-7`, `policy:shipping:1-3`, `sql:d9b4ade9-da85-4023-88dd-3aa2a899d69a:1`
...
- `G27`: 없음
- `G28`: 없음
- `G29`: `sql:ca026905-521c-4ece-a333-7a109f0cc527:1`
- `G30`: `sql:ff387549-3ed0-4e8a-a070-dfdb408e76e0:1`
...
- 생성 문제: **4건** — 정답 조항을 **전부** 채택했지만 기각·인계
...
- `G02`: **생성 문제** — 정답 근거 `policy:shipping:1-3`, `policy:shipping:1-4` / 채택 근거 `policy:shipping:1-4`, `policy:shipping:1-3`, `policy:shipping:1-7`, `policy:shipping:1-6`, `policy:shipping:1-2` / 최종 answered·라벨 일치
- `G09`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:2b873d79-ba61-47ef-bab1-b8b62e2b9f32:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G10`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:5f9e483b-2624-40ac-85ae-45afd9de8146:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G11`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-1` / 채택 근거 `sql:84c541cb-b2bc-45bd-9d6d-fb240cf4de48:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G12`: **생성 문제** — 정답 근거 `policy:refund:2-4` / 채택 근거 `policy:refund:2-4`, `policy:support:4-3`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-2`, `sql:714bcc39-17fd-464d-9d6c-bff0111c0666:1` / 최종 answered·라벨 일치
- `G13`: **근거 없이 답변 확정** — 정답 근거 `policy:refund:2-4` / 채택 근거 `sql:8d39c941-a116-4beb-80a3-cf3659fde7b6:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G14`: **근거 없이 답변 확정** — 정답 근거 `policy:exchange:3-5` / 채택 근거 `sql:16fdd8fc-3b7d-4a3b-b63c-d05edb74b73c:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- `G29`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:ca026905-521c-4ece-a333-7a109f0cc527:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G30`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:ff387549-3ed0-4e8a-a070-dfdb408e76e0:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- 판정 토큰: 입력 30042 / 출력 5893 (픽스처당 3266.8)
- 판정 호출 지연: 평균 6617.9 ms / p50 6209.6 ms / p95 14832.4 ms (측정 11건 / 미측정 0건 — 재시도와 판정 실패 호출을 포함한 총 벽시계다)
...
- 이번 실측 세트: 3/3회 `evaluation-live-l2-28`, `evaluation-live-l2-29`, `evaluation-live-l2-30`
