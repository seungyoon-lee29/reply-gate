...
- 실행 시각(UTC): `2026-08-19T05:17:01+00:00`
...
- 지연 p50: 10429 ms / p95: 38627 ms (파이프라인 `run` 의 벽시계 시간 — 처리 기록 저장은 포함하지 않는다)
...
- `G05`: `policy:support:4-3`, `policy:support:4-4`, `policy:support:4-2`, `policy:refund:2-1`, `policy:refund:2-2`
...
- `G08`: `policy:shipping:1-7`, `policy:shipping:1-6`, `policy:shipping:1-4`, `policy:shipping:1-5`, `policy:shipping:1-2`
- `G09`: `sql:b56cc01c-c1a0-4fab-a0d8-890917baff33:1`
- `G10`: `sql:9c1790aa-be29-4382-81c6-d83267f3e496:1`
- `G11`: `sql:18497837-e85f-46cd-8b2a-f78fc78888b5:1`
- `G12`: `policy:refund:2-4`, `policy:support:4-3`, `policy:refund:2-1`, `policy:refund:2-6`, `policy:refund:2-2`, `sql:a3e43d4e-7ef0-41db-821a-10707885113e:1`
- `G13`: `sql:a9bf3d27-c708-4413-8e18-70eb626e76b9:1`
- `G14`: `sql:92e36b79-1ae5-4126-94d8-aed004c4c683:1`
- `G15`: `policy:refund:2-5`, `policy:exchange:3-3`, `policy:shipping:1-7`, `policy:shipping:1-6`, `policy:refund:2-7`, `sql:5f65cf1a-5fe7-4d56-a190-31673607dca4:1`
- `G16`: `policy:support:4-1`, `policy:support:4-2`, `policy:support:4-5`, `policy:support:4-4`, `policy:support:4-3`
- `G17`: `policy:support:4-1`, `policy:support:4-5`, `policy:support:4-2`, `policy:support:4-4`, `policy:support:4-7`
...
- `G26`: 없음
- `G27`: 없음
- `G28`: `policy:refund:2-4`, `policy:refund:2-6`, `policy:refund:2-3`, `policy:refund:2-5`, `policy:exchange:3-4`
- `G29`: `sql:5674b7bb-bb85-41f7-85b1-790cfa77ece6:1`
- `G30`: `sql:f36b2ab1-cb81-48d6-9f4f-32c71e3e57b0:1`
...
- `G02`: **생성 문제** — 정답 근거 `policy:shipping:1-3`, `policy:shipping:1-4` / 채택 근거 `policy:shipping:1-3`, `policy:shipping:1-4`, `policy:shipping:1-6`, `policy:shipping:1-7`, `policy:shipping:1-2` / 최종 answered·라벨 일치
- `G09`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:b56cc01c-c1a0-4fab-a0d8-890917baff33:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G10`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-5` / 채택 근거 `sql:9c1790aa-be29-4382-81c6-d83267f3e496:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G11`: **근거 없이 답변 확정** — 정답 근거 `policy:shipping:1-1` / 채택 근거 `sql:18497837-e85f-46cd-8b2a-f78fc78888b5:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G13`: **근거 없이 답변 확정** — 정답 근거 `policy:refund:2-4` / 채택 근거 `sql:a9bf3d27-c708-4413-8e18-70eb626e76b9:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G14`: **근거 없이 답변 확정** — 정답 근거 `policy:exchange:3-5` / 채택 근거 `sql:92e36b79-1ae5-4126-94d8-aed004c4c683:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- `G28`: **빈 정답 정상 인계** — 정답 근거 없음 / 채택 근거 `policy:refund:2-4`, `policy:refund:2-6`, `policy:refund:2-3`, `policy:refund:2-5`, `policy:exchange:3-4` / 주문 단계 사전 인계 — 구조적 사유가 이긴다(계약상 정상)
- `G29`: **근거 없이 답변 확정** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:5674b7bb-bb85-41f7-85b1-790cfa77ece6:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
- `G30`: **검색 실패(전부 누락)** — 정답 근거 `policy:support:4-5` / 채택 근거 `sql:f36b2ab1-cb81-48d6-9f4f-32c71e3e57b0:1` / **정책 검색 미실행**(의도 분류가 `order` 로 라우팅해 정책 검색이 실행되지 않았다)
...
- **L2 검출률: 100.0%** (6/6)
- **L2 오탐률: 20.0%** (1/5)
- 사유 목록까지 정확히 일치: 81.8% (9/11)
- claim 단위 판정 일치: 94.7% (18/19)
- 모순 근거쌍: 기대 4건 중 3건 검출 (기대 밖 검출 0건)
- 판정 토큰: 입력 29928 / 출력 5565 (픽스처당 3226.6)

### 사유 2종별 내역

| 사유 | 기대 픽스처 | 검출 | 검출률 | 오발화(기대하지 않은 발화) |
| --- | ---: | ---: | ---: | ---: |
| `unsupported_claim` | 4 | 4 | 100.0% | 1 |
| `contradictory_evidence` | 3 | 2 | 66.7% | 0 |

### 기대와 어긋난 픽스처

- `J04` (direct_no_value): 기대 pass → 실제 reject[unsupported_claim]
- `J11` (combined): 기대 reject[unsupported_claim, contradictory_evidence] → 실제 reject[unsupported_claim]

## 회귀 가드 — 이중 기준선 두 줄 보고

- 이번 실측 세트: 1/3회 `evaluation-live-l2-10`
...
- 사유: 판정 보류 — 조건이 확인된 실측 1/3 — 세트가 아직 안 찼다 (조건 불일치로 제외: `evaluation-live-l2-6` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-5` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-4` — 조건이 다르다: acceptance_cut: 기준선 `0.3` → 이번 `0.5` · `evaluation-live-l2-3` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개) · `evaluation-live-l2-2` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개) · `evaluation-live-l2-1` — 조건 지문이 확인되지 않는다(미상 11개: label_version, retrieval_labels_version, abstention_gate_statistic 외 8개)). 아래 항목은 잠정 관측이다.
...
- **일치 감소(경보)**: `G01` — 기준선 3/3 → 이번 1/1 · `G02` — 기준선 3/3 → 이번 1/1 · `G03` — 기준선 3/3 → 이번 1/1 · `G04` — 기준선 3/3 → 이번 1/1 · `G05` — 기준선 3/3 → 이번 1/1 · `G06` — 기준선 3/3 → 이번 1/1 · `G07` — 기준선 3/3 → 이번 1/1 · `G08` — 기준선 3/3 → 이번 1/1 · `G09` — 기준선 3/3 → 이번 1/1 · `G10` — 기준선 3/3 → 이번 1/1 · `G11` — 기준선 3/3 → 이번 1/1 · `G12` — 기준선 3/3 → 이번 1/1 · `G13` — 기준선 3/3 → 이번 1/1 · `G14` — 기준선 3/3 → 이번 1/1 · `G15` — 기준선 3/3 → 이번 1/1 · `G16` — 기준선 3/3 → 이번 1/1 · `G17` — 기준선 3/3 → 이번 1/1 · `G18` — 기준선 3/3 → 이번 1/1 · `G19` — 기준선 3/3 → 이번 1/1 · `G20` — 기준선 3/3 → 이번 1/1 · `G21` — 기준선 3/3 → 이번 1/1 · `G22` — 기준선 3/3 → 이번 1/1 · `G23` — 기준선 3/3 → 이번 1/1 · `G24` — 기준선 3/3 → 이번 1/1 · `G25` — 기준선 3/3 → 이번 1/1 · `G26` — 기준선 3/3 → 이번 1/1 · `G27` — 기준선 3/3 → 이번 1/1 · `G28` — 기준선 3/3 → 이번 1/1 · `G29` — 기준선 3/3 → 이번 1/1 · `G30` — 기준선 3/3 → 이번 1/1
