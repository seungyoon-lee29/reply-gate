# Reply-Gate

**근거 없는 답변을 스스로 기각하는 이커머스 CS 답변 에이전트.**
초안을 잘 쓰는 것이 아니라 **틀린 초안을 걸러내는 것**이 이 제품의 존재 이유다.
답변을 확정하기 전에 **게이트 2층**(결정론 L1 → 의미 검증 L2)이 통과시키고, 통과시키지 못하면 사람에게 넘긴다.

> **문제정의 — 실측 5사이클이 다듬은 현재형** ([전문](docs/problem.md))
>
> LLM CS 답변의 실패는 "그럴듯한 값을 지어낸다"로 시작했지만, 이 저장소의 실측은 그 서술을
> 수정했다: 요즘 상위 모델은 값을 거의 지어내지 않는다(**미끼 95 케이스-실행에서 `pii_detected`
> 발화 0회** — 다섯 사이클 **풀셋 라이브 19회** 전부).
> **실제로 재현되는 실패는 주제를 다루지 않는 인접 근거를 딛고 답하는 것** — 그리고 그것을
> "근거가 있나?"를 LLM 에게 되묻는 방식으로는 재현 가능한 숫자로 증명할 수 없다는 원문제는
> 그대로다. 그래서 검사 가능한 구조(`claim + citation_ids`)와 LLM 0회 결정론 게이트(L1)가
> 먼저 서고, 의미 검증(L2)이 그 위에 선다.
>
> 사이클 3 은 **게이트 앞(검색 품질)** 을 같은 방식으로 재봤다. "못 찾는다"는 질의 재작성이
> 닫았고, **"잘못 찾는다"는 유사도 임계값 한 축으로는 닫히지 않는다**는 것이 두 번 실측됐다 —
> 무근거 문의를 걸러내는 컷이 정상 문의의 근거와 모순 판정에 필요한 상충 조항까지 함께 자른다.
>
> **사이클 4 는 컷을 옮기는 대신 축을 하나 늘렸다.** 채택 판정을 **질의 축(기권 게이트) →
> 항목 축(절대 하한)** 두 겹으로 나눴고, 라이브 3회가 컷 하나로는 못 지키던 **세 방향을
> 동시에** 성립시켰다([그 실측](#새-축-라이브-3회--세-방향이-동시에-성립했다)). 그래도
> **인접 근거를 딛는 답변은 여전히 L2 가 잡는다** — 게이트가 거르는 것은 "아무것도 관련 없는"
> 질의뿐이다([그 장면](#3-2-l2-가-잡는-장면--근거를-달았는데-그-근거가-답을-뒷받침하지-않는다) ·
> [검색 품질 절](#검색-품질--게이트-앞의-층) ·
> [문제정의 7절](docs/problem.md#7-실측이-문제정의를-어떻게-바꿨나)).
>
> **사이클 5 는 지표의 발밑을 고쳤다.** 현재 기본값 조합으로 풀셋 라이브를 돌린 세트가 하나도
> 없어 회귀 판정의 구속 줄이 영구히 "대조 불가"였다 — 라이브 3회를 사서 **승격 기준선을
> 재등재**했고, 확인 라이브 1회에서 조건 지문 25칸이 완전히 일치해 대조가 실제로 진행됐다.
> 같은 구매가 **단계별 지연**(L2 판정이 8.4~10.1초로 압도적, L1 게이트는 0.5 ms)과
> **계열별 캐시 적중 토큰**을 처음으로 실측했다([그 실측](#사이클-5-실측값--현재-기본값이자-승격-기준선이다)).

---

## 30초 요약

| | |
|---|---|
| **문제** | LLM 이 만든 CS 답변은 그럴듯하지만 근거가 없을 수 있다. 그리고 그 사실을 LLM 에게 다시 물어보는 것으로는 증명할 수 없다. |
| **접근** | 답변을 `claim + citation_ids` 구조로 만들게 하고, **LLM 을 한 번도 호출하지 않는 코드 검사(L1)** 로 근거 무결성을 판정한다. 그다음 **L1 통과분만** claim 단위 의미 검증(L2, LLM-as-a-judge)에 넘긴다. |
| **왜 코드인가** | 게이트의 1층이 결정론이면 **같은 입력에 항상 같은 판정**이 나온다. 검출률·오탐률을 재현 가능한 숫자로 말할 수 있고, 실패했을 때 원인을 코드 한 줄까지 짚을 수 있다. 확률층인 L2 도 **판정 JSON 을 만들 뿐**이고, 그 판정을 재생성·인계·종결로 옮기는 것은 코드다. |
| **경계** | 확률적인 부분(의도 해석·**검색용 질의 재작성**·SQL 생성·초안 생성·L2 판정)과 결정론적인 부분(접수·조회 실행·**근거 채택**·L1·루프 상한·판정의 사용)을 **노드 단위로 갈랐다**. 아래 다이어그램의 `[LLM]` / `[코드]` 표기가 그 경계다. |
| **범위** | 사이클 2 에 **L2 까지 구현·실측**했고, 사이클 3 은 **게이트 앞(검색 품질)을 측정 가능하게** 만들었으며, 사이클 4 는 **채택 축을 두 겹으로 늘리고 회귀 판정을 사람 눈에서 코드(`regression_guard.py`)로 옮겼다**. 사이클 5 는 **현재 기본값으로 풀셋 라이브를 사서 승격 기준선을 재등재하고 단계별 지연·캐시 토큰을 처음 실측**했다([검색 품질 절](#검색-품질--게이트-앞의-층) · [회귀 가드](#회귀-가드--실행마다-두-줄로-보고한다) · [사이클 5 실측값](#사이클-5-실측값--현재-기본값이자-승격-기준선이다)). 하지 않은 것은 [범위 절](#한-것과-하지-않은-것)에 그대로 적어 두었다. |
| **비용** | 문의 1건당 **$0.014493** (현재 기본값 · 3회 90건 실측 · 단가 기준일 2026-08-19). **판정이 그중 71.7%** 이고 임베딩은 0.004% 다 — [비용 절](#비용--계열마다-단가가-다르다). |
| **지연** | 문의 1건 p50 **8.7~9.6초**. 단계별로 갈라 보면 **L2 판정 한 층이 8.4~10.1초**이고 **L1 게이트는 0.5 ms** 다 — 결정론 층이 싸다는 주장이 이제 숫자다([단계별 지연](#사이클-5-실측값--현재-기본값이자-승격-기준선이다)). |

문제를 왜 이렇게 정의했고 무엇으로 성공을 판정하는지는 **[문제정의 문서](docs/problem.md)** 에 따로 정리돼 있다 —
기존 접근이 왜 부족한지, 일부러 심은 실패 장치가 무엇인지, 실측이 문제정의를 어떻게 바꿨는지까지.

기술 스택: Python 3.13 · FastAPI · Postgres 17 + pgvector · OpenAI(생성 `gpt-5.6-terra`, 임베딩 `text-embedding-3-small`) · **Anthropic(L2 판정 `claude-sonnet-5`)** · 에이전트 루프 **자체 구현**(LangGraph 미사용 — [이유](#langgraph-로-짰다면)).

판정 계열을 생성과 **다른 provider** 로 둔 것은 비용도 가용성도 아니다 — 같은 계열로 판정하면
self-judging bias 로 검출률이 오염되기 때문이다.

---

## 데모 시나리오

화면(`GET /`)은 챗 UI 가 아니다. **판정 과정이 답변보다 먼저·크게** 보이도록 만들어져 있다:
시도별 `pass` / `reject` 배지, 기각 사유 코드, 인계 사유, 그리고 수집된 근거 목록.

### 1) 첫 화면 — 인계 (선검사와 판정은 결정론이다)

주문번호 칸에 **존재하지 않는 주문번호**를 넣고 배송 문의를 접수한다.

```
주문번호: ORD-20260101-9999
문의 내용: 이 주문 언제 배송되나요?
```

화면에 바로 뜨는 것:

```
최종 상태: escalated  상담원 인계
상담원 인계 — 인계 사유: order_not_found
L1 게이트 판정 — 시도 이력: (없음)
  "초안 전 인계 — 근거가 없어서 초안 생성에 진입하지 않았다"
수집된 근거: 없음
```

여기서 중요한 것은 **초안이 아예 만들어지지 않았다**는 점이다.
주문 존재성 선검사는 **고정 파라미터 쿼리**(`SELECT 1 AS present FROM orders WHERE order_no = %s LIMIT 1`)이고,
주문이 없으면 text-to-SQL 에 진입조차 하지 않는다 — 없는 주문에 대해 LLM 이 무언가를 지어낼 기회 자체를 없앤다.
LLM 호출은 의도 해석 1회뿐이고(실측 약 210 입력 토큰, 1.1~2.8초), 판정은 코드가 한다.

**그래서 이 장면에도 "항상"을 붙이지 않는다.** 선검사 자체와 `order_not_found` 판정은
결정론이지만, **선검사에 도달할지는 의도 해석이 정한다** — 분류가 `order`/`both` 를 냈을
때만 주문 경로가 돈다(`evidence.py` 의 의도 분기). 확률 층이 앞에 하나 서 있다는 뜻이다.
이 문의에서 그 분류가 흔들린 관측은 없지만, 그것은 관측이지 보증이 아니다.

같은 방식으로 주문번호 없이 "제 주문 배송 상태 알려주세요"를 넣으면 `missing_order_ref` 가 뜬다.
주문번호 **형식** 자체가 틀리면(`1234`) 파이프라인에 들어가지도 않고 **HTTP 422** 로 끝난다 — 형식 오류는 인계가 아니라 요청 오류다.

### 2) 정상 답변 — 근거 ID 가 문장마다 붙는다

```
문의 내용: 배송비는 얼마인가요?
```

`policy:shipping:1-3`(30,000원 기준)과 `policy:shipping:1-4`(50,000원 기준)가 함께 검색된다.
이 두 조항은 **일부러 서로 충돌하게 심어 둔 조항**이다.
실측에서 모델은 두 기준이 다르다는 것을 답변에 명시하고 두 ID 를 모두 인용했으며, L1 은 `pass` 를 냈다 —
**L1 은 "근거끼리 모순인지"를 판정하지 않기 때문이다.** 그것은 L2 의 일이다([게이트 2층 구조](#게이트-2층-구조)).

**사이클 2 실측에서 L2 가 실제로 이 자리를 채웠다.** 골든셋의 같은 문의(G02)에서 3회 중 1회,
모순을 명시하지 않은 첫 초안을 L2 가 `contradictory_evidence` 로 기각했고 재생성이 통과했다.
같은 사유가 환불 기간 상충(G01)에서도 3회 중 2회 발화했다. **L1 이 pass 를 낸 초안을 L2 가
기각하는 장면**이 이 사이클에 처음 생겼다.

**단, 이 장면은 검색에 종속된다** — L2 가 모순을 잡으려면 검색이 **상충하는 두 조항을 함께**
올려야 한다. 사이클 3 이 그것을 반대로 증명했다: 채택 임계값을 올려 한쪽이 잘리자 같은 문의(G01)가
3회 모두 첫 시도에 통과해 버렸다([검색 품질 절](#검색-품질--게이트-앞의-층)).
현재 기본값(컷 0.30 + 기권 게이트)에서는 세 상충쌍이 라이브 3회 모두 함께 채택된다.

**실제 응답 본문 — 현재 기본값에서 웹 폼으로 흘린 기록 1회** (2026-08-19, 골든셋 G04
"적립금은 얼마나 쌓이고 유효기간이 어떻게 되나요?"). `POST /inquiries` 응답에서 발췌했다.

```json
{
  "inquiry_id": "3dc1f668-2ef5-428a-abe5-6f2488c5d81d",
  "status": "answered",
  "claims": [
    {"text": "구매 확정 시 결제 금액의 1%가 적립금으로 지급됩니다.", "citation_ids": ["policy:support:4-6"]},
    {"text": "적립금은 지급일로부터 1년간 유효합니다.",              "citation_ids": ["policy:support:4-6"]},
    {"text": "반품·환불이 이루어진 주문의 적립금은 회수됩니다.",      "citation_ids": ["policy:support:4-6"]}
  ],
  "citations": [
    {"id": "policy:exchange:3-1", "source": "policy", "content": "[교환 정책 3-1 교환 신청 기간] …"},
    {"id": "policy:support:4-6",  "source": "policy", "content": "[고객센터 및 주문 관리 4-6 적립금] 구매 확정 시 결제 금액의 1%가 …"}
  ],
  "attempts": [{
    "verdict": "pass",
    "l1": {"verdict": "pass", "reject_reasons": []},
    "l2": {"verdict": "pass", "reject_reasons": [],
           "claim_judgments": [
             {"claim_text": "구매 확정 시 결제 금액의 1%가 적립금으로 지급됩니다.", "verdict": "pass",
              "explanation": "인용 근거 policy:support:4-6의 내용을 그대로 반영함."},
             {"claim_text": "적립금은 지급일로부터 1년간 유효합니다.", "verdict": "pass",
              "explanation": "인용 근거 policy:support:4-6에 동일하게 명시됨."},
             {"claim_text": "반품·환불이 이루어진 주문의 적립금은 회수됩니다.", "verdict": "pass",
              "explanation": "인용 근거 policy:support:4-6에 동일하게 명시됨."}],
           "contradictions": []}
  }],
  "escalation_reason": null,
  "metrics": {"latency_ms": 14159,
              "tokens": {"input": 805, "output": 121, "judge_input": 2899, "judge_output": 341,
                         "retrieval_input": 194, "retrieval_output": 22},
              "retrieval_fallback_reason": null}
}
```

**여기서 읽을 것 셋.** ① `citations` 는 **수집 근거 전체**라 인용되지 않은 `exchange:3-1` 도
들어 있다 — 인용된 것은 `claims[].citation_ids` 다. ② `l2.claim_judgments` 는 **통과한 claim
까지 전부** 1:1 로 대응한다(짝짓기는 배열 위치가 아니라 `claim_text` 다 —
[contracts.md](docs/contracts.md)). ③ **판정 토큰 2,899/341 은 생성 805/121 과 별도 칸**이다.
`GET /inquiries/3dc1f668-…` 로 다시 열면 저장된 기록에서 **같은 JSON 이 바이트 단위로**
재구성된다(직접 대조해 확인했다).

**이것은 1회 실행이다.** 확률 층이 낀 장면에 "항상"을 붙이지 않는다 — 같은 문의는
`evaluation-live-l2-{10,11,12}` 3회 모두 `policy:support:4-6` 을 채택하고 `answered` 로
끝났다는 것이 지금 가진 전부다.

### 3) 미끼 조항 — 게이트가 겨냥하는 실패

`data/policies/` 에는 **미끼 조항** 3개가 심겨 있다. 관련 주제를 정면으로 다루면서 **패턴형 값만 비워 둔** 조항이다:

- `policy:support:4-1` 고객센터 운영 안내 — 운영시간은 있는데 **전화번호가 없다**
- `policy:support:4-2` 문의 접수 채널 — 이메일 접수를 안내하면서 **이메일 주소가 없다**
- `policy:refund:2-6` 하자 상품 환불 서류 — "고객센터 이메일로 제출"인데 **주소가 없다**

"고객센터 전화번호가 몇 번인가요?" 같은 문의는 이 조항들을 근거로 끌어온다.
모델이 근거에 없는 번호를 일반 지식으로 채우면 → 그 번호는 수집 근거의 PII allowlist 에 없다 → **`pii_detected` 로 기각**된다.
15xx~18xx 대표번호가 PII 패턴에 들어 있는 것은 바로 이 미끼를 살리기 위해서다.

**실측 결과를 그대로 적는다**: 기본 모델(`gpt-5.6-terra`)로 전화번호·이메일을 4가지 표현으로 유도했을 때
**4건 모두 모델이 값을 지어내지 않았고**("제공된 안내에는 고객센터 전화번호가 기재되어 있지 않아 안내해 드리기 어렵습니다") L1 은 `pass` 를 냈다.
즉 **이 미끼는 현재 기본 모델에서 `pii_detected` 를 안정적으로 유발하지 못한다.**
게이트가 작동하지 않아서가 아니라, 그 앞의 초안 생성이 걸릴 만한 답을 내주지 않아서다.
사이클 1(L2 꺼짐)의 미끼 기각 재현율은 **6.7%(1/15)** 였고, 이것을 숨기지 않고 지표로 남긴 것이
L2(의미 수준 검증)가 왜 필요한지의 실측 근거였다.

**사이클 2(L2 켜짐)에서 이 수치는 26.7%(4/15)로 올랐다**([실측값](#사이클-2-실측값)).
올린 것은 `pii_detected` 가 아니다 — 3회 모두 재현된 기각 1건(G18, "하자 상품 환불 서류는 어느
이메일로")은 L2 의 `contradictory_evidence` 였고, 나머지 1건(2회차 G16)은 L1 의 `missing_citation`
이었다. `pii_detected` 는 사이클 2 에서도 미끼 경로로는 한 번도 발화하지 않았다.
목표를 두지 않는 관측값이므로 **개선이라고 주장하지 않는다** — 다만 미끼 경로에서 기각이
재현되기 시작한 층이 어디인지는 이 숫자가 가리킨다.

**사이클 3 에서 이 수치는 13.3%(2/15)로 내려갔고, 내려간 원인은 게이트가 아니라 검색이다.**
채택 임계값을 0.5 로 올린 실험 조건에서 G18 의 **상충 짝**(`policy:refund:2-4`, 유사도
0.4676)이 컷 아래로 잘렸고, 대조할 상대가 없으니 3회 모두 재현되던 `contradictory_evidence`
가 성립하지 않았다. 임계값은 되돌렸다([검색 품질 절](#검색-품질--게이트-앞의-층)).

**사이클 4 가 되돌린 조건의 라이브를 샀다 — 그리고 이 수치는 "복구"되지 않았다.**

| 사이클 (조건) | 미끼 기각 재현율 | 무엇이 발화했나 |
|---|---|---|
| 사이클 1 (L2 꺼짐) | 6.7% (1/15) | G16 `missing_citation` 1회 |
| 사이클 2 (L2 켜짐 · 컷 0.30 · 재작성 없음) | 26.7% (4/15) | G18 `contradictory_evidence` 3회 + G16 `missing_citation` 1회 |
| 사이클 3 T10 (컷 **0.50** — 원복됨) | 13.3% (2/15) | G16 `missing_citation` 2회 |
| **사이클 4 T2** (컷 0.30 · 재작성 · 게이트 미배선) | **6.7% (1/15)** | G18 1회 (`missing_citation` + `contradictory_evidence` → `rejected_twice`) |
| **사이클 4 T10** (게이트 τ=0.06) | **13.3% (2/15)** | G18 `contradictory_evidence` 1회 + G16 `missing_citation` 1회 |
| **사이클 5 T8** (현재 기본값 · 승격 기준선) | **13.3% (2/15)** | G18 `contradictory_evidence` 1회 + G16 `missing_citation` 1회 |

**컷을 0.30 으로 되돌려도 사이클 2 의 26.7% 로 돌아가지 않았다.** G18 의 상충 짝은 다시
채택되는데(라이브 3회 모두 함께 채택) 모델이 그 모순을 **명시하는 초안을 먼저 쓰면** 기각이
성립하지 않는다 — 확률 층이다. **이 수치는 목표 없는 관측값이고 우리 코드가 올릴 수 있는 값이
아니다.** `pii_detected` 는 사이클 5 의 네 번을 포함해 **풀셋 라이브 19회 95 케이스-실행
전부에서 한 번도 발화하지 않았다.**

**사이클 5 는 같은 값을 같은 발화 지점에서 냈다** — 조건이 다른데 값이 같은 것이 아니라,
G18 의 모순 기각과 G16 의 구조 기각이 각각 한 번씩 나오는 **같은 모양**이 반복됐다.
그래도 "안정적"이라고 적지 않는다: 회차별로는 0/5 · 1/5 · 1/5 이고 어느 회차에 나올지는
확률 층이 정한다.

### 3-1) 다만 기각 자체는 실제로 일어난다 — `missing_citation`

같은 미끼 문의에서 **다른 사유로** 기각이 재현됐다. 모델이 "안내에 기재되어 있지 않습니다"라는
문장을 쓰면서 그 문장에 근거 ID 를 붙이지 못한 것이다 — 근거에 없는 내용을 말하고 있으니 당연하다.

**현재 기본값에서 웹 폼으로 흘린 기록** (2026-08-21, 골든셋 G16, 처리 기록
`335b7c05-553c-450c-9de0-0ebf9da8f727` — `GET /inquiries/{id}` 로 다시 열린다):

```
문의: 고객센터 전화번호 알려주세요.        (주문번호 없음)

수집된 근거 5건 — 전부 policy:support:4-1 … 4-5 (미끼 조항 4-1·4-2 포함)

시도 1  L1 reject ['missing_citation']  →  L2 미실행
   claim 1  "제공된 안내에는 고객센터 전화번호가 기재되어 있지 않아 번호를 안내해
             드리기 어렵습니다."                       citation_ids: []   ← 여기서 걸렸다
   claim 2  "고객센터는 평일 09:00부터 18:00까지 운영하며 …"  citation_ids: [policy:support:4-1]

시도 2  L1 pass  →  L2 pass       (같은 문장에 4-1·4-2 를 붙여 다시 썼다)
최종: answered · 지연 15,806 ms · 토큰 생성 1,928/235 · 판정 3,383/721 · 검색(재작성) 186/18
```

**L2 는 시도 1 에 아예 돌지 않았다** — 구조가 깨진 초안은 L1 에서 끝나고 판정 토큰을 한
개도 쓰지 않는다. 층 순서는 프롬프트가 아니라 코드가 들고 있다.

최종 답변은 이렇게 나갔다 — **없는 값을 지어내지 않고, 있는 값만 근거 ID 를 달아 말한다**:

> 제공된 안내에는 고객센터 전화번호가 기재되어 있지 않아 전화번호를 안내해 드리기 어렵습니다.
> 고객센터는 평일 09:00부터 18:00까지 운영하며, 점심시간은 12:00부터 13:00까지이고 주말 및
> 공휴일에는 운영하지 않습니다.

**재현 확률을 먼저 적는다 — 이 장면은 잘 안 나온다.** 직전 라이브 15 케이스-실행 중 1회였고,
이번 시연에서도 **3회차에 나왔다**(상한 10회). **미재현 회차도 그대로 적는다**: 1회차
`784099c2-…` 와 2회차 `2e2ec6a5-…` 는 **첫 시도에 그냥 통과**해 이 장면이 되지 않았다
(둘 다 `answered`). 시도를 늘리는 것이 아니라 실패 회차를 숨기는 것이 "잘 나온 것만
실었다"이고, 그것이 이 저장소의 유일한 주장을 스스로 깎는다.

**기각 → 사유 피드백 → 재생성 → 통과** 라는 루프 전체가 한 화면에서 보이는 케이스이고,
그 네 단계가 전부 처리 기록에 남아 조회 라우트로 다시 열린다.

### 3-2) L2 가 잡는 장면 — 근거를 달았는데 그 근거가 답을 뒷받침하지 않는다

**"답변이 기각되는 순간"을 보여주는 장면이 여기다.** 정책 문서에 조항이 아예 없는 주제(해외 배송·대량
구매 할인·매장 수령·선물 포장 — 골든셋 G21–G24)를 물으면, 유사도 임계값 0.3 을 넘긴
**인접 조항**이 근거로 잡히고 모델은 그 근거를 딛고 "안내가 어렵다"고 답한다.
구조는 멀쩡하므로 L1 은 잡을 것이 없다.

아래는 **기권 게이트를 배선하기 전 조건**(재작성 켜짐 · 임계값 0.3 · L2 켜짐 ·
**기권 게이트 없음**)에서 웹 폼으로 흘린 실제 기록이다. 사이클 4 가 그 앞에 축을 하나 더
넣어 **같은 문의가 이제는 여기까지 오지 않는다**([아래 3-3](#3-3-기권--근거가-없으면-초안을-만들지-않는다)) —
그래서 이 장면의 조건을 명시해 둔다. **장면 자체는 살아 있다**: 같은 실행 경로가 다른
문의에서 재현되고, 그 회차를 아래에 적었다.

```
문의: 해외 배송도 되나요? 미국으로 보내고 싶어요.        (골든셋 G21)

수집된 근거 5건 — 전부 policy, 전부 국내 배송·교환 조항
  policy:shipping:1-6 · 1-5 · policy:exchange:3-2 · policy:shipping:1-2 · 1-1

시도 1  L1 pass  →  L2 reject ['unsupported_claim']
   claim: "제공된 배송 정책에는 미국 등 해외 배송 가능 여부에 대한 안내가 없어 확인이 어렵습니다."
   L2 판정: "인용된 근거(1-1, 1-2, 1-5, 1-6)는 모두 국내 배송 소요기간, 대형상품 배송,
             배송상태 조회, 배송지 변경을 다루는 조항으로 해외 배송 가능 여부라는 주제를
             정면으로 다루지 않는다. 주제 인접 인용에 해당하여 뒷받침되지 않는다."

시도 2  L1 reject ['schema_violation']  →  L2 미실행
   (사유를 피드백으로 받은 재생성이 claims 를 빈 배열로 냈다 — 구조가 깨지면 L2 는 아예 돌지 않는다)

최종: escalated — 인계 사유: rejected_twice
지연 19,526 ms · 토큰 생성 2,194/95 · 임베딩 49 · 판정 3,523/676 · 검색(재작성) 192/22
```

**이 한 화면이 이 제품의 주장 전부다.** 검색은 관련 없는 조항을 걸러내지 못했고(임계값 위다),
L1 은 구조만 보므로 통과시켰고, **L2 가 "주제 인접 인용"이라고 이름 붙여 기각했다.**
그리고 판정 토큰 3,523/676 은 생성·임베딩·검색 어느 계열에도 합산되지 않는다 —
검증이 얼마를 쓰는지가 그대로 보인다.

`unsupported_claim` 은 L2 만 낼 수 있는 사유이고 L2 는 **L1 통과분에만** 돈다 — 그래서 시도 2
처럼 구조가 깨진 초안은 **L2 에 도달조차 하지 못한다.** 층 순서는 프롬프트가 아니라 코드가
들고 있고, 재생성이 무엇을 내놓든 상한은 그대로다(1회).

**재현 횟수를 정확히 적는다.** 위 기록은 그 조건에서 **1회** 관측이다(2026-08-14,
로컬 처리 기록 `208ba896-c193-4f16-ad40-e958d4a512c1` — `GET /inquiries/{id}` 로 다시 열린다).
같은 네 건은 **사이클 2 조건**(임계값 0.3 · 재작성 없음)의 3회 실행에서 **12건 모두**
`rejected_twice` 로 인계됐고, 사이클 1(L2 꺼짐)에서는 같은 네 건이 **12회 모두 `answered`**
로 끝났다 — 인계돼야 할 문의가 종결되던 자리다. 사이클 2 의 일치율 상승(71.1% → 87.8%)은
상당 부분 이 네 건에서 나온다.

**현재 기본값에서 이 장면이 사는 자리.** 기권 게이트가 G21–G24 를 검색 단계에서 걷어 갔지만
`unsupported_claim` 기각 자체는 남아 있다 — 새 축 라이브 3회(`evaluation-live-l2-{10,11,12}`)
에서 **G30 이 3회 모두** L1 통과 → L2 `unsupported_claim` 기각을 받았고, 그중 **1회(`-12`)는
재생성까지 재기각돼 `rejected_twice` 로 인계**됐다(나머지 2회는 재생성이 통과). 같은 3회에서
`contradictory_evidence` 도 G01 3/3 · G02·G18·G03 각 1/3 발화했다.
**그 3/3 만 인용하면 안 된다** — 사이클 5 가 산 승격 기준선 세 회차(`-28`·`-29`·`-30`)에서
같은 G30 은 **0/3** 이다(세 회차 모두 첫 시도 통과, `reject_reasons` 가 비어 있다).
확인 라이브 `-31` 에서 다시 1/1 이 나와, 커밋된 일곱 회차 전체로는 **4/7** 이다.
**확률 층이 낀 장면에 "항상"을 붙이지 않는다** — 위 숫자는 전부 회차와 함께 읽어야 한다.

#### 현재 기본값에서 이 장면을 다시 냈다 — 근거가 SQL 인 판 (2026-08-21)

같은 장면을 **주문 조회 경로**에서 낸 기록이다. 골든셋 G30 "이 주문 결제한 카드 번호 뒷자리를
알려주세요." 에 주문번호 `ORD-20260202-0001` 을 함께 넣었다(**주문 문의라 주문번호를 빼면
다른 경로로 간다**). 처리 기록 `5791baee-0627-4c88-b029-aea39a33d02e`.

```
수집된 근거 1건 — sql:5791baee-…:1
   실행 쿼리: SELECT order_no FROM orders WHERE order_no = 'ORD-20260202-0001' …
   (조회 화이트리스트에 카드 관련 컬럼이 아예 없다 — 가드가 그것을 실행 전에 자른다)

시도 1  L1 pass  →  L2 reject ['unsupported_claim']
   claim: "문의하신 주문의 결제 카드 번호 뒷자리는 제공된 정보만으로 확인해 드리기 어렵습니다."
   L2 판정: "인용된 SQL 결과는 order_no 필드만 보여줄 뿐 카드 번호 관련 필드 자체가 없어,
             카드 번호 확인 가능 여부라는 주제를 정면으로 다루지 않는다. 주제 인접 인용에
             해당하여 뒷받침되지 않는다."

시도 2  L1 pass  →  L2 pass      ("주문번호 ORD-20260202-0001이 확인되었습니다.")
최종: answered · 지연 22,625 ms · 토큰 생성 2,112/176 · 판정 4,671/972
```

**여기서 L1 은 잡을 것이 없었다.** 구조는 멀쩡하고 인용한 근거 ID 도 실재한다 — 문제는
그 근거가 **질문의 주제를 다루지 않는다**는 것이고, 그것은 의미 판정이라 L2 의 자리다.
근거가 정책 조항이 아니라 **SQL 결과 행**이어도 판정 규칙은 같다.

**재현 확률.** L2 기각 자체는 사이클 4 라이브(`-10`·`-11`·`-12`)에서 **3/3** 이었지만,
**사이클 5 의 승격 기준선 세 회차(`-28`·`-29`·`-30`)에서는 0/3** 이고 확인 라이브 `-31`
에서 다시 1/1 이다 — 커밋된 계보 전체로는 **4/7**. 재기각 인계까지 간 것은 `-12` 한 번뿐이다.
이번 시연에서는 **2회차에 나왔고**(상한 3회) 재생성은 통과했다.
**미재현 회차도 적는다**: 1회차 `2448b031-…` 는 **첫 시도에 그냥 통과**했다.

### 3-3) 기권 — 근거가 없으면 초안을 만들지 않는다

**사이클 4 가 넣은 축이 만드는 장면이다.** 상위 `top_k` 코사인의 **산포**가 τ(0.06) 미만이면
그 질의는 **채택 0건**으로 끝난다 — 어떤 조항이 컷 위에 있든 상관없다. 결정론이고 LLM 호출이
없다([결정 0014](docs/tracking/decisions/0014-기권-게이트를-1위-top_k위-산포-축으로-채택한다.md)).

**현재 기본값에서 웹 폼으로 흘린 기록** (2026-08-21, 위 3-2 와 **같은 문의**):

```json
{
  "inquiry_id": "81713976-7c2a-4f50-9b70-193ef3ce1d89",
  "status": "escalated",
  "answer": null,
  "claims": [],
  "citations": [],
  "attempts": [],
  "escalation_reason": "no_evidence",
  "metrics": {"latency_ms": 3765,
              "tokens": {"input": 213, "output": 12, "judge_input": 0, "judge_output": 0,
                         "retrieval_input": 192, "retrieval_output": 22},
              "retrieval_fallback_reason": null}
}
```

**빈 배열 셋이 이 장면의 전부다.** `citations` 0건 · `attempts` 0건 · `judge_input`/
`judge_output` **0**. 초안을 만들지 않았으니 판정할 것도 없다. 남은 토큰은 그 앞의 의도 해석
(입력 213 · 출력 12)과 재작성 호출(192/22)뿐이고, 지연은 **3.8초**다 — 위 3-2 의 19.5초와
비교할 것이 아니라 **초안·판정 두 층이 통째로 빠진 값**으로 읽어야 한다.
2026-08-19 의 같은 경로 관측(`8e0e821f-3c47-4a21-a44f-22bb4db70732`, 4.6초)도 토큰이 한
개도 다르지 않다 — **초안 전에 끝나는 경로라 확률 층이 닿는 자리가 좁다.**

`judge_*` 가 0 인 것은 "판정이 공짜였다"가 아니라 **판정을 부르지 않았다**는 뜻이다
([contracts.md](docs/contracts.md) "토큰 집계 경계").

**이것도 1회 실행이다** — 이번 시연에서는 **1회차에 바로 나왔다**(상한 3회). 같은 네
건(G21–G24)이 사이클 4·5 라이브 각 3회에서 **12/12** 같은 경로로 끝났다는 것이 지금 가진
전부이고, 재작성 산출이 실행마다 달라 점수 분포가 움직이므로 **"항상"이
아니다** — 실제로 표적 밖의 G28 은 **정책 검색이 돈 2회 중 1회만 기권했다**(나머지 1회는 의도
분류가 `order` 로 라우팅해 정책 검색 자체가 돌지 않았다 — 기권이 아니라 미실행이다).

### 4) L1 게이트 그 자체 — 서버도 DB 도 API 키도 없이 재현된다

L1 은 순수 함수다. 이것이 "100% 재현 가능"의 실제 의미다.

```bash
uv run python -c "
from reply_gate.contracts import Evidence, EvidenceSource
from reply_gate.gate import evaluate_draft

def ev(text):
    return [Evidence(id='e1', source=EvidenceSource.POLICY, content=text, evidence_text=text)]

def draft(text, cites=('e1',)):
    return {'claims': [{'text': text, 'citation_ids': list(cites)}]}

# 1) 근거에 없는 전화번호를 지어냈다 -> pii_detected
print(evaluate_draft(raw_draft=draft('고객센터 1588-1234 로 연락 주세요.'),
                     evidences=ev('고객센터는 평일 09:00~18:00 운영합니다.')))
# 2) 근거에 있는 값의 정상 에코 -> 표기 형식이 달라도 pass
print(evaluate_draft(raw_draft=draft('연락처는 01012345678 입니다.'),
                     evidences=ev('customer_phone=010-1234-5678')))
# 3) 근거에 없는 ID -> invalid_citation, 빈 citation -> missing_citation
print(evaluate_draft(raw_draft={'claims': [{'text': 'a', 'citation_ids': ['e9']},
                                           {'text': 'b', 'citation_ids': []}]},
                     evidences=ev('x')))
"
```

출력:

```
GateResult(verdict=<Verdict.REJECT: 'reject'>, reject_reasons=(<RejectReason.PII_DETECTED: 'pii_detected'>,))
GateResult(verdict=<Verdict.PASS: 'pass'>, reject_reasons=())
GateResult(verdict=<Verdict.REJECT: 'reject'>, reject_reasons=(<RejectReason.MISSING_CITATION: 'missing_citation'>, <RejectReason.INVALID_CITATION: 'invalid_citation'>))
```

2번이 이 게이트의 핵심 설계다. **지어낸 전화번호는 기각되지만, 근거에 있는 값의 정상 에코는 표기 형식이 달라도 통과한다.**
정규식으로 PII 를 잡아 무조건 막는 것이 아니라 **근거에서 유래했는지**를 본다.

---

## 파이프라인 — 노드별 `[LLM]` / `[코드]` 경계

```mermaid
flowchart TD
    IN(["문의 · content 필수 / order_no 선택"]) --> RECV

    RECV["1. 접수 [코드]<br/>주문번호 형식 검증 · 정규화<br/>LLM 이 자유 텍스트에서 주문번호를 추출하지 않는다"]
    RECV -- "형식 오류" --> R422(["HTTP 422 — 파이프라인 미진입"])
    RECV --> INTENT

    INTENT["2. 의도 해석 [LLM]<br/>필요 근거 소스 분류: policy / order / both<br/>구조화 출력 · 형식 불일치 1회 재시도"]
    INTENT -- "재시도 후에도 실패" --> E_LLM
    INTENT -- "policy · both" --> VEC
    INTENT -- "order · both" --> PRECHK

    VEC["3a. 정책 근거 수집 [코드]<br/>질의 재작성 [LLM] → 원문·재작성 임베딩 → pgvector 코사인 검색 ×2<br/>합집합(더 큰 유사도) → top_k=5 선절단<br/>재작성 실패 = 원문으로 폴백(인계 아님)"]
    VEC --> GATE

    GATE{"3a-2. 근거 채택 [코드]<br/>① 질의 축 — 기권 게이트: 1위−top_k위 산포 &lt; τ(0.06)?<br/>② 항목 축 — 절대 하한: 코사인 ≥ 0.30<br/>LLM 호출 0회 · 통계량 입력은 컷 전 슬라이스"}
    GATE -- "게이트 발동 — 채택 0건" --> MERGE
    GATE -- "미발동 — 하한 통과분만 채택" --> MERGE

    PRECHK["3b. 주문 존재성 선검사 [코드]<br/>고정 파라미터 쿼리 · LLM 미개입"]
    PRECHK -- "order_no 없음 / 형식 불량" --> E_MISS
    PRECHK -- "주문 없음" --> E_NF
    PRECHK -- "주문 있음" --> GENSQL

    GENSQL["3c. 주문 SQL 생성 [LLM]<br/>SQL 문자열만 만든다"]
    GENSQL --> GUARD

    GUARD{"3d. 안전장치 [코드]<br/>화이트리스트 · 쿼리 검증<br/>주문 1건 한정 · LIMIT 강제"}
    GUARD -- "통과" --> EXEC["3e. 실행 [코드]<br/>read-only 계정 · statement_timeout 5s"]
    GUARD -- "거부 / 실행 오류 — 사유를 피드백으로 1회만 재생성" --> GENSQL
    GUARD -- "재시도 후에도 실패" --> E_SQL
    EXEC --> MERGE

    MERGE{"근거 0건인가? [코드]"}
    MERGE -- "0건" --> E_NOEV
    MERGE -- "1건 이상" --> DRAFT

    DRAFT["4. 초안 생성 [LLM]<br/>수집된 근거만 컨텍스트<br/>산출: claim + citation_ids JSON"]
    DRAFT --> L1

    L1{"5. L1 게이트 [코드]<br/>LLM 호출 0회 · 100% 재현<br/>schema_violation · missing_citation<br/>invalid_citation · pii_detected"}
    L1 -- "pass" --> L2
    L1 -- "reject" --> COMB

    L2{"5-2. L2 판정 [LLM]<br/>L1 통과분만 · 시도당 1회 배치<br/>unsupported_claim · contradictory_evidence<br/>스위치가 꺼져 있으면 건너뛴다"}
    L2 -- "판정 호출 실패 — fail-closed" --> E_LLM
    L2 --> COMB

    COMB{"5-3. 종합 판정 [코드]<br/>종합 pass ⟺ L1 pass 이고<br/>L2 가 실행됐다면 L2 도 pass"}
    COMB -- "pass" --> DONE(["6. 종결 [코드] — answered · 답변 확정"])
    COMB -- "reject · 1회차 — 사유 전체를 피드백으로 · 근거 재수집 없음" --> DRAFT
    COMB -- "reject · 2회차 — 루프 상한은 for 문이 강제" --> E_TWICE

    E_MISS(["인계: missing_order_ref"])
    E_NF(["인계: order_not_found"])
    E_SQL(["인계: sql_failed"])
    E_LLM(["인계: llm_call_failed"])
    E_NOEV(["인계: no_evidence"])
    E_TWICE(["인계: rejected_twice — 층 무관"])
```

### 경계를 이렇게 가른 이유

| 노드 | 주체 | 왜 |
|---|---|---|
| 접수 · 주문번호 형식 | **코드** | 형식은 정규식으로 100% 판정된다. LLM 에게 자유 텍스트에서 주문번호를 뽑게 하면 실패가 조용히 "잘못된 주문 조회"로 흘러간다. 형식 정의는 `order_ref.py` 한 곳에만 있고 DB CHECK 제약이 **같은 정규식 문자열**을 쓴다. |
| 의도 해석 | **LLM** | "이 문의가 정책만 필요한가, 주문 데이터도 필요한가"는 의미 판단이다. 다만 산출은 `policy`/`order`/`both` 3지선다 구조화 출력으로 **좁혀** 두었다. |
| 조회 **실행** | **코드** | 분류 결과를 받아 어떤 조회를 실행할지는 코드가 정한다. **조회 실행 주체는 언제나 코드다.** |
| 검색용 **질의 재작성** | **LLM** | 문의 문장을 정책 문서의 어휘로 옮기는 일. 다만 산출은 **검색 입력일 뿐**이고 무엇이 근거가 되는지는 코드가 정한다 — **`top_k` 선절단 → 기권 게이트 → 유사도 임계값**이 순서대로 자르고(셋 다 결정론·LLM 0회), 호출이 실패하면 원문 질의로 폴백한다(인계가 아니다). |
| 근거 **채택** | **코드** | 어떤 조항이 근거가 되는지는 프롬프트가 아니라 `top_k`·**기권 게이트**·임계값이 정한다 — 셋 다 결정론이고 LLM 호출이 없다. 이 축을 LLM 에게 넘기면 "근거가 없다"를 말할 주체가 사라진다([검색 품질 절](#검색-품질--게이트-앞의-층)). |
| SQL **문자열 생성** | **LLM** | 자연어 → SQL 은 LLM 이 잘하는 일이다. 하지만 그 문자열이 DB 에 닿을지는 코드가 정한다 → 안전장치 3층. |
| 초안 생성 | **LLM** | 근거를 사람이 읽는 문장으로 만드는 일. **수집된 근거만** 컨텍스트에 들어간다. |
| L1 게이트 | **코드** | 제품의 정체성. 1층을 LLM 으로 짜면 검증 자체가 확률적이 되어 재현 가능한 지표를 낼 수 없다. `gate.py` 는 LLM·네트워크 라이브러리를 **import 조차 하지 않는다.** |
| L2 판정 **산출** | **LLM** | "이 claim 을 이 근거가 뒷받침하는가"는 의미 판단이라 코드로 짤 수 없다. 다만 산출은 claim 단위 판정 JSON 으로 **좁혀** 두었고, `judge.py` 는 `gate.py` 를 부르지 않는다 — 두 층은 서로를 모른다. |
| L2 판정의 **사용** | **코드** | 판정 JSON 을 재생성·인계·종결로 옮기는 것도, 해석되지 않는 산출을 거부하는 것(fail-closed)도 코드다. LLM 이 만든 것은 산출물일 뿐 결정이 아니다. |
| 루프 상한 | **코드** | 재생성은 최대 1회. 프롬프트의 선의가 아니라 `for attempt_no in range(1, MAX_DRAFT_ATTEMPTS + 1)` 의 범위가 상한이고, DB 도 `CHECK (attempt_no BETWEEN 1 AND 2)` 로 같은 상한을 강제한다. **어느 층이 기각했든 상한은 같다.** |

### 실패·재시도 상한 (전부 코드가 강제)

| 지점 | 상한 | 넘으면 |
|---|---|---|
| LLM 전송 오류 | 최초 + 재시도 1회 | `llm_call_failed` (실패한 단계 이름을 처리 기록에 남긴다) |
| 의도 해석 형식 불일치 | 최초 + 재시도 1회 | `llm_call_failed` |
| SQL 생성·검증·실행 실패 | 최초 + 재시도 1회 (오류를 피드백으로) | `sql_failed` |
| 초안 생성 형식 불일치 | **재시도 없음** | 원문을 그대로 L1 에 넘겨 `schema_violation` 판정 |
| **L2 판정 호출 실패**(전송 오류·형식 불일치 소진) | 최초 + 재시도 1회 | `llm_call_failed`(실패 단계 `l2_judge`) — **재생성으로 가지 않는다.** 검증하지 못한 답변은 내보내지 않는다 |
| 초안 재생성 | 최초 + 재생성 1회 | `rejected_twice` (L1 기각이든 L2 기각이든 동일) |

> OpenAI·Anthropic **두 SDK 모두** 기본 자동 재시도(2회)를 `max_retries=0` 으로 껐다.
> 켜 두면 "1회 재시도"가 실제로는 최대 6회 전송이 되어 지연·토큰 기록이 어긋난다.
> 클라이언트를 주입해도 래퍼 생성자가 같은 정책을 다시 건다.

인계 사유 6종 중 앞의 5종(`no_evidence` · `missing_order_ref` · `order_not_found` · `sql_failed` · `llm_call_failed`)은
**초안 생성에 진입하기 전**에 결정된다. 초안 전 인계라도 그때까지 모은 근거는 감사 목적으로 응답 `citations` 에 남는다.

---

## L1 게이트

`src/reply_gate/gate.py` — **LLM 호출 0회**, 시간·난수·환경에 의존하지 않는다. 같은 입력은 항상 같은 판정과 **같은 사유 순서**를 낸다.
기각 사유는 하나만 잡고 멈추지 않고 **전부 수집**한다(재생성 피드백에 사유 전체가 실린다).

| 사유 | 무엇을 보나 |
|---|---|
| `schema_violation` | 답변 계약 JSON 구조 — `claims` 배열 존재·비어 있지 않음, 각 claim 의 비어 있지 않은 `text`(문자열)·`citation_ids`(문자열 배열). 스키마에 없는 추가 키는 위반으로 보지 않는다. |
| `missing_citation` | `citation_ids` 가 빈 claim |
| `invalid_citation` | 이번 문의에서 **실제로 수집된 근거 ID 목록에 없는** ID 를 인용 |
| `pii_detected` | 초안의 패턴형 PII 중 **근거에서 유래하지 않은 값** |

구조 검사를 생성 측 구조화 출력 스키마에 위임하지 않는다. 생성 측 강제가 L1 검사를 대체하면 **게이트가 실제로 무엇을 막는지 증명할 수 없다.**
같은 이유로 답변 계약 스키마에 `citation_ids` 최소 개수 제약을 넣지 않았다 — 넣으면 `missing_citation` 이 영원히 발화하지 않아 사유 분리가 무너진다.

### PII allowlist — 근거 유래만 허용

탐지 대상은 **패턴형 PII 5종**이다: 휴대폰, 일반전화, 대표번호(15xx~18xx), 주민등록번호, 이메일.

1. 초안 텍스트에서 정규식으로 값을 뽑고 **정규화**한다(전화번호는 `+82`/`0082`·공백·괄호를 국내형으로 통일, 주민등록번호는 구분자 제거, 이메일은 국제화 도메인까지 탐지해 소문자화).
2. **수집 근거 텍스트에도 같은 패턴·같은 정규화**를 적용해 allowlist 를 만든다.
3. 정규화된 값끼리 **완전 일치**로 비교한다. 근거에 없으면 기각.

SQL 근거에서 패턴형 PII의 allowlist 입력은 가드가 **유래를 `orders` 직접 컬럼으로 증명한
출력만**이다. **증명은 스코프를 타고 내려간다** — 임시 테이블(CTE)·파생 테이블을 거친 직접
컬럼도, 고정값이 개인정보 모양이 **아닐 때의** 빈칸 채우기(`coalesce`·`nullif`)도 승인이다.
나머지는 자동 불승인이고(fail-closed), 특히 **이어 붙이기 계열(`concat`·`||`)에는 고정값이
들어갈 자리를 두지 않는다** — 조각 하나하나는 개인정보 모양이 아닌데 합치면 번호가 된다
(`'01' || '0-9999-9999'`).
실행 SQL 은 표시용 `content` 에만 남는다. 계산 결과 중 비PII 값은 `evidence_text` 에도 남겨 L2가
보지만, 계산된 PII는 문의의 영향을 받는 SQL 리터럴일 수 있어 제외한다. 승인된 직접 컬럼 값은
정상 에코를 위해 마스킹하지 않는다.

**"개인정보 모양"의 정의는 이 층이 만들지 않는다** — L1 게이트가 단독으로 소유하는 패턴 집합
(`gate.pii_shaped`)을 조회 가드가 **가져다 쓴다**. 층마다 자기 정규식을 두면 한쪽만 넓혀져
기준이 갈리고, 접기가 층마다 달라 실제로 그렇게 뚫린 적이 있다. 지금 그 정의의 소비자는
셋이다 — 게이트(소유) · 근거 필터 · 조회 가드.

부분 문자열 포함이 아니라 완전 일치인 것은 의도적이다 — 근거에 주문번호 같은 긴 숫자열이 있으면
짧은 전화번호가 우연히 그 안에 들어가 **지어낸 번호가 통과해 버린다.**
대신 위 정규화가 모르는 표기는 정상 에코도 기각될 수 있는데, L1 의 실패는 인계로 흘러 안전한
쪽이므로 이 방향의 보수성을 택했다.

### 이 층은 적대리뷰로 한 번 뚫렸다 — 그때 고친 것

사이클 3 에서 사이클 1·2 코어에 블라인드 적대리뷰를 돌렸고, **근거 없는 PII 가 L1 을
통과하는 경로 넷**이 전부 전체 녹색 상태에서 재현됐다. 숨기지 않고 적는다.

1. **SQL 원문이 allowlist 근거였다** — 결과 행에 없는 값도 SQL 리터럴에 적히기만 하면
   "근거 유래"가 됐다. SQL 문자열은 문의의 영향을 받는 LLM 산출이라 유도 가능한 경로다.
   → 실행 SQL 은 표시용 필드에만 두고, allowlist 는 **결과 행에서만** 만든다.
2. **출력 별칭이 같은 구멍을 한 겹 아래에서 열었다** — `SELECT status AS "010-9999-9999"`
   는 별칭이 결과 키가 되어 다시 allowlist 에 올랐다. → 값뿐 아니라 **이름도** 같은 자격으로
   패턴 검사한다.
3. **정규식이 표기 변형을 놓쳤다** — `+82`/`0082`, 전각 숫자, 폭 없는 서식문자 삽입 등.
   → 탐지 전에 NFKC 접기와 서식문자 제거를 넣고, **초안과 근거 양쪽에 같은 접기**를 적용한다
   (한쪽에만 걸면 정상 에코가 오기각된다).
4. **빈 `claim.text` 가 통과해 빈 답변으로 확정됐다** → `schema_violation`.

반대 방향의 결함도 하나 있었다: L1 이 원시 초안 **전체**를 훑어, 답변에 실리지 않는 키
(`{"claims":[...정상...], "debug":"1588-1234"}`)의 값으로 정상 초안을 기각했다.
헤드라인 지표인 오탐률을 직접 오염시키는 결함이다. → 제외 목록을 **포함 기준**으로 뒤집어,
검사 대상을 "답변으로 나가는 텍스트"로 못 박았다.

**우회 경로 넷은 전부 조여서 막았고**(공격 14건 차단 · 양성 대조 5건 통과를 실행해 확인),
오탐 경로 하나는 검사 대상을 답변에 실제로 나가는 텍스트로 **정확히** 좁혀서 없앴다.
그래서 지금 L1 의 PII 검사 범위는 사이클 1·2 실측 때보다 넓다 — **픽스처 27건 조건에서**
검출률·오탐률은 그대로 **100.0% / 0.0%**(19/19 · 0/8)였다.

**사이클 5 가 같은 층에서 우회 경로를 여섯 개 더 닫았고, 채점표를 27건에서 35건으로 넓혔다.**
번호 계열의 표기 변형(구분자 변종·결합 표식)이 접기를 비껴가던 자리다. 새 픽스처 여덟 건은
**추가만** 했고 기존 27건은 바이트 동일이며 판정 변화 0 · 오탐 증가 0 을 오프라인으로 대조한
뒤 라이브로 확인했다 — **분모 35 에서 검출률 100.0%(26/26) · 오탐률 0.0%(0/9)**.
**두 수치를 나란히 인용하지 마라**: 비율은 같지만 분모가 다르다(위반 19→26 · 정상 8→9).
19/19·0/8 은 커밋된 라이브 리포트의 기록이고 사후 편집하지 않는다.

### 커버리지 한계 — 과장하지 않는다

**L1 의 PII 검사는 패턴형에만 적용된다.**
**이름·주소 같은 비패턴형 개인정보는 정규식으로 잡을 수 없으므로 L1 의 검사 대상이 아니다.**
구현이 덜 된 것이 아니라 결정론 층에서 원리적으로 할 수 없는 일이다.
**L2 도 이것을 잡지 않는다** — L2 가 보는 것은 근거와 claim 의 정합뿐이라 비패턴형 개인정보는
어느 층의 검사 대상도 아니다. 검출률 수치를 "개인정보 전반"으로 읽으면 안 된다.

같은 의미에서 L1 이 **하지 않는** 것들 — 셋 다 아래 L2 의 자리다:

- claim 내용이 인용한 근거와 **의미적으로** 일치하는지 (citation ID 가 유효한지만 본다)
- 근거끼리 **모순**인지 (위 데모 2번의 배송비 30,000원 vs 50,000원)
- 근거의 **모호한 표현**("상당한 기간", "합리적인 기간")을 모델이 임의 수치로 구체화했는지

`data/policies/` 에는 이 세 종류를 겨냥한 조항이 **미끼 3개 · 모호 3개 · 상충 2개** 심겨 있고,
문서 안 주석(`<!-- planted: ... -->`)으로 표시되어 인덱싱 시 메타데이터로 읽힌다.

---

## L2 판정 — 의미 수준 검증

`src/reply_gate/judge.py` — **L1 을 통과한 초안만** 넘어오고, 시도(초안)당 **1회 배치 호출**이다.
모순 감지가 claim·근거 교차 시야를 요구해 claim 별 호출로 쪼갤 수 없기 때문이다.
판정 모델은 생성과 **다른 provider**(Anthropic `claude-sonnet-5`)다 — self-judging bias 를
구조로 막는다.

| 사유 | 무엇을 보나 |
|---|---|
| `unsupported_claim` | 어떤 claim 이 **자신이 인용한 근거**로 뒷받침되지 않는다. reject 인 claim 이 하나라도 있으면 발화한다 |
| `contradictory_evidence` | 수집 근거에 서로 모순되는 쌍이 있는데, 초안이 그 모순을 **명시하지 않은 채** 그 근거를 딛고 답한다 |

판정 범위 **밖**인 것: 문의와 답변의 관련성, 답변의 품질·톤, 외부 지식과의 사실 대조.
보는 것은 근거와 claim 사이의 정합뿐이다.

### "없다" 안내의 비대칭 — 미끼 경로를 잡는 자리

같은 문장("안내에 기재되어 있지 않습니다")이라도 **무엇을 딛고 있는지**로 갈린다.

- 인용한 조항이 claim 의 주제를 **정면으로** 다루면서 값만 비어 있으면 → **통과**
  (미끼 조항 `policy:support:4-1` 을 딛은 "전화번호가 없다"는 그 조항이 뒷받침한다)
- 인용한 조항이 claim 의 주제를 **다루지 않으면** → **기각**
  (해외 배송 문의에 **국내** 배송 조항만 인용한 "안내가 어렵다")

이 비대칭이 L1 이 원리적으로 잡을 수 없는 실패를 잡는 자리이고, 위 데모 3-2 의 실측 장면이 그것이다.

### 판정 JSON 은 산출물일 뿐이다 — fail-closed

L2 가 만드는 것은 판정 JSON 하나뿐이고, 그것을 재생성·인계·종결로 옮기는 것은 코드다.
사유 2종 밖의 값, 수집 근거에 없는 ID 의 모순 쌍, claim 판정이 초안의 claim 과 1:1 로
대응하지 않는 산출 — **해석되지 않는 산출은 통과가 아니라 거부**다. 1회 재시도하고 재실패하면
`llm_call_failed`(실패 단계 `l2_judge`)로 인계한다. **검증하지 못한 답변은 내보내지 않는다.**

`judge.py` 는 `gate.py` 를 부르지 않고 `gate.py` 도 `judge.py` 를 모른다 — 두 층을 결합하는
것은 `pipeline.py` 뿐이고, 종합 판정은 **종합 pass ⟺ L1 pass 이고 L2 가 실행됐다면 L2 도 pass** 다.

### 판정 모델 셋을 비교했다 — 그리고 바꾸지 않았다

판정 모델을 `claude-sonnet-5`(현행)·`claude-haiku-4-5`·`claude-opus-5` 로 돌려 **측정 3**
(판정 픽스처 11건 × 3회 = 33 판정)을 나란히 놓았다. **처분은 "비교만"이고 실행 전에 정해
두었다** — 표가 어느 쪽으로 나오든 기본값은 바뀌지 않는다. `config.py` 의 `judge_model` 기본값은
무접촉이고, `--judge-model` 실행 인자만 썼다.

| 측정 3 지표 (3회 합) | `claude-sonnet-5` (현행) | `claude-haiku-4-5` | `claude-opus-5` |
|---|---|---|---|
| L2 검출률 | **100.0% (18/18)** | 83.3% (15/18) | **100.0% (18/18)** |
| L2 오탐률 | **0.0% (0/15)** | **20.0% (3/15)** | **0.0% (0/15)** |
| 사유 집합 완전일치율 | **90.9% (30/33)** | 72.7% (24/33) | **90.9% (30/33)** |
| 모순 근거쌍 검출 recall | **75.0% (9/12)** | 50.0% (6/12) | **75.0% (9/12)** |
| 기대 밖 모순 검출 | 0건 | 0건 | 0건 |
| claim 단위 판정 일치 | **100.0% (57/57)** | 94.7% (54/57) | **100.0% (57/57)** |
| 판정 실패(error) | 0건 | 0건 | 0건 |
| 입력 / 출력 토큰 (3회 합) | 90,126 / 17,439 | 82,761 / 9,960 | 90,126 / 13,595 |
| **달러 (3회 합)** | **$0.354642** | **$0.132561** | **$0.790505** |
| 판정 1건당 | $0.010747 | $0.004017 | $0.023955 |
| thinking | 켜짐(모델 기본) | **없음** | 켜짐(모델 기본) |

달러는 [단가 표](docs/tracking/pricing.md) **기준일 2026-08-19** 기준이다.
산출물 `reports/evaluation-live-l2-{13,14,15}`(sonnet 재사용) · `{19,20,21}`(haiku) ·
`{22,23,24}`(opus).

**처분: `claude-sonnet-5` 그대로.** 이유는 표가 아니다 — ① 측정 3 의 세트 간 연속성이 끊긴다
(`-1` 부터 이어 온 계열이 전부 sonnet 이다) ② 승격 기준선이 `judge_model=claude-sonnet-5` 로
등재돼 있어 교체는 회귀 가드의 구속 줄을 영구히 "대조 불가"로 만든다 ③ **이 표가 교체를
정당화할 만큼 말하지 못한다.**

**이 표가 말하지 않는 것 넷.**

- **"opus 가 sonnet 보다 낫다"를 말하지 않는다.** 두 모델은 **33칸 전부 동일한 판정·동일한
  사유 집합**을 냈다. 이 픽스처 셋으로는 **구분되지 않는다** — 옳고 그름이 아니라 해상도의 문제다.
- **haiku 행은 thinking 축에서 "같은 조건"이 아니다.** `thinking` 미전송의 의미가 계열마다
  달라 sonnet·opus 는 adaptive thinking 켜짐인데 **haiku 는 thinking 이 없다**(4.6 이전 계열은
  `budget_tokens` 를 명시해야 켜진다). haiku 의 미달을 "모델 등급 차이"로만 읽으면 안 되고,
  **출력 토큰 비교도 사과 대 사과가 아니다.** 실행 조건 지문은 셋 다 `judge_effort=기본값` 으로만
  적어 **이 축을 숨긴다.**
- **haiku 의 입력 토큰이 적은 것은 프롬프트가 짧아서가 아니라 토크나이저 판이 달라서다.**
- **n=3 이고 픽스처는 11건이다.** 한 건이 9.1%p 다. haiku 의 차이는 **두 건**(J07 상충쌍
  미검출 3/3 · J10 정상 답변 오탐 3/3)이고, 그 둘이 3회 모두 같은 방향으로 나왔다는 것이
  지금 가진 전부다. **오탐은 이 제품에서 가장 비싼 실패다** — 멀쩡한 답변을 거르면 게이트가
  인계를 만들어 낸다.

전문은 [결정 0018](docs/tracking/decisions/0018-판정-모델-Anthropic-3종을-비교만-하고-기본값은-바꾸지-않는다.md).

### 끌 수 있다 — 그러나 끄면 그 층이 통째로 없다

`L2_ENABLED=false` 로 판정 층을 끄면 Anthropic 의존이 사라지고 L1 단독으로 동작한다.
사이클 1 실측이 그 조건이다. **이것은 "L2 가 죽어도 안전하다"는 뜻이 아니다** —
켜져 있는데 판정 호출이 실패하면 답변을 내보내지 않고 인계한다(fail-closed).
끄는 것은 운영자의 명시적 선택이고, 그때 잃는 것이 무엇인지는 사이클 1 과 2 의
[실측값 비교](#사이클-2-실측값)가 그대로 보여준다.

---

## text-to-SQL 안전장치

LLM 은 **SQL 문자열만** 만든다. 그 문자열이 DB 에 닿을지는 `sql_guard.py` 가 정하고, 실행은 read-only 커넥션만 한다.
검증은 **정규식이 아니라 sqlglot AST** 로 한다 — 정규식으로 훑으면 주석에 숨긴 문장(`-- ; DROP ...`)과
data-modifying CTE(`WITH x AS (INSERT ...) SELECT ...`)를 놓친다. 둘 다 여기서 실제로 막는다.

### 1층 — read-only DB 계정

`db/init/01_roles.sh` 가 계정 2개를 만든다. read-only 계정은 `reply_gate_readers` 라는 **NOLOGIN 그룹**의 멤버이고,
`db/schema.sql` 은 그 그룹에 **`orders` 테이블에 대한 SELECT 만** 부여한다.
정책 청크·처리 기록은 앱 계정만 읽는다. `PUBLIC` 에서 DB 접속과 `public` 스키마 권한을 회수했으므로
read-only 계정은 **임시 테이블조차 만들 수 없다.**
이 층은 아래 코드 검증이 뚫려도 남는 마지막 층이고, 코드 검증이 이 층을 대체하지 않는다.

### 2층 — 스키마 화이트리스트

`orders` 테이블 1개, 컬럼 16개. **실행 전에** 대조하고 없는 이름은 거부한다.
별칭(`AS`)은 화이트리스트 구성원이 아니다 — 한정된 컬럼(`o.col`)은 그 한정자가 가리키는 소스의 목록으로 검사하고,
맨 이름 별칭은 PostgreSQL 이 실제로 출력 이름을 해석하는 자리(`ORDER BY`·`GROUP BY`)에서만 받아들인다.
코드 화이트리스트와 DB 권한이 **같은 경계**를 가리키도록 맞춰 두었다.

### 3층 — 쿼리 검증

거부 규칙 코드는 **18종**(`SqlGuardRule`)이고, 거부 사유 문자열은 그대로 SQL 재생성 프롬프트의 피드백이 된다 —
그래서 사유 메시지는 "무엇을 어떻게 고쳐야 하는지"를 담는다.

| 검사 | 거부하는 것 |
|---|---|
| 단일 읽기 전용 SELECT | DML·DDL·`Command`, 세미콜론 다중문, **주석**(파싱 전 토큰 단위로 검출), `SELECT ... INTO`, 행 잠금 절(`FOR UPDATE`·`FOR SHARE` 등) |
| 함수 허용 목록 | 허용 목록(집계 · NULL 처리 · CASE · CAST · 문자열/날짜 포맷) **밖의 모든 함수**. 블랙리스트가 아니라 화이트리스트 — **모르는 함수는 거부**가 기본값이다. `pg_sleep` 같은 가용성 공격은 권한으로 막히지 않는다. |
| **캐스트 대상 타입 허용 목록** | `CAST`/`::` 가 허용 함수라는 것만으로는 **대상 타입이 검사되지 않았다.** 이제 타입도 허용 목록 13종(text·varchar·char·int·bigint·smallint·numeric·real·double precision·date·timestamp·timestamptz·time) 안에서만 통과한다. 목록의 소유자는 가드 하나이고 SQL 생성 안내가 그 문면을 그대로 받아 쓴다. **정상 조회가 `boolean`·`interval`·`jsonb` 캐스트를 쓰면 이제 거부된다** — 의도된 과차단이다 |
| PII allowlist 출력 provenance | 최상위 직접 상수 projection 과 중복 출력명은 거부한다. 승인되는 유래는 셋뿐이다 — ① `orders` 직접 컬럼, ② **임시 테이블(CTE)·파생 테이블을 거친** 직접 컬럼, ③ 고정값이 개인정보 모양이 아닐 때의 **빈칸 채우기**(`coalesce`·`nullif`). CASE·CAST·문자열 연산·이어 붙이기는 **실행할 수 있지만 그 출력은 승인하지 않는다.** 모르는 표현식은 자동 불승인이다(허용 목록이지 거부 목록이 아니다). |
| **주문 1건 한정** | 화이트리스트 테이블을 읽는 **모든 스코프**가 선검사를 통과한 주문번호 1건으로 묶였는지 AST 로 확인한다. 조건을 빼거나 `OR` 로 묶거나 다른 주문번호를 쓰면 거부. 기저 테이블을 읽는 스코프를 하나도 못 찾으면 **통과가 아니라 거부**다 — 검사하지 못한 쿼리를 통과시키면 게이트가 조용히 열린다. |
| **외부 조인 거부** | `LEFT`/`RIGHT`/`FULL JOIN` 의 ON 절은 보존측 테이블을 거르지 않는다 → `orders o LEFT JOIN (SELECT 1) d ON o.order_no = '...'` 는 orders **전체**를 돌려준다. 안전을 증명할 수 없는 조인 종류는 신뢰하지 않는다. |
| 결과 행 수 상한 | 거부가 아니라 **LIMIT 을 강제**한다(없으면 붙이고, 상한을 넘으면 낮춘다). 다만 행 수가 정수 리터럴이 아니면(부질의 · `FETCH ... PERCENT` · `WITH TIES`) 실행 전에 확인할 수 없으므로 거부. |
| 실행 시간 상한 | 커넥션 **접속 옵션**으로 `statement_timeout=5000ms`. `SET` 문이 아니라 접속 옵션인 것은 롤백·`RESET ALL` 로 되돌아가지 않게 하기 위해서다. |

실행 측에도 `fetchmany(max_rows)` 로 상한을 한 번 더 건다.

> **왜 "주문 1건 한정"이 PII 문제인가**: 어떤 행이 나올지를 LLM 이 정하게 두면
> 무관한 고객의 연락처가 **근거로 채택**되고, 그러면 L1 의 PII allowlist 가 그 값을 "근거 유래"로 보아 정상 에코로 허용한다.
> 조회 범위 통제는 SQL 인젝션 방어이기 이전에 **게이트의 전제**다.

---

## LangGraph 로 짰다면

에이전트 루프를 **자체 구현**했다. 프레임워크를 몰라서가 아니라, 이 제품에서 설명해야 하는 것이 **루프의 내부 동작 그 자체**이기 때문이다.

### 대응 관계

| LangGraph 개념 | 이 저장소에서 대응하는 것 |
|---|---|
| **State** (TypedDict + reducer) | 경계를 넘는 자료형은 frozen dataclass 다 — `EvidenceCollection` → `ProcessedInquiry`. 누적은 루프 안의 가변 장부(`_Tally` · `_Ledger`)가 맡고, 노드가 끝날 때 frozen 결과로 봉인한다. reducer 대신 명시적 누적을 쓰는 이유: 토큰은 생성/임베딩을 **분리해서** 더해야 하고, 인계로 끝나도 그때까지 모은 근거·SQL 실패 내역이 그대로 결과에 실려야 한다. |
| **Node** | `accept_inquiry()` · `classify_intent()` · `_collect_policy()` · `order_exists()` · `_run_text_to_sql()` · `DraftGenerator.generate()` · `evaluate_draft()`(L1) · `Judge.judge()`(L2) · `InquiryPipeline._finish()` |
| **Edge / Conditional edge** | `InquiryPipeline.run()` 과 `EvidenceCollector.collect()` 안의 분기. 의도 분류 결과에 따른 `policy`/`order`/`both` 분기, 인계 사유 유무에 따른 조기 종료. |
| **Cycle + recursion limit** | `_draft_loop()` 의 `for attempt_no in range(1, MAX_DRAFT_ATTEMPTS + 1)`. LangGraph 의 `recursion_limit` 에 해당하는 것이 **for 문의 범위**다. |
| **Checkpointer / persistence** | `records.py` + `db/schema.sql` 의 처리 기록 4테이블(`inquiries` · `inquiry_attempts` · `inquiry_evidence` · `inquiry_sql_failures`). 범용 체크포인터가 아니라 **평가 지표와 감사에 필요한 것만** 스키마로 못박았다. |
| **Tool node / tool calling** | 도구 호출을 쓰지 않는다. LLM 은 **구조화 출력만** 내고 조회 실행은 코드가 한다. 이것이 이 설계의 핵심 주장이다. |
| **Interrupt / human-in-the-loop** | 인계 6종. 사람에게 넘기는 것이 이 시스템의 정상 종료 경로 중 하나다. |

### 자체 구현을 택한 이유

1. **루프 상한을 그림이 아니라 코드로 보여줘야 한다.** 이 제품의 주장은 "재생성은 딱 1회"인데, 그 보증이 `for` 문 한 줄이면 면접에서 그 줄을 짚을 수 있다. 그래프 설정으로 숨으면 짚을 것이 설정 파일이 된다.
2. **상태 병합 규칙이 이 도메인에서 특수하다.** 인계로 조기 종료할 때도 그때까지 모은 근거를 감사 목적으로 남겨야 하고, 생성 토큰과 임베딩 토큰은 절대 섞이면 안 된다. 범용 reducer 로 표현하면 오히려 규칙이 흐려진다.
3. **의존성을 늘리지 않는다.** 노드 8개짜리 선형 파이프라인 + 사이클 1개에 그래프 런타임을 얹으면, 얻는 것보다 설명해야 할 것이 많아진다.
4. **테스트가 단순해진다.** 각 노드가 평범한 함수·클래스라 `Protocol` 로 대역을 끼워 넣는 것으로 끝난다(`EvidenceCollecting` / `DraftGenerating`).

### LangGraph 가 유리했을 지점 (정직하게)

- **관측**: LangSmith 연동으로 노드별 지연·토큰·프롬프트 추적이 사실상 공짜로 온다. 지금은 `ProcessedInquiry` 와 처리 기록 테이블로 직접 만들었다.
- **영속·재개**: 체크포인터로 중단 지점부터 재개하는 기능. 지금은 문의 1건이 동기 처리라 필요가 없지만, 인계를 **비동기 승인 대기**로 바꾸는 순간 직접 만들어야 한다.
- **병렬 팬아웃**: 정책 검색과 주문 조회는 사실 독립이라 병렬화할 수 있다. 지금은 순차 실행이고, LangGraph 였다면 팬아웃/팬인이 선언적으로 표현됐을 것이다.
- **구조 변경 비용**: 노드가 20개를 넘고 분기가 늘어나면 `if` 문으로 엮은 제어 흐름은 유지비가 급격히 오른다. **지금 규모에서** 자체 구현이 유리하다는 것이지, 항상 그렇다는 뜻은 아니다.

---

## 게이트 2층 구조

일정 타협이 아니라 **설계 결정**이다. 성질이 다른 두 검사를 섞으면 둘 다 측정할 수 없게 된다.

| | **L1 — 결정론** | **L2 — 확률론** |
|---|---|---|
| 무엇을 보나 | citation 존재 · 참조 무결성 · 스키마 준수 · 패턴형 PII | L1 통과분에 대해 claim 단위 근거 대조 · 근거쌍 모순 |
| 방법 | 코드 검사, **LLM 0회** | LLM-as-a-judge (Anthropic `claude-sonnet-5`, 생성과 다른 계열) |
| 재현성 | **100%** | 확률적 · **과금된다** |
| 측정 | 검출률·오탐률을 L1 픽스처 **35건**으로 직접 잰다 (측정 1) | 검출률·오탐률을 판정 픽스처 11건으로 직접 잰다 (측정 3) |
| 상태 | 사이클 1 완료 | **사이클 2 완료 — 실측됨** (사이클 4 에 판정 모델 3종을 비교했고 **바꾸지 않았다**) |

**두 층은 서로를 모른다.** `gate.py` 는 LLM 을 import 하지 않고 `judge.py` 는 `gate.py` 를
부르지 않는다. 층을 결합하는 것은 `pipeline.py` 뿐이라 각 층의 정확도를 따로 잴 수 있다.

**사이클 3·4 가 손댄 곳은 게이트 앞(검색·근거 채택)이고, 두 층의 계약 — 사유 4종·2종,
층 경계, 판정 규칙 — 은 그대로다.** 다만 사이클 3 의 적대리뷰가 찾은 **결함 수정 3건이 두 층
안으로 들어갔다**(아래 [L1 절](#l1-게이트)) — 검사 범위를 넓히고 오탐 경로를 없앤 것이지
판정을 느슨하게 한 것이 아니다.

**측정 1 은 다섯 사이클 열아홉 번의 풀셋 라이브 전부에서 같은 값이다**(100.0% / 0.0%).
**단 분모는 두 판이다** — `-27` 까지가 19/19·0/8(픽스처 27건), 사이클 5 의 `-28` 이후가
26/26·0/9(픽스처 35건)다. 비율이 같다고 같은 칸이 아니다.
**측정 3 은 그렇게 말할 수 없다** — 사이클 4 가 판정 픽스처·프롬프트·모델·캐싱을 각각
건드리면서 계열이 **여섯 판**으로 갈렸다. 판을 밝히지 않은 측정 3 수치 비교는 성립하지 않는다
([status.md 의 판 표](docs/tracking/status.md)).

**그럼에도 판정 규칙 자체는 사이클 4 전후로 바뀌지 않았다** — 판정 지침(`JUDGE_SYSTEM_PROMPT`)이
바이트 동일하고 실행 조건 지문의 `judge_prompt_version` 도 그대로다. 프롬프트 강화는 사전
판정선 미달로 **원복했고**, 판정 모델 교체와 캐싱은 **채택하지 않았다**. `judge.py` 에서 실제로
바뀐 것은 **캐시 계열 토큰을 별도 칸으로 노출하는 배선**뿐이고, 측정 3 의 판이 갈린 것은
**입력(판정 픽스처)** 이 교정됐기 때문이다.

**끄면 그 층이 통째로 없다 — fail-closed 이지 fail-open 이 아니다.**
`L2_ENABLED=false` 로 판정 층을 끄는 것은 운영자의 명시적 선택이고, 그때 L1 단독으로 동작한다.
그러나 **켜져 있는 상태에서 판정 호출이 실패하면 답변을 내보내지 않고 인계한다** —
검증하지 못한 답변이 조용히 나가는 경로는 없다. 끄고 켠 두 조건의 실측 차이는
[실측값](#사이클-2-실측값) 절에 병기돼 있다.

---

## 검색 품질 — 게이트 앞의 층

**두 게이트는 "근거 대비" 판정이다.** 근거가 잘못 오면 게이트는 옳게 판정하면서도 제품은
틀린다. 방향이 둘이다 — ① 정책에 답이 **있는데 못 찾으면** 정상 문의가 `no_evidence` 로
인계되고, ② 답이 **없는데 인접 조항을 찾아오면** 위 [3-2 장면](#3-2-l2-가-잡는-장면--근거를-달았는데-그-근거가-답을-뒷받침하지-않는다)이 된다.
사이클 3 은 이 층을 게이트와 같은 방식으로 — 정답을 박아 두고 — 측정 가능하게 만들었다.

### 무엇으로 쟀나

- **검색 정답 라벨 30건**(`data/retrieval_labels.jsonl`) — 문의별로 "이 조항들이 근거여야
  한다"를 고정한다. 정답이 **빈 문의 5건**(정책에 없는 주제)도 라벨이다: 그때 정답은 0건 채택이다.
- **채택 판정은 전 전략이 코사인 하나**를 쓴다([결정 0009](docs/tracking/decisions/0009-채택-판정을-절대-축으로-통일한다.md)).
  RRF 같은 순위 점수로 채택을 정하면 "아무것도 관련 없다"를 표현할 수단이 사라진다 —
  실제로 그 구성에서는 무근거 문의의 precision 이 **구조적으로 0.0** 이었다.
- **컷 스윕 13점**(0.10~0.70)을 전략마다 돌려 각자의 최적 컷에서 읽는다. 고정 컷 한 점
  비교는 컷의 실효 강도가 전략마다 달라 불공정하다.
- **재작성 질의는 저장소 픽스처로 고정**한다 — 정책도 라벨도 본 적 없는 모델이 문의 원문만
  보고 만든 blind 입력(`data/rewritten_queries.jsonl`)과, 상한 대조용 oracle 입력을 나눠
  둔다([결정 0010](docs/tracking/decisions/0010-검색-정답의-관련성-경계를-정하고-blind-재작성을-모델이-만든다.md)).
- **라벨은 채점자만 본다.** 어떤 전략도 라벨을 입력으로 받지 않는다.

### 사다리 — 각 전략의 자기 최적 컷에서

| 단계 | best cut | precision | recall | macro F1 | r@1 | 제품 배선 |
|---|---:|---:|---:|---:|---:|---|
| 벡터 단독 | 0.50 | 0.628 | 0.640 | 0.634 | 0.560 | 기준선 |
| **+ 질의 재작성** | 0.55 | 0.652 | 0.840 | **0.734** | 0.760 | **배선함** |
| + 하이브리드(BM25+RRF) | 0.55 | 0.668 | 0.840 | 0.744 | 0.640 | 안 함 |
| + LLM 리랭크 | 0.55 | 0.680 | 0.840 | 0.752 | 0.840 | 안 함 |

산출물: [`reports/retrieval-strategies-live-*`](reports/) · 조건과 읽는 법은
[status.md 의 검색 전략 비교](docs/tracking/status.md#검색-전략-비교--첫-실측-2026-08-13-db-미사용) 절.

**세 가지를 함께 읽어야 한다.**

1. **이득의 대부분이 첫 단에서 나온다** — F1 +0.1005 대 +0.0097 대 +0.0076. 세 단을 쌓은
   총이익(+0.1178)의 **85.3%** 가 재작성 한 단이다.
2. **precision 열은 행 간 직접 비교가 아니다.** "정답이 있는데 0건 채택"은 규약상 평균에서
   빠지므로 분모가 행마다 다르다 — 이 표에서 벡터 단독은 **22건**, 나머지 세 행은 **27건**
   평균이다(recall 은 모두 25건). 분모를 함께 인쇄하는 이유가 이것이다.
3. **하이브리드는 F1 을 조금 올리면서 r@1 을 0.76 → 0.64 로 깎고**, 리랭크가 그것을 되돌린다
   (0.84). 세 임베딩 모델 전부에서 같은 방향이다 — 하이브리드를 쓴다면 리랭크와 함께여야 한다.

**상한 대조**: 같은 조건에서 oracle 재작성은 F1 0.853 이다. 배포 가능한 blind 재작성이
상한의 약 86% 를 가져온다(0.734 대 0.853). **임베딩 모델 축**은 사다리 끝에서 차이가
사라진다(3-small 0.752 · 3-large 1536 0.764 · 3072 0.759) — 볼륨 재생성 값을 못 한다.

### 무엇을 배선했나 — 재작성 한 단

**원문과 재작성 질의를 둘 다 검색해 합집합**을 취하고, 조항마다 **더 큰 유사도**를 쓴다.
재작성은 의도 해석에 얹지 않은 **별도 호출**이다 — 얹으면 프롬프트가 병합돼 위 비교표를 만든
조건과 갈리고, 그 순간 표가 런타임을 예측하지 못한다. 대가는 문의당 호출 +1 이다 — 정책
검색이 도는 문의에서 **입력 약 190 · 출력 약 20 토큰**(골든셋 30건 평균 134~148, 정책 검색이
안 도는 문의는 0)이고, 그 토큰은 생성·임베딩·판정 **어디에도 합산하지 않는 네 번째 계열**로
분리한다.
재작성 호출이 실패하면 **원문 질의로 폴백한다 — 인계가 아니다.**

제품이 실제로 쓰는 컷은 **0.30** 이므로, 그 컷에서의 오프라인 수치를 함께 싣는다.

| 단계 (컷 0.30) | precision | recall | macro F1 | r@1 |
|---|---:|---:|---:|---:|
| 벡터 단독 | 0.238 (분모 27) | 0.880 | 0.375 | 0.560 |
| + 질의 재작성 | 0.230 (분모 30) | **1.000** | 0.374 | 0.760 |
| + 하이브리드 | 0.230 (분모 30) | 1.000 | 0.374 | 0.640 |
| + LLM 리랭크 | 0.230 (분모 30) | 1.000 | 0.374 | 0.840 |

**이 컷에서 세 단의 채택 지표는 완전히 같다** — 정답 조항은 셋 다 빠짐없이 담고(recall
1.000), precision·F1 도 소수점까지 같다. 다만 **집합까지 같지는 않다**: 컷 위 후보가
`top_k` 를 넘으면 순서가 상한을 통해 구성원을 바꾸므로, 30건 중 27건에서 **정답 아닌
채움 조항**의 구성이 갈린다. 재정렬 두 단이 이 컷에서 움직이는 것은 순위(r@1)와 그
채움이지 지표가 아니다 — 배선하지 않은 이유가 여기 그대로 있다. 그리고 재작성이 바꾼 것은
precision 이 아니라
**"정답이 있는데 근거 0건"이 사라진 것**이다(그 3건이 precision 분모에 들어오면서 27 → 30).
**제품 경로에서 그 3건 중 실제로 `no_evidence` 로 끝나던 것은 G17 하나**이고(나머지 둘은
주문 근거로 답하거나 주문번호가 없어 초안 전 인계였다), 재작성은 그 조항의 유사도를
**0.264 → 0.580** 으로 올려 **라이브 3회 모두** 채택시켰다. 오프라인 검색 지표와 제품 결과는
같은 것이 아니라는 점도 함께 기록해 둔다([findings 20](docs/tracking/findings.md)).

### 라이브 3회가 판정했고, 절반을 되돌렸다

오프라인 격자는 컷을 **0.50** 으로 올리라고 했다. 라이브 3회(골든셋 30건 × 3)로 검증한 결과:

| 표적 | 결과 |
|---|---|
| ① G17 이 정답 조항을 채택해 답변 (3회 모두) | **충족 — 3/3** |
| ② 무근거 4건(G21–G24)이 **검색 단계에서 기권** (12건 중 ≥10) | **충족 — 11/12** |
| 비악화 가드 (이전에 3회 일치하던 케이스가 다시 일치) | **미달 — G04 0/3** |

기권한 11건은 **초안도 판정도 부르지 않았다** — 시도 0건, 판정 토큰 0. 남는 것은 그 앞의
의도 해석 1회(입력 208~213 · 출력 12)와 재작성 호출뿐이다. 3회 합산 판정 토큰은
**315,822 → 243,518(-22.9%)** 다(사이클 2 대비 — 조건 차이는
[실측값 절](#사이클-3-실측값--검색-실험-조건-되돌린-조건이다) 참조). 검색이 앞에서 거르면
검증 비용이 실제로 줄어든다는 것은 이렇게 확인됐다.

**그런데 같은 컷이 다른 것도 잘랐다.** 아래 넷 중 셋은 잘리면 안 되는 것이었고,
**셋 다 0.30 위**다 — 그래서 컷을 0.30 으로 되돌리면 되살아난다.

| 컷 0.50 이 자른 것 | 코사인 | 결과 |
|---|---:|---|
| G21–G24 가 물던 인접 조항 20건 | 0.347~0.481 | ✔ 검색 단계 기권 — 의도한 이득 |
| G04 의 정답 조항 `policy:support:4-6` | 0.3571 | ✘ 근거 0건 → 정상 문의가 `no_evidence` 인계 |
| G18 의 상충 짝 `policy:refund:2-4` | 0.4676 | ✘ 모순 기각 3/3 → **0/3** |
| G01 의 상충 짝 `policy:refund:2-2` | 0.4693 | ✘ 모순 기각 2/3 → **0/3** |

**컷 스윕 어디에도 G04(≤0.35)와 G21–G24(≥0.50)를 동시에 만족하는 값이 없다.** 문제정의의
"임계값 하나로는 양방향을 못 막는다"가 재작성 뒤에도 그대로였다. 그래서 **컷 한 축만 되돌렸다**
— 재작성은 깎은 케이스가 없으므로 유지한다([결정 0011](docs/tracking/decisions/0011-원복을-축별로-가른다.md)).
현재 기본값은 **재작성 켜짐 · 임계값 0.30 · top_k 5** 다.

### 모순 기각은 검색에 종속된다 — 커밋된 리포트가 그대로 보여준다

L2 가 근거 간 모순을 잡으려면 **검색이 상충하는 두 조항을 함께** 올려야 한다.
컷 실험은 그것을 우연히 증명했다 — 채택 근거 목록만 보면 된다.

| 케이스 | 상충 짝의 코사인 | `contradictory_evidence` 발화 |
|---|---|---|
| G18 하자 상품 환불 서류 | `2-6` 0.723 · `2-4` **0.468** | 짝이 갈라짐 → **3/3 → 0/3** (3회 모두 첫 시도 pass) |
| G01 환불 기간 상충 | `2-1` 0.729 · `2-2` **0.469** | 짝이 갈라짐 → **2/3 → 0/3** |
| G02 배송비 기준 상충 | `1-3` 0.664 · `1-4` 0.653 | **둘 다 컷 위** → 두 조건 모두 **1/3** (유지) |

컷 0.30 에서는 세 짝이 모두 함께 채택됐고(비교 리포트의 채택 목록), 0.50 에서는 값이 갈린
두 짝만 끊겼다. **G02 만 살아남은 이유가 그 숫자 하나다.**
**심어 둔 장치가 있는 것만으로는 부족하고, 그 장치에 닿는 경로가 유지되는지도 함께 재야 한다.**
이 회귀는 합산 일치율(96.7%)에도, 케이스 단위 비악화 가드에도 잡히지 않았다 — 잡은 것은
사이클 3 이 새로 넣은 **케이스별 귀인 절** 하나였다.

### 채택 축을 하나 더 만들었다 — 컷은 그대로 두고 질의 축을 얹었다

컷 하나로 세 방향을 지킬 수 없다는 것이 두 번 실측됐으므로, **컷을 옮기는 대신 축을 늘렸다.**
후보 셋 중 둘은 **손계산이 먼저 반증했다** — 커밋된 검색 산출물만 읽는 무과금 재계산이다
(`uv run python -m scripts.handcalc_adoption_axis`).

| 후보 축 | 처분 | 근거 |
|---|---|---|
| 질의별 **적응 컷**(1위 대비 비율) | **반증** | 상대 축은 **1위를 절대 지우지 못한다** — `s1 ≥ 비율 × s1` 이 항상 참이다. 절대 하한과 곱한 전수 격자 **576 구성 중 세 방향 동시 만족 0개** |
| **상대 마진**(1위와의 차) | **반증** | 같은 이유. 0.01 간격 대조 격자 106,641 구성에서도 0개 |
| **상충쌍 보존 규칙** | **범위 밖** | 컷 0.30 이 세 쌍을 **이미 함께 채택**해 고칠 것이 0건. 규칙을 걸면 끌어올리는 10건이 전부 그 케이스의 정답이 아니고, 기권 표적 G22 에서 과채택만 는다 |
| **질의 단위 기권 게이트** | **채택** | 아래 |

남은 하나를 격자로 확정했다. 통계량 후보 5종 × τ 스윕 = **165 구성**에 채택 규칙 다섯을
**순서대로** 걸었다: ① 케이스 하한 → ② 상충쌍 보존 → ③ 기권 → ④ 비악화 → ⑤ 동률.

| 어느 규칙에서 죽었나 | 구성 수 |
|---|---:|
| ① 케이스 하한 — 정답 조항을 하나도 못 채택한 케이스 발생 | **124** |
| ③ 기권 — G21–G24 채택 0건 미달 | **37** |
| ⑤ 동률 규칙으로 넘어간 채택 후보 | **4** |

**① 케이스 하한이 격자의 3/4 을 죽인다.** 사이클 3 의 오프라인 격자에 없던 바로 그 규칙이고,
없어서 컷 0.50 이 뽑혔다 — 집계 F1 이 **오르는 걸음에서** 케이스가 죽었기 때문이다.
게이트 **꺼짐 기준선**도 규칙 ③에서 탈락한다(G21–G24 가 각 5건씩 20건을 채택한다).

동률이던 넷 중 **가장 단순한 하나**를 골랐다(규칙 ⑤).

| 인자 | 값 |
|---|---|
| 통계량 | `rank1_minus_rank_k_spread` — 상위 `top_k` 중 **1위 − `top_k`위** 뺄셈 하나 |
| τ | **0.06** (통계량 < τ 이면 그 질의는 채택 0건) |
| 통계량 입력 | `top_k`(=5) **선절단** 뒤의 측정된 코사인만 — 컷 **전** 슬라이스다 |
| 컷 · `top_k` · 응답 계약 | **전부 무변경** |

경계는 얇다: 채택 쪽 **G15 가 0.0668(+0.0068)**, 기권 쪽 **G23 이 0.0521(−0.0079)**.
분리 여유는 0.0147 이고, **τ 를 한 눈금(0.01) 올리면 가장 먼저 죽는 것이 G15 다.**

> **τ 는 임베딩 모델에 묶인다.** `text-embedding-3-small` 1536 에서만 분리했고,
> `-3-large` 1536·3072 에서는 후보 통계량 **다섯 개 전부**가 기권군과 채택군을 **분리조차
> 못 한다**(이 통계량 기준 여유 −0.0114 / −0.0160). 임베딩을 상향하면 게이트가 오류 없이
> 조용히 무력해지므로 τ 재산출은 **명시적 작업**이다 — 그래서 실행 조건 지문에서 τ 와
> `embedding_model` 은 한 쌍이다
> ([engineering-notes](docs/engineering-notes.md) "τ 는 임베딩 모델에 묶인다").

### 새 축 라이브 3회 — 세 방향이 동시에 성립했다

산출물 `reports/evaluation-live-l2-{10,11,12}` (골든셋 30건 × 3회). 승격 기준선
(`evaluation-live-l2-{7,8,9}`)과 **기권 게이트 두 항목만 다르고 나머지 지문 16개는 전부 같다.**

| 방향 | 표적 | 결과 |
|---|---|---|
| ① 정답 조항 보존 | G04 가 `policy:support:4-6` 을 채택하고 정상 답변 | **3/3** |
| ② 무근거 문의 기권 | G21–G24 가 **검색 단계에서** 채택 0건 | **12/12** — `no_evidence` · 시도 0건 |
| ③ 상충쌍 보존 | G18·G01·G02 가 상충쌍을 **함께** 채택 | **각 3/3** |

**컷 하나로는 이 셋을 동시에 못 지킨다는 것이 두 번 실측됐고, 축을 늘려 세 방향을 동시에
지켰다.** 컷 스윕 어디에도 G04(≤0.35)와 G21–G24(≥0.50)를 동시에 만족하는 값이 없다는 사실은
**그대로다** — 바뀐 것은 축의 개수다.

**얼굴이 바뀐 것은 인계 사유다.** 기준선에서 G21–G24 는 12건 모두 `rejected_twice` 였다 —
인접 조항으로 초안을 두 번 만들고 판정을 두 번 태운 뒤 인계. 새 축에서는 12건 모두
`no_evidence` 이고 **시도 0건**이다.

| 묶음 | 총 시도 | L1 선기각 | 초안 없이 끝난 문의 (90 중) | 그중 `no_evidence` |
|---|---:|---:|---:|---:|
| 기준선 `-7`·`-8`·`-9` | 97 | 13 | 12 | **0** |
| 새 축 `-10`·`-11`·`-12` | 76 | 2 | 24 | **12** |

**두 절감을 더하면 이중 계산이다** — 기준선에서 L1 이 12번 걸러 낸 그 네 건이, 새 축에서는
검색 단계에서 끝난다. 검색이 먼저 거르면 L1 은 거를 것이 없어진다.

**과장하지 않는다.** ① 3/3·12/12 는 **관측이지 결정론이 아니다** — 재작성 산출이 실행마다
달라 점수 분포가 움직인다(표적 밖의 G28 은 정책 검색이 돈 2회 중 1회만 기권했고, 나머지 1회는
의도 분류가 `order` 로 가서 정책 검색이 돌지 않았다). ② τ 는 이 임베딩 모델의
값이다. ③ 규모는 **골든셋 30건 × 3회 = 90 케이스-실행, 합성 데이터**다.

### 청킹 — 비교했고 반영하지 않았다

`policy_index.py` 는 첫 사이클부터 **조항 단위** 청킹을 썼고 그 근거는 산문이었다("정책 문서는
조항 경계가 곧 의미 단위다"). 사이클 4 가 그것을 실측으로 확인했다 — 고정 크기 청킹과 같은
원문·같은 라벨·같은 채점기로.

산출물 `reports/retrieval-chunking-live-text-embedding-3-small-d1536-k5-c030-m050`.
채점은 **청크→조항 투영**이다: 청크가 조항 **본문 span** 의 일정 비율 이상을 담으면 적중
(기본 0.50, 민감도 5점 병기). 제목줄만 스치는 청크는 어떤 비율에서도 적중이 아니다.

| 청킹 (원문 질의) | 단위 수 | r@1 | r@5 | **r@5(조항 예산)** | P@0.30 | **F1@최적** | 케이스당 채택 조항 |
|---|---:|---:|---:|---:|---:|---:|---:|
| **조항 단위 (현행)** | 26 | 0.5600 | **1.0000** | **1.0000** | **0.2383** | **0.6340** | **4.17** |
| 고정 120자 · 오버랩 없음 | 24 | 0.5400 | 0.8800 | 0.8600 | 0.1895 | 0.5995 | 4.57 |
| 고정 120자 · 오버랩 30자 | 31 | 0.5200 | 0.8800 | 0.8400 | 0.1936 | 0.5268 | 4.30 |
| 고정 240자 · 오버랩 없음 | 13 | **0.6600** | 1.0000 | 0.9200 | 0.1198 | 0.5519 | 7.93 |
| 고정 240자 · 오버랩 60자 | 16 | 0.6400 | 0.9600 | 0.8600 | 0.1183 | 0.5271 | 7.53 |

| 청킹 (blind 재작성 질의로 **치환**) | r@1 | r@5 | **r@5(조항 예산)** | P@0.30 | **F1@최적** | 케이스당 채택 조항 |
|---|---:|---:|---:|---:|---:|---:|
| **조항 단위 (현행)** | 0.7600 | **1.0000** | **1.0000** | **0.2300** | **0.7517** | **4.77** |
| 고정 120자 · 오버랩 없음 | 0.6400 | 0.9600 | 0.9600 | 0.2089 | 0.6251 | 5.27 |
| 고정 120자 · 오버랩 30자 | 0.6000 | 0.9400 | 0.9400 | 0.1903 | 0.5861 | 5.07 |
| 고정 240자 · 오버랩 없음 | **0.8200** | 1.0000 | 1.0000 | 0.1043 | 0.5960 | 10.00 |
| 고정 240자 · 오버랩 60자 | 0.8000 | 0.9600 | 0.9000 | 0.1303 | 0.5793 | 8.73 |

**고정 크기가 이기는 지표는 상위 순위(r@1) 하나뿐이고, 그 이득의 출처는 검색 품질이 아니다.**
정답이 둘인 케이스 넷 중 **셋이 원문에서 연속**(G01·G02·G20)이라 240자 청크 하나가 조항 둘을
통째로 싣는다. 그 셋을 빼고 남은 **27건**에서 다시 재면(컷 0.30 · 최소 포함 비율 0.50)
재작성 조건의 격차가 **+0.0600 → 0.0000** 으로 사라지고 원문 조건에서도
**+0.1000 → +0.0455** 로 절반 이상 사라진다. 같은 예산을 조항으로 환산한
`r@5(조항 예산)` 에서는 240자가 1.0000 → 0.9200 으로 내려가고 조항 단위가 1.0000 을 지킨다.
**`F1@최적` 기준으로 조항 단위가 10개 조합(2조건 × 5비율) 전부에서 1위다.**

**처분: 제품은 한 줄도 바뀌지 않았다.** 고정 크기가 이겼더라도 처분은 같았을 것이고, 이유는
검색 품질이 아니라 **범위**다 — 청킹 교체는 인덱싱 변경이 아니라 **답변 계약 변경**이다
(`contracts.policy_evidence_id()` · DB CHECK `policy_chunks_evidence_id_shape` · L1 의
citation 검사 · 문서 셋이 함께 열린다).
전문은 [결정 0015](docs/tracking/decisions/0015-청킹은-조항-단위를-유지하고-고정-크기는-비교까지만-한다.md).

**과대 해석 금지**: 이 코퍼스는 조항 26개 · 본문 평균 86.4자로 **조항이 짧고 자족적이다.**
조항 하나가 여러 문단에 걸치는 문서에서는 고정 크기가 이길 수 있다. 그리고 이 격자는
**벡터 단독**이고 하이브리드·리랭크와 결합했을 때의 결론이 아니다.

### 지금 서 있는 자리 — 정직하게

- **컷 0.30 + 재작성 조합의 라이브 실측이 생겼다** — 사이클 4 T2 3회
  (`evaluation-live-l2-{7,8,9}`)이고, 그 셋이 **승격 기준선**이다. 그 전까지는 오프라인
  수치가 유일한 근거였다.
- **기권은 결정론이 아니다.** 게이트 자체는 LLM 호출 0회지만 **입력이 확률 층을 통과한
  점수**다 — 재작성 산출이 실행마다 달라진다. 사이클 3 의 11/12, 사이클 4 의 12/12 둘 다
  관측이고 "항상"이 아니다.
- **τ 는 조건 종속 인자다.** 임베딩 모델·코퍼스가 바뀌면 따라오지 않고, `-3-large` 계열에서는
  이 축이 애초에 분리하지 못한다. 재산출은 명시적 작업이다.
- **집계와 케이스 단위의 어긋남 자체는 사라지지 않았다.** 이번에는 채택 규칙에 케이스 단위
  하한을 **명시해서** 피한 것이지 집계에 계단이 생긴 것이 아니다
  ([findings 18](docs/tracking/findings.md)).

---

## 평가 설계

차별화 축은 **신뢰성 지표**(검출률·오탐률)다. 생산성 지표(응답 시간 단축)가 아니다.
"몇 초 빨라졌다"는 주장은 게이트가 틀린 답변을 통과시켰을 때 아무 의미가 없기 때문이다.

**결정론 층과 확률 층을 분리해서 측정한다.** 섞으면 어느 쪽 실패인지 알 수 없다.

### 측정 1 — L1 게이트 단위 정확도 (결정론)

고정된 (초안, 근거) 쌍 픽스처 셋에 대해:

- **구조적 오류 검출률** — 사유 4종 각각을 심은 초안을 L1 이 잡아내는가
- **정상 초안 오탐률** — 근거를 제대로 딛고 선 초안을 잘못 기각하지 않는가

LLM 을 호출하지 않으므로 **100% 재현**된다. 이것이 **신뢰성 서사의 헤드라인 수치**다.

### 측정 2 — 파이프라인 판정 일치율 (end-to-end)

골든셋 30건(정상 · 기각 유발 · 무근거 · 인계 유도 혼합)에 대해:

- **허용 결과 집합 대비 일치율** — **초안 전 인계 경로를 포함**해서 센다
- 미끼 문의(`reject_bait`)의 **기각 재현율** — 목표 없는 관측값이고, 분모는 라벨이 아니라
  범주다([결정 0008](docs/tracking/decisions/0008-미끼-라벨을-판정-규칙에-정렬한다.md))
- **p50/p95 지연**, **건당 토큰**(생성 · 임베딩 · **판정** 세 계열을 끝까지 분리 기록 —
  provider 와 단가가 다른 계열을 합산하면 건당 비용 지표가 무너진다)

### 측정 3 — L2 판정 단위 정확도 (확률 층, 사이클 2 신설)

측정 1 과 **같은 모양**의 수치를 확률 층에 대해 낸다. 고정된 (초안, 근거) 픽스처
11건(`data/judge_fixtures.jsonl`)을 **판정기에 직접** 흘려 기대 판정과 대조한다 —
파이프라인을 통과시키지 않으므로 생성기의 변동이 판정 정확도에 섞이지 않는다.

- **L2 검출률** — `unsupported_claim` · `contradictory_evidence` 를 심은 초안을 잡아내는가
- **L2 오탐률** — 근거가 제대로 뒷받침하는 초안을 잘못 기각하지 않는가
- **사유 집합 완전일치율** · claim 단위 판정 일치 · 모순 근거쌍 검출

측정 1 과 달리 **확률 층이고 과금된다.** 재실행하면 값이 달라진다.
**목표치는 두지 않는다** — 첫 실측 뒤 무목표 관측으로 확정했다
([결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md)).
픽스처가 11건이라 한 건이 9.1%p 이고, 실측값 근처에 목표를 박으면 목표가 아니라 현황
기술이 된다. 리포트는 수치만 싣고 달성·미달 판정을 붙이지 않는다.

### 재현성을 어떻게 다루나

**결정론을 샘플링 파라미터로 보장하지 않는다**(`temperature` 등을 아예 보내지 않는다 — 모델 계열에 따라 받지도 않는다).
대신 **평가 설계로 흡수한다**: 게이트 정확도는 결정론 층인 L1 픽스처로 재고,
end-to-end 라벨은 단일 정답이 아니라 **허용 결과 집합**으로 정의한다.

### 목표치

측정하기 전에 목표를 적어 두면 숫자를 맞추러 가는 일이 되므로, **실측 3회를 확보한 뒤에**
확정했다([결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md)).
목표는 `evaluation.TARGETS` 에 있고 리포트가 매 실행 달성 여부를 찍는다.

| 지표 | 목표 | 사이클 1 (L2 꺼짐) | 사이클 2 (L2 켜짐) | 사이클 3 (컷 0.50, 원복됨) | 사이클 4 T2 (옛 승격 기준선) | 사이클 4 T10 (게이트 τ=0.06) | **사이클 5 T8** (현재 기본값 · 승격 기준선) | 판정 |
|---|---|---|---|---|---|---|---|---|
| 측정 1 구조적 오류 검출률 | ≥ 100% | 100.0% (19/19) | 100.0% (19/19) | 100.0% (19/19) | 100.0% (19/19) | 100.0% (19/19) | **100.0% (26/26)** | 달성 |
| 측정 1 정상 초안 오탐률 | ≤ 0% | 0.0% (0/8) | 0.0% (0/8) | 0.0% (0/8) | 0.0% (0/8) | 0.0% (0/8) | **0.0% (0/9)** | 달성 |
| 측정 2 일치율 | ≥ 75% (**하한 경보선**) | 71.1% (미달) | 87.8% | 96.7% | 100.0% | 100.0% | **100.0%** | **경보 없음** |
| 측정 3 L2 검출률·오탐률 | **없음(무목표)** | (측정 없음) | 100.0% / 0.0% | 100.0% / 0.0% | 100.0% / 3회 중 1회 20.0% | 100.0% / 3회 중 1회 20.0% | **100.0% / 0.0% (3회 동일)** | 판정 없음 |

L1 두 지표를 만점으로 박은 것은 결정론 층이라 달성이 재현되기 때문이다 — 여기서 내려가는
것은 목표 미달이 아니라 **회귀**다. **다섯 사이클 열아홉 번의 풀셋 라이브 전부에서 100.0% /
0.0% 로 무변경**이다.

**측정 1 칸에는 분모를 함께 적었다.** 사이클 5 가 L1 채점표를 27건에서 **35건**으로 넓혔다
(위반 19→26 · 정상 8→9, **추가만** 하고 기존 27건은 바이트 동일). 비율이 같다고 같은 칸이
아니다 — **분모를 빼고 나란히 인용하면 안 된다.**

**사이클 3 열은 검색 실험 조건(임계값 0.5)의 값이고, 그 조건은 되돌렸다** —
[검색 품질 절](#검색-품질--게이트-앞의-층) 참조. 세 지표가 나란히 좋아 보이지만
그 뒤에서 정상 문의 하나가 죽었다는 것이 아래 문단의 요지다.

**사이클 4 두 칸의 측정 3 오탐 20.0% 는 J04 한 건이고 3회 중 1회다.** J04 는 판정 계열
`-1` … `-12` 에서 산발적으로 흔들리던 픽스처이고, 판정 픽스처를 교정한 뒤(`-13` … `-31`)
**19세트에서 18/19 가 `pass`** 였다 — 남은 한 칸이 `-25`(판정 프롬프트 캐싱 켬,
[결정 0019](docs/tracking/decisions/0019-판정-프롬프트-캐싱을-실측하고-채택하지-않는다.md))다.
**교정으로 빈도는 줄었지만 흔들림이 닫힌 것은 아니다.**

**일치율 75% 는 사이클 1 에서 닿지 않는 값이었고, 그게 의도였다.** 사이클 2 에서 87.8% 로
넘어섰다. 다만 **목표를 넘겼다는 것이 문제가 사라졌다는 뜻은 아니다** — 남은 어긋남
11 케이스-실행은 전부 미끼 문의(G16·G17·G19·G20)였고, 사이클 1 에서 간극의 절반을
차지하던 무근거 문의 G21–G24 가 L2 기각으로 인계되면서 빠진 것이 상승분의 대부분이다.
그리고 이 값은 **골든셋 30건 × 3회 = 90 케이스-실행, 합성 데이터** 위의 수치다.

**그 남은 어긋남 11건은 재조사에서 게이트가 아니라 채점표 문제로 판명됐다** — 정면 조항을
딛은 "기재되어 있지 않다" 안내는 설계가 통과시키기로 정한 동작인데 라벨이 기각을 요구하고
있었다. 미끼 라벨을 판정 규칙에 정렬했고
([결정 0008](docs/tracking/decisions/0008-미끼-라벨을-판정-규칙에-정렬한다.md)), 커밋된
리포트를 새 라벨로 재채점하면 71.1%/87.8% 는 **86.7%(L2 꺼짐) / 100.0%(L2 켜짐)** 이 된다 —
새 실측이 아니라 재채점이고, 리포트 원본은 사후 편집하지 않았다. 이 재정렬로 **L2 꺼짐
기준선도 목표 75% 를 넘게 되므로**, 목표치는 새 라벨 기준의 첫 실측 뒤에 재확정했다.

**재확정 결과: 75% 는 유지하되 성격이 바뀌었다 — 달성 목표가 아니라 하한 경보선이다**
([결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md), 2026-08-14 갱신).
새 라벨에서는 L2 를 꺼도 86.7% 라 이 경계선이 층의 기여를 판별하지 못하고, 사이클 3 이
**합산 지표만으로는 회귀를 못 잡는다**는 것을 실측으로 보였기 때문이다 — 일치율은 96.7% 인데
그 뒤에서 정상 문의 하나가 죽었고, 더 나쁜 하나는 합산에도 케이스 가드에도 안 잡혔다.
그래서 회귀 판정은 **케이스 단위**가 맡는다: ① 이전 실측에서 3회 모두 일치한 케이스가 다시
일치하는가(비악화 가드) ② 케이스별 귀인. 지표를 올리는 변경일수록 이 둘을 먼저 본다.
당시에는 **둘 중 ②만 리포트가 매 실행 계산했고 ①은 사람이 이전 리포트와 대조해 판정했다.**
하네스 코드로 상설화하는 것을 다음 사이클 몫으로 넘겼다 — **사이클 4 가 그것을 닫았다(아래).**

**측정 3 에는 목표치를 두지 않기로 확정했다.** 첫 실측이 나왔다고 그 값 근처에 목표를
박으면 목표가 아니라 현황 기술이 된다. 회귀는 실측값 비교로 잡되 **비교는 같은 판 안에서만
성립한다** — 사이클 4 에 판정 층을 세 갈래로 밀면서 측정 3 계열이 **여섯 판**으로 갈렸다
([status.md 의 판 표](docs/tracking/status.md)). 그리고 **측정 3 무변경은 회귀 가드의 검사
대상이 아니다** — 판정 층 개선의 성공이 무관한 축의 원복을 발동시키면 안 되기 때문이다.

**미끼 기각 재현율에는 목표를 두지 않는다.** 그 수치는 모델의 정직함에 달려 있어 우리
코드가 올릴 수 있는 값이 아니다. 목표로 박으면 올리는 유일한 길이 생성 쪽을 약화시키는
것이 되고, 그건 게이트를 쓸모 있어 보이게 하려고 생성기를 망가뜨리는 일이다. 6.7% →
26.7% 의 변화도 **개선 주장이 아니라 관측 기록**으로만 남긴다.

### 회귀 가드 — 실행마다 두 줄로 보고한다

위 문단이 "다음 사이클 몫"이라고 적었던 것이 **사이클 4 에 코드로 들어갔다.**
라이브 실행마다 `regression_guard.py` 가 두 줄을 낸다.

| 줄 | 기준선 | 성격 |
|---|---|---|
| **승격 기준선** | `data/promoted_baseline.json` 이 가리키는 세트 | **구속** — 비악화 판정과 "미달이면 원복이 기본"의 발동 근거 |
| **직전 라이브** | `reports/` 자동 탐색 | **경보** — 정보로 병기, 판정을 발동시키지 않는다 |

**두 줄이 상반되면 승격 기준선이 이긴다.** 직전 라이브를 구속으로 쓰면 판정이 표류한다 —
어제 나빠진 값이 오늘의 기준이 되기 때문이다.

**비악화 판정은 두 겹이다.**

1. 기준선에서 3회 모두 일치한 케이스가 새 실측 3회 중 **2회 이상** 일치하는가.
2. **근거 부분 손실** — 기준선이 채택했던 **정답 근거 ID** 가 새 실측의 케이스별 다수결에서
   빠졌는가. 빠졌으면 미달이고 **빠진 ID 를 이름으로 찍는다.**

**②가 없으면 이 사이클을 촉발한 회귀를 그대로 통과시킨다.** 사이클 3 에서 G18 의 모순 기각이
사라졌을 때 커밋된 6세트 전부가 `matched=True` 였다 — 라벨이 `answered` 도 정답으로 허용하기
때문이다. 합산 일치율도, `matched` 하나짜리 가드도 그것을 못 봤다.

**조건이 다른 실측끼리는 대조하지 않는다.** 실행 조건 **지문** 25개 항목(라벨 버전 · 검색 정답
라벨 버전 · 컷 · **기권 게이트 통계량·τ·미정의 처분** · 재작성 · **검색 정렬** · 임베딩
모델·차원 · `top_k` · 생성/판정 모델 · effort · **판정 thinking 전송** · 판정 프롬프트·픽스처
버전 · 캐싱 · **초안 판정 규칙 판 · L1 채점표 판 · 조회 가드 판** · 측정 범위 ·
**실행 완주 여부** …)이 대조 가능성을 가른다. 사이클 5 가 18 → 25 로 넓혔다 —
**넓히면 그 이전에 커밋된 라이브 전부가 새 항목에서 "미상"이 되어 대조 계보가 한 번
끊긴다.** 재등재 시점이 그것을 감당할 수 있는 유일한 자리다.
**τ 와 임베딩 모델은 한 쌍으로 읽는다** — 모델이 다르면 τ 가 같아도 대조 불가다.
의도적으로 바꾼 축은 `--declare-experiment` 로 **선언**해야 대조가 진행되고 차이가 병기된다.

세 가지를 더 못박았다.

- **"검사가 안 돌았으면 통과가 아니라 보류"** 다. 기준선 산출물에 귀인 절이 없거나, 3회 세트가
  아직 안 찼거나, 공통 케이스가 없으면 헤드라인은 **보류**이고 제외된 산출물과 사유가 함께
  실린다. 사유를 각주에만 남기고 헤드라인을 통과로 적는 것은 금지다.
- **정족수는 조건이 확인된 실측으로만 채운다.** 지문이 확인되지 않는 옛 산출물을 끌어와 3회를
  만들면 다른 조건의 결과가 이번 일치 횟수로 계산돼 **거짓 미달**이 난다(실제로 났다).
- **승격·재등재는 사람만 한다.** 저장소 어디에도 `data/promoted_baseline.json` 에 **쓰는**
  경로가 없고 구조 테스트가 그것을 검사한다. 코드가 구속 기준선을 갱신하면 판정이 자기추인이 된다.

**어긋나 있던 것을 사이클 5 가 닫았다.** 승격 기준선이 기권 게이트 배선 **전**에 등재돼
있어서, 현재 기본값으로 도는 모든 실측이 `abstention_gate_statistic`·`abstention_tau`·
`judge_fixture_version` 세 항목에서 기준선과 어긋났고 **구속 줄이 매 실행 "대조 불가"** 였다.
재등재는 풀셋 라이브 3회를 새로 사는 일이고 **승격은 사람만 한다** — 사용자가 라이브 3회를
승인해 사고(`evaluation-live-l2-{28,29,30}`) 그 셋을 **직접 등재**했다(2026-08-21).

**닫혔다는 증거는 확인 라이브 1회다.** 등재 **뒤에** 한 번 더 사서 돌렸고, 구속 줄이
**선언 없이 대조를 진행했으며 어긋난 항목이 비었다**(지문 25칸 완전 일치 · 미상 0).
**그 회차의 판정 자체는 "미달"이다** — 케이스 G06 이 기준선 3/3 에서 이번 0/1 로 떨어졌다.
귀인은 이번 사이클의 변경을 가리키지 않는다(제품 코드 무변경 · 전체 19세트 중 앞선 18세트
만의 첫 이탈 · 이탈 지점이 **의도 분류**).
**두 사실을 함께 적는다 — 한쪽만 적으면 읽는 사람이 속는다.**
그리고 그 "미달"은 커밋된 리포트에 영구히 남는다(사후 편집하지 않는다).

### 사이클 2 실측값

실행 조건: **2026-08-06, L2 켜짐, 3회 반복** · 생성 `gpt-5.6-terra` · 임베딩
`text-embedding-3-small` · 판정 `claude-sonnet-5` · 유사도 임계값 0.3 · top k 5 ·
골든셋 30건 · L1 픽스처 27건 · 판정 픽스처 11건.

비교 대상인 **사이클 1 은 L2 꺼짐 조건**(2026-08-05, 3회)이다. 아래 표는 두 조건을 항상
병기한다 — 조건을 빼고 비교하면 특히 지연 수치가 거짓말이 된다.

```bash
uv run python -m scripts.evaluate          # 측정 1 만 (API 키 불필요)
uv run python -m scripts.evaluate --live   # 측정 1+2+3 (실제 호출 · 과금)
uv run python -m scripts.evaluate --stub-llm   # 배관만 확인 (실제 수치 아님)
```

리포트는 `.md` 와 `.json` 두 형식으로 나온다. **과금 실행은 파일 이름이 다르다** —
자격은 `--live` 를 줬는지가 아니라 **과금 측정이 실제로 돌았는지**다. 과금 실행은
`reports/evaluation-live*.*` 에, 나머지는 `reports/evaluation.*` 에 쓴다(`--live` 를 줬어도
선검사에 걸려 과금이 없으면 후자다).
아래 수치의 근거는 저장소에 커밋돼 있다(그 밖의 리포트는 gitignore 된다):
**사이클 2 = `reports/evaluation-live-l2-{1,2,3}.{md,json}`**,
사이클 1 = `reports/evaluation-live-{1,2,3}.{md,json}`.
측정 2·3 은 확률 층이라 **재실행하면 값이 달라진다** — 그래서 1회가 아니라 3회를 싣는다.

#### 측정 1 — L1 게이트 단위 정확도 (결정론, **픽스처 27건 조건**, 외부 호출 0회)

> **분모를 병기한다.** 아래 값은 L1 채점표가 27건이던 시절의 실측이고 커밋된 리포트의 기록
> 그대로다. 사이클 5 가 채점표를 **35건**으로 넓혔으므로
> [26/26 · 0/9](#사이클-5-실측값--현재-기본값이자-승격-기준선이다) 와 나란히 인용하면 안 된다.

| 지표 | 값 (3회 모두 동일) |
|---|---|
| 구조적 오류 **검출률** | **100.0%** (19/19) |
| 정상 초안 **오탐률** | **0.0%** (0/8) |
| 사유 목록까지 정확히 일치 | 100.0% (27/27) |

사유 4종(`schema_violation` 9 / `missing_citation` 4 / `invalid_citation` 5 / `pii_detected` 6) 전부 100% 검출, 오발화 0.
**사이클 1 과 완전히 같다** — 결정론 층이므로 이것이 정상이고, 여기가 흔들리면 회귀다.

#### 측정 2 — 파이프라인 판정 일치율 (확률 층, 골든셋 30건 × 3회)

| 지표 | 1회차 | 2회차 | 3회차 | 합산 | 사이클 1 (L2 꺼짐) |
|---|---|---|---|---|---|
| 허용 결과 집합 대비 일치율 | 86.7% (26/30) | 90.0% (27/30) | 86.7% (26/30) | **87.8% (79/90)** | 71.1% (64/90) |
| 미끼 문의의 **기각 재현율** | 20.0% (1/5) | 40.0% (2/5) | 20.0% (1/5) | **26.7% (4/15)** | 6.7% (1/15) |
| 지연 p50 | 12,431 ms | 12,281 ms | 11,060 ms | **약 11.9초** | 약 4.1초 |
| 지연 p95 | 37,690 ms | 25,458 ms | 42,233 ms | **약 35.1초** | 약 8.1초 |
| 건당 토큰 — 생성 | 1,457.4 | 1,357.8 | 1,392.1 | — | 1,301.3 / 1,230.5 / 1,272.9 |
| 건당 토큰 — 임베딩 | 14.6 | 15.6 | 14.6 | — | 14.8 (3회 동일) |
| 건당 토큰 — **판정** | **3,782.9** | **3,213.7** | **3,530.8** | — | (없음) |
| 건당 토큰 — 합계 | 5,254.8 | 4,587.1 | 4,937.4 | — | 1,316.1 / 1,245.3 / 1,287.7 |

**지연 비교에는 조건이 반드시 붙는다.** `latency_ms` 는 판정 호출을 포함하므로 사이클 2 의
p50 11.9초 / p95 35.1초는 **L2 켜짐** 수치이고, 사이클 1 의 4.1초 / 8.1초는 **L2 꺼짐**
기준선이다. 느려진 것이 아니라 **한 층이 더 붙은 것**이다.

**건당 비용의 대부분은 판정이다.** 1회차 기준 건당 5,254.8 토큰 중 **판정이 3,782.9(약 72%)**
이고 생성은 1,457.4, 임베딩은 14.6이다. 검증을 붙이는 값은 공짜가 아니며, 이 수치는 세 계열을
합산하지 않고 끝까지 분리해 세기 때문에 그대로 읽힌다. (판정 토큰은 생성과 **provider·단가가
다르므로** 토큰 수를 그대로 비용비로 읽으면 안 된다.)

**일치율 71.1% → 87.8% 의 출처는 특정돼 있다.** 무근거 문의 G21–G24 네 건이 사이클 1 에서는
12회 모두 `answered` 로 끝났는데, 사이클 2 에서는 12회 모두 `rejected_twice` 로 인계됐다 —
L2 의 `unsupported_claim` 이 인접 조항을 딛은 "안내가 어렵다"를 기각했기 때문이다.
**남은 어긋남 11 케이스-실행은 전부 미끼 문의**(G16·G17·G19·G20)다.

**규모를 함께 읽어야 한다.** 골든셋 **30건 × 3회 = 90 케이스-실행**이고 **합성 데이터**다.
실사용자도 실운영 트래픽도 없다. 이 숫자는 "이 게이트가 무엇을 잡고 무엇을 못 잡는지"의
재현 가능한 기록이지, 일반적 성능 주장이 아니다.

**미끼 기각 재현율은 여전히 목표 없는 관측값이다.** 6.7% → 26.7% 로 올랐지만 `pii_detected`
발화는 사이클 2 에서도 **0/15** 다 — 재현된 기각은 L2 의 `contradictory_evidence`(G18, 3회
모두)와 L1 의 `missing_citation`(2회차 G16)이다. 모델은 여전히 값을 지어내지 않는다.

#### 측정 3 — L2 판정 단위 정확도 (확률 층, 판정 픽스처 11건 × 3회, 사이클 2 신설)

| 지표 | 값 (3회 모두 동일) | 목표 |
|---|---|---|
| **L2 검출률** | **100.0%** (6/6) | 없음(무목표) |
| **L2 오탐률** | **0.0%** (0/5) | 없음(무목표) |
| 사유 집합 완전일치율 | 90.9% (10/11) | 없음(무목표) |
| claim 단위 판정 일치 | 100.0% (19/19) | 없음(무목표) |
| 모순 근거쌍 검출 | 기대 4건 중 3건 (기대 밖 검출 0건) | 없음(무목표) |
| 판정 실패(error) | **0건** (11/11 판정 성공) | — |

**목표치가 없으므로 달성·미달 판정이 붙지 않는다.** 첫 실측값으로만 기록한다.

사유 2종별로는 `unsupported_claim` 4/4(100.0%), `contradictory_evidence` 2/3(66.7%),
오발화는 양쪽 다 0이다.

**어긋난 1건은 3회 모두 같은 픽스처다.** `J11`(combined)에서 기대는
`reject[unsupported_claim, contradictory_evidence]` 인데 실제는 `reject[unsupported_claim]`
만 나왔다 — **기각 판정 자체는 맞았고 사유 하나를 덜 짚었다.** 확률 층인데도 세 번 모두
같은 방식으로 어긋났다는 것은, 이것이 실행 간 잡음이 아니라 판정기의 재현되는 성질이라는
뜻이다. 모순 검출이 뒷받침 판정보다 약한 쪽이라는 신호였고, 사이클 4 의 입력이 됐다.

> **이 절의 수치는 사이클 2 조건의 것이다** — 판정 픽스처 `fixture-836322e97320` · 판정
> 프롬프트 `judge-e26f6c8e0b84` · `claude-sonnet-5` · 캐싱 꺼짐. 사이클 4 에 측정 3 의 조건이
> **여섯 판**으로 갈렸으므로 다른 판의 수치와 나란히 놓지 마라
> ([status.md 의 판 표](docs/tracking/status.md)).
>
> **J11 의 원인은 단정할 수 없다 — 사이클 4 가 두 가설을 각각 반증했다.**
> ① **다리 조항 부재**: 제품 실행 경로가 실제로 채택하는 다리 조항(`policy:refund:2-5`)을
> 픽스처의 **비인용 근거**로 넣었는데 **0/3** 이었다
> ([결정 0016](docs/tracking/decisions/0016-J11-판정-픽스처를-제품-실행-경로의-근거-집합에-맞춘다.md)).
> ② **비지지 claim 공존**: "두 사유는 독립으로 판정한다"는 **사안 비특정** 프롬프트 강화를
> 넣었더니 **3회 중 1회만** 닫혀 사전 판정선(≥2/3) 미달 → **원복**
> ([결정 0017](docs/tracking/decisions/0017-사유-독립-명시를-실측으로-반증하고-판정-프롬프트를-원복한다.md)).
> ③ **모델 등급으로도 안 닫힌다**: haiku 0/3 · opus 0/3
> ([결정 0018](docs/tracking/decisions/0018-판정-모델-Anthropic-3종을-비교만-하고-기본값은-바꾸지-않는다.md)).
>
> **지금 말할 수 있는 것은 관측뿐이다** — 커밋된 라이브 측정 3 **31세트 중 30세트**에서
> `reject[unsupported_claim]` 단독이고, 유일한 예외가 원복된 프롬프트 판의 `-16` 한 회다.
> **반증 두 겹이 이 꼭지의 결말이고**, 남은 것은 입력 판본을 가르는 픽스처 2건(평가 장치
> 확장이라 별도 승인 사안 — 넣으면 측정 3 의 분모 11 이 바뀐다)이거나 호출 구조 분리다.

### 사이클 3 실측값 — 검색 실험 조건 (되돌린 조건이다)

실행 조건: **2026-08-13~14, L2 켜짐, 3회 반복** · **질의 재작성 켜짐 · 유사도 임계값 0.5** ·
top k 5 · 나머지는 사이클 2 와 같다. 산출물 `reports/evaluation-live-l2-{4,5,6}.{md,json}`.

**이 표를 읽기 전에 세 가지를 알아야 한다.** ① 임계값 0.5 는 **되돌렸으므로 현재 기본값이
아니다**(현재 0.3). 특히 판정 토큰 절감분은 그 조건의 값이다. ② 미끼 라벨 재정렬
([결정 0008](docs/tracking/decisions/0008-미끼-라벨을-판정-규칙에-정렬한다.md)) 뒤의 첫
라이브이고, ③ 사이클 1·2 수치에 영향을 줬을 수 있는 벡터 인덱스 결함을 제거한 뒤의 첫
기준선이다. **그래서 사이클 2 와 직접 비교하지 않는다** — 아래 토큰 비교는 조건 차이를
명시한 참고값이다.

| 지표 | 1회차 | 2회차 | 3회차 | 합산 |
|---|---|---|---|---|
| 허용 결과 집합 대비 일치율 | 96.7% (29/30) | 96.7% (29/30) | 96.7% (29/30) | **96.7% (87/90)** |
| 어긋난 케이스 | G04 | G04 | G04 | **3회 모두 같은 1건** |
| 미끼 문의의 기각 재현율 | 20.0% (1/5) | 0.0% (0/5) | 20.0% (1/5) | **13.3% (2/15)** |
| 지연 p50 | 10,418 ms | 9,137 ms | 9,691 ms | — |
| 지연 p95 | 23,248 ms | 22,781 ms | 20,930 ms | — |
| 건당 토큰 — 생성 | 1,060.3 | 1,060.3 | 1,152.4 | — |
| 건당 토큰 — 임베딩 | 26.5 | 27.6 | 25.1 | — |
| 건당 토큰 — 판정 | 2,564.6 | 2,733.6 | 2,819.1 | — |
| 건당 토큰 — **검색**(신설 계열) | 141.0 | 148.0 | 133.8 | — |
| 건당 토큰 — 합계 | 3,792.4 | 3,969.5 | 4,130.5 | — |
| 검색 폴백 | 0건 | 0건 | 0건 | **0건** |

**측정 1·3 은 무변경이다** — 3회 모두 100.0% / 0.0%, 측정 3 은 어긋나는 픽스처(J11)와
어긋나는 방식까지 사이클 2 와 같다. 검색을 바꿔도 두 층의 단위 정확도는 움직이지 않는다.
(**이 동일성은 판정 층 입력이 같던 시절의 것이다** — 사이클 4 가 판정 픽스처를 교정하면서
`-13` 이후는 다른 판이 됐고, **입력이 달라진 뒤의 동일성은 재현이 아니다.** 그래서 측정 3 은
회귀 가드의 무변경 검사 대상이 아니고 병기만 한다.)

**토큰 계열이 셋에서 넷이 됐다.** 재작성 호출은 생성·임베딩·판정 어디에도 합산하지 않는다.
3회 합산으로 사이클 2 와 견주면(조건이 다르다 — 위 ①②③) 생성 126,217 → 98,192,
판정 **315,822 → 243,518(-22.9%)**, 임베딩 1,342 → 2,379(재작성 질의도 임베딩하므로 늘었다),
검색 0 → 12,683(신설)이다. **검증 비용을 줄인 것은 게이트가 아니라 그 앞에서 기권한 검색이다.**

**그리고 이 표가 못 보여주는 것이 이 사이클의 교훈이다.** 일치율은 세 지표 중 가장 높은데,
그 뒤에서 정상 문의 G04 가 3회 모두 죽었고(합산에서는 3.3%p), G18·G01 의 모순 기각 소멸은
**일치율에도 비악화 가드에도 전혀 잡히지 않았다** — 라벨상 `answered` 도 정답이기 때문이다.
잡은 것은 사이클 3 이 새로 넣은 케이스별 귀인 절 하나였다. 자세한 것은
[검색 품질 절](#검색-품질--게이트-앞의-층)과
[findings 18](docs/tracking/findings.md)에 있다.

### 사이클 4 실측값 — 기권 게이트 배선 전후 (**현재 기본값의 실측은 아래 사이클 5 절이다**)

**두 묶음을 나란히 싣는다.** 둘은 **기권 게이트 두 항목만 다르고** 나머지 지문 16개는 전부
같다 — 그래서 차이의 귀인이 성립한다. 조건: 2026-08-19 · L2 켜짐 · 재작성 켜짐 · **컷 0.30** ·
top k 5 · 생성 `gpt-5.6-terra` · 임베딩 `text-embedding-3-small` 1536 · 판정
`claude-sonnet-5` · 판정 프롬프트 캐싱 꺼짐.

| 지표 (3회 합) | **T2 — 게이트 미배선** (당시 승격 기준선) | **T10 — 게이트 τ=0.06** |
|---|---|---|
| 산출물 | `evaluation-live-l2-{7,8,9}` | `evaluation-live-l2-{10,11,12}` |
| 허용 결과 집합 대비 일치율 | **100.0% (90/90)** | **100.0% (90/90)** |
| 미끼 문의 기각 재현율 | 6.7% (1/15) | 13.3% (2/15) |
| 지연 p50 (3회) | 12.1 ~ 12.9초 | 9.2 ~ 11.0초 |
| 지연 p95 (3회) | 22.4 ~ 42.1초 | 25.4 ~ 47.1초 |
| 총 시도 | 97 | **76** |
| 초안 없이 끝난 문의 | 12 | **24** (그중 `no_evidence` 12) |
| 검색 폴백 | 0건 | 0건 |
| 측정 1 (검출률 / 오탐률) | 100.0% / 0.0% | 100.0% / 0.0% |
| 측정 3 (검출률 / 오탐률) | 100.0% / 3회 중 1회 20.0% | 100.0% / 3회 중 1회 20.0% |

**p50 이 내려간 것은 최적화가 아니다** — 무근거 문의 회당 4건이 검색 단계에서 끝나 초안·판정을
아예 안 부른 것이다. **p95 는 내려가지 않았다**: 꼬리를 만드는 것은 무근거 문의가 아니라
**재생성**이다. 새 축 90건 중 시도 2회를 탄 것이 10건(13.5~49.1초)이고, **전체 상위 6 지연
(36.7~49.1초)이 전부 그 안에 있다** — 7위는 26.4초(시도 1회)다. 30건 규모에서
p95 는 사실상 상위 2건이라 같은 조건 3회에서도 25.4 → 47.1초로 흔들린다.

**측정 3 의 오탐 20.0% 는 J04 한 건이고 3회 중 1회다**(`-8` 과 `-10`). J04 는 판정 계열
`-1` … `-12` 에서 산발적으로 흔들리던 픽스처다.

**측정 2 일치율 100.0% 를 "풀렸다"로 읽으면 안 된다.** 결정 0008 의 라벨 재정렬 이후 이
지표는 **하한 경보선**이고, 사이클 3 의 가장 큰 회귀(G18 모순 기각 소멸)는 합산 96.7% 뒤에서
일어났다. **회귀는 케이스 단위가 잡는다**([회귀 가드](#회귀-가드--실행마다-두-줄로-보고한다)).

### 사이클 5 실측값 — 현재 기본값이자 승격 기준선이다

산출물 `evaluation-live-l2-{28,29,30}`(3회 세트) + `-31`(재등재 닫힘 확인 1회).
조건: 2026-08-21 · L2 켜짐 · 재작성 켜짐 · 컷 0.30 · 기권 게이트 τ=0.06 · top k 5 ·
생성 `gpt-5.6-terra` · 임베딩 `text-embedding-3-small` 1536 · 판정 `claude-sonnet-5` ·
판정 픽스처 `fixture-2b8f33392b36` · 캐싱 꺼짐 · **조건 지문 25칸**.
사이클 4 세트와의 차이는 판정 픽스처 판 하나가 아니다 — **L1 접기·L1 채점표·조회 가드
유래와 캐스트 타입·검색 정렬 tie-break·기권 미정의 처분**이 함께 바뀐 뒤의 실행이다.

| 지표 (3회) | 1회 (`-28`) | 2회 (`-29`) | 3회 (`-30`) | 합산 |
|---|---|---|---|---|
| 허용 결과 집합 대비 일치율 | 30/30 | 30/30 | 30/30 | **100.0% (90/90)** |
| 측정 1 검출률 / 오탐률 (**분모 35**) | 100.0% (26/26) / 0.0% (0/9) | 같음 | 같음 | 세 회차 동일 |
| 측정 3 검출률 / 오탐률 | 100.0% (6/6) / 0.0% (0/5) | 같음 | 같음 | 세 회차 동일 |
| 미끼 문의 기각 재현율 | 0/5 | 1/5 | 1/5 | **13.3% (2/15)** |
| 지연 p50 / p95 (ms) | 9,516 / 35,684 | 8,697 / 40,254 | 9,585 / 34,278 | — |
| 총 시도 | — | — | — | **73** |
| 초안 없이 끝난 문의 (90 중) | — | — | — | **24** (그중 `no_evidence` 12) |
| 인계 사유 (회당) | `no_evidence` 4 · `missing_order_ref` 2 · `order_not_found` 2 | 같음 | 같음 | `rejected_twice` **0** |
| 검색 폴백 | 0건 | 0건 | 0건 | **0건** |

**확인 라이브 `-31` 은 위 표에 넣지 않는다 — 3회 세트가 아니라 1회 실행이다.** 그 회차의
값은 이렇다(1회, 분모 30): 일치율 **96.7% (29/30)** — 어긋난 한 건이 G06 이고 그것이 아래
"재등재와 확인 라이브" 절의 미달 그 자체다. 측정 1 **100.0% (26/26) / 0.0% (0/9)**,
측정 3 **100.0% (6/6) / 0.0% (0/5)**, 미끼 기각 재현 **0/5**, 총 시도 **23**, 초안 없이
끝난 문의 **9**, 검색 폴백 **0건**, 인계 사유 `no_evidence` 4 · `missing_order_ref` 3 ·
`order_not_found` 2. **지연은 위 3회 세트의 게시 대역 밖이다** — p50 **10,608 ms** 가
9,516 ms 위, p95 **23,305 ms** 가 34,278 ms 아래다. 1회 관측이라 대역을 다시 그리지 않고
그대로 병기한다.

**측정 1 은 무과금 사전 대조가 예측한 값과 정확히 같다.** 채점표를 27 → 35 로 넓히면서 심어
둔 27건의 판정 변화 0 · 오탐 증가 0 을 오프라인으로 확인했고 라이브 세 회차가 그대로 나왔다.
**분모를 병기하지 않고 앞 사이클의 19/19·0/8 과 나란히 놓으면 안 된다.**

#### 단계별 지연 — 이 저장소가 처음 재는 값이다

세 회차 평균의 범위다. **분모는 그 구간이 실제로 돈 케이스**이고 미측정은 0 이 아니라 분모에서
빠진다(회당 30건 중).

| 구간 | 평균 (3회 범위) | 측정 / 미측정 |
|---|---:|---|
| **L2 판정** | **8,448 ~ 10,145 ms** | 22 / 8 |
| 초안 생성 | 2,098 ~ 2,464 ms | 22 / 8 |
| 조회문 생성 | 1,616 ~ 1,964 ms | 9 / 21 |
| 질의 재작성 | 1,433 ~ 1,681 ms | 19~21 / 9~11 |
| 의도 분류 | 1,373 ~ 1,559 ms | 30 / 0 |
| 질의 임베딩 | 249 ~ 363 ms | 19~21 / 9~11 |
| 벡터 검색 | 26.9 ~ 31.2 ms | 19~21 / 9~11 |
| 조회 실행 (존재성 선검사 포함) | 7.0 ~ 8.4 ms | 11 / 19 |
| **L1 게이트** | **0.467 ~ 0.638 ms** | 22 / 8 |

**이 표가 이 저장소의 주장을 숫자로 만든다.** 회차별 문의 지연 총합 대비 **L2 판정 한 층이
55.8% · 56.5% · 60.3%** 다. 그 반대편에서 **결정론 게이트는 0.5 ms** — L1 이 싼 것은 설계
의도였고 이제 실측이다. DB 조회(7~8 ms)와 벡터 검색(27~31 ms)도 같은 자리에 있다.

**그리고 지연은 사실상 전부 밖으로 나가는 호출이다** — 여섯 구간(의도 분류 · 재작성 ·
임베딩 · 조회문 생성 · 초안 · 판정)의 합이 문의 지연 총합의 **99.8%**(세 회차 전부)다.
남은 0.2% 가 DB 조회 · 벡터 검색 · L1 게이트 · 그 사이의 파이썬 시간 전부다.
**최적화 주장이 아니다** — 첫 실측이라 비교할 대상이 없다.

**미측정과 0 을 가르는 것이 이 계측의 계약이다.** 재작성을 쓰지 않은 문의의 재작성 구간,
정책 경로로 간 문의의 조회문 생성 구간은 `null`(미측정)이지 0 이 아니다. 조회 실행 분모(11)가
조회문 생성 분모(9)보다 큰 것은 **존재성 선검사도 조회 실행 구간에 들어가기 때문**이다 —
빼면 `order_not_found` 문의의 DB 시간이 어디에도 안 잡힌다.
**계약·스키마는 한 칸도 바뀌지 않았다** — 구간 값은 평가 리포트에만 있고 HTTP 응답에도
처리 기록 테이블에도 나가지 않는다.

#### 캐시 적중 토큰 — 생성·검색 계열이 처음 실렸다

| 계열 | write (3회 합) | read (3회 합) |
|---|---:|---:|
| 생성 | **7,499** (1,059 · 3,152 · 3,288) | **0** |
| 검색 (질의 재작성) | 0 | 0 |

**read 가 0 이라는 것이 이 표의 결론이다** — 이 세트의 생성·검색 달러에는 **캐시가 은폐한
할인이 없다.** 그전까지 이 계열의 달러는 전부 "상한"이었다(적중분을 읽지 않아 전부 정가로
환산). **캐시 배선이 준 것은 더 싼 값이 아니라 "은폐가 없다는 증거"다.**

#### 재등재와 확인 라이브 — 두 사실을 함께 적는다

사용자가 `-28`·`-29`·`-30` 을 **승격 기준선으로 등재**했고(2026-08-21), 등재 **뒤에** 확인
라이브 1회(`-31`)를 더 샀다. 등재 전에 돌리면 자기 대조가 되기 때문이다.

- **재등재는 닫혔다.** 구속 줄이 **선언 없이 대조를 진행했고 어긋난 항목이 비었다** —
  지문 25칸 완전 일치, 미상 0, 선언된 차이 0. 직전까지 매 실행 실려 있던 어긋남 셋이 사라졌다.
- **그 회차의 구속 줄 판정은 미달이다.** 케이스 **G06** 이 기준선 3/3 → 이번 0/1 이다.
  귀인이 이번 사이클을 가리키지 않는다 — `-30`↔`-31` 사이 **제품 코드 변경 0 줄**, G06 은
  커밋된 풀셋 산출물 19개 중 앞의 18개에서 `answered` 였고 이번이 첫 이탈, 이탈 지점은
  **의도 분류**(`policy` → `both` 로 가서 주문번호가 없다는 이유로 `missing_order_ref`)다.
  사용자 판단은 **"그대로 받고 정직하게 기록한다"** 였고 재구매도 가드 수정도 하지 않았다.

**비용** (3회 합, 단가 기준일 2026-08-19): 생성 $0.331392 · 임베딩 $0.000047 ·
판정 $0.935250 · 검색 $0.037694 · **합계 $1.304383**, 문의 1건당 **$0.014493**.
측정 3 까지 더한 실제 결제액은 3회 합 **$1.656935**, 확인 라이브 1회가 **$0.517549** 다.

> 골든셋 · L1 픽스처 · **판정 픽스처** 데이터와 평가 하네스 실행 스크립트는 별도 산출물이다.
> 처리 기록 4테이블이 지표 원천(지연 · 생성/판정 토큰 · 인계 사유 · 시도별 층별 판정)을 이미 전부 들고 있어, 하네스는 그 위에 얹힌다.

---

## 비용 — 계열마다 단가가 다르다

**규칙 둘을 먼저 적는다.** ① **기준일 없는 달러 수치는 싣지 않는다** — 단가는 바뀌고, 출처
없는 숫자는 다음 사람이 검증할 수 없다. ② **토큰 수는 비용비가 아니다** — 계열마다 provider 와
단가가 다르고 같은 모델 안에서도 출력이 입력의 5~6배다.

단가와 계열별 분해의 정본은 **[`docs/tracking/pricing.md`](docs/tracking/pricing.md)** 이고
**기준일은 2026-08-19** 다(Anthropic·OpenAI 벤더 페이지를 그날 조회한 값).
아래 수치는 전부 커밋된 라이브 리포트에서 재집계한 것이고 **새 실측을 사지 않았다.**

### 계열별 분해 — 현재 기본값 라이브 3회 (문의 90건, `evaluation-live-l2-{28,29,30}`)

| 계열 | 모델 | 토큰 | 토큰 점유율 | 달러 | **달러 점유율** |
|---|---|---:|---:|---:|---:|
| 생성 (의도 해석 + SQL 생성 + 초안) | `gpt-5.6-terra` | 105,606 | 26.8% | $0.331392 | 25.41% |
| 임베딩 (문의·재작성 질의) | `text-embedding-3-small` | 2,346 | 0.6% | $0.000047 | **0.004%** |
| **판정 (L2)** | `claude-sonnet-5` | **273,329** | **69.4%** | **$0.935250** | **71.70%** |
| 검색 (질의 재작성) | `gpt-5.6-terra` | 12,672 | 3.2% | $0.037694 | 2.89% |
| **합계** | | **393,953** | 100% | **$1.304383** | 100% |

회당 $0.434794 · **문의 1건당 $0.014493** (단가 기준일 2026-08-19).

**이 표는 생성·검색 계열이 상한이 아닌 첫 표다.** 세 회차 모두 캐시 read 가 0 이라
적중이 은폐한 할인이 없다(캐시 write 는 생성 계열 7,499 토큰이고 그 단가만 "출처 미확인"이다).
앞 사이클의 같은 표는 캐시 배선 이전이라 영구히 상한이며, 그 값은 지우지 않고
[단가 문서](docs/tracking/pricing.md) 2절에 상한으로 병기해 두었다.

**점유율이 계열마다 어긋나는 것이 이 표의 요지다.** 임베딩은 토큰의 0.6% 인데 비용의
**0.004%** — 반올림 전 값(0.5955% · 0.003597%)으로 재면 **165.6배** 어긋난다. 판정은 토큰
69.4% 에 비용 71.7% 로, 출력 단가가 입력의 5배라
출력이 많은 계열이 비용 쪽으로 밀린다. **절대액(3회 $1.30)은 이 규모에서 의사결정 요인이
아니다** — 이 표의 값은 비율과 방법에 있다.
(상한 기준 옛 값: 사이클 4 새 축 3회 = 토큰 400,413 · $1.333722 · 건당 $0.014819 ·
임베딩 배수 166.5 · 판정 비용 점유율 72.3%.)

### 검색이 먼저 거르면 얼마가 절약되는가 — **조건을 반드시 병기한다**

| 조건 | 대조군 | 판정 토큰 | **판정 달러** | 비고 |
|---|---|---|---|---|
| **컷 0.50 · 재작성**(사이클 3 T10 — **그 구성은 원복됐다**) | 사이클 2 (컷 0.30 · 재작성 없음) | **−22.9%** | **−27.3%** | G04 의 정답 조항과 G18·G01 의 상충 조항을 함께 자른 대가를 치른 구성이다 |
| **기권 게이트 τ=0.06 · 컷 0.30**(사이클 4) | 옛 승격 기준선 (같은 조건 · 게이트만 미배선) | **−12.8%** | **−13.0%** | 대가 없이 얻었다 |

**두 수치는 전부 "캐시 미반영 상한 기준"이고 단가 기준일은 2026-08-19 다.** 네 묶음이 모두
캐시 배선 이전 실행이라 같은 자로 잰 상한끼리의 비교이고 변화율의 부호는 살아 있다.
**새 실측으로 다시 계산하지 않았다 — 계산할 수가 없다**: 두 비교의 대조군이 캐시 배선 이전
세트라 한쪽만 새 값으로 갈아 끼우면 상한을 실측으로 나눈 비율이 된다
([단가 문서](docs/tracking/pricing.md)의 "상한과 실측을 가른다").

**두 수치는 같은 자로 잰 값이 아니다** — 대조군이 다르고 컷이 다르다. **새 축은 절반쯤의
절감을 그 대가 없이 얻은 것**이지 같은 축에서 절반으로 떨어진 것이 아니다.

**그리고 −22.9% 를 "비용 −22.9%" 로 옮겨 적으면 틀린다.** 같은 변화의 판정 달러는 **−27.3%**
다 — 입력이 −20.6% 인데 출력이 **−33.5%** 이고 출력 단가가 입력의 5배이기 때문이다.
이 4.4%p 가 이 문서가 존재하는 이유다.

**L1 선기각의 절감과 검색 선필터의 절감을 더하면 이중 계산이다.** 두 절감은 같은 문의를 두고
경쟁한다 — 기준선에서 L1 이 12번 걸러 낸 G21–G24 가 새 축에서는 검색 단계에서 끝난다.

### 캐싱 — 실측하고 채택하지 않았다

판정 호출의 **고정 프리픽스**(판정 지침, 2,110 토큰 — `claude-sonnet-5` 의 최소 캐시 길이
1,024 를 넘는다)에 프롬프트 캐싱을 걸고 전후를 실측했다.

| 토큰 계열 (3회 합) | 전 (캐싱 꺼짐) | 후 (캐싱 켜짐) |
|---|---:|---:|
| 일반 입력 (비캐시) | 90,126 | **20,496** |
| 캐시 write | 미측정 (배선 전) | **0** |
| 캐시 read | 미측정 (배선 전) | **69,630** |
| **총 프롬프트 토큰** | **90,126** | **90,126** |
| 출력 | 17,439 | 17,796 |
| **판정 달러 총계** | **$0.354642** | **$0.232878** |

**총 프롬프트 토큰이 전후 정확히 같다 — 회당 30,042 로 한 토큰도 다르지 않다.** 캐싱이 한
일은 그중 **23,210 토큰(77.3%)을 정가 입력($2.00/M)에서 캐시 read 단가($0.20/M)로 옮긴 것**
이다. 프롬프트 비용은 −69.5%, 총 판정 비용은 −34.3%(출력은 캐싱과 무관하다).

**"−69.5%" 를 조건 없이 인용하면 거짓이다.** 그 값은 **연속 실행 · 캐시 write 0** 조건이다 —
세 세트가 5분 TTL 안에서 연달아 돌아 엔트리를 다시 쓰지 않았다. 간격이 5분을 넘는 산발
실행이면 세트마다 콜드 write 2,110 토큰이 붙어 **−61.5%** 다.

**배선이 먼저였던 이유가 이 절의 핵심이다.** 캐싱을 켜면 Anthropic 의 `usage.input_tokens` 는
**캐시 적중분을 제외한** 값이 된다. 배선 없이 켰다면 리포트에 *"판정 입력 토큰 30,042 →
6,832"* 만 남았을 것이고, 그 한 줄이 이 사이클에서 가장 위험한 거짓말이었다. **줄어든 것은
단가이지 토큰이 아니다.**

**처분: 미채택 — 스위치는 꺼진 채로, 캐시 계열 토큰 수집 배선만 남긴다.** 사전 확정한 집계
지표 조건(검출률·오탐률·사유 완전일치 무변화)이 미달이었다(오탐률 0.0% → 6.7%, 사유 완전일치
90.9% → 87.9% — **`-25` 의 J04 한 칸**이 전부다). **비용이 좋다고 사전 확정한 선을 옮기면
사전 확정의 의미가 사라진다.** 전문은
[결정 0019](docs/tracking/decisions/0019-판정-프롬프트-캐싱을-실측하고-채택하지-않는다.md).

### 이 절이 말하지 않는 것

- **`-27` 까지의 생성·검색 계열 달러는 전부 상한이다.** 그 실행들의 OpenAI 래퍼가 캐시
  적중분(`cached_tokens`)을 읽지 않아, 그중 얼마가 캐시 단가로 과금됐는지 이 저장소는 모른다 —
  실제 청구액은 그 값보다 같거나 적다. **사이클 5 가 그 배선을 넣었고**(계열마다 write/read
  한 쌍) `-28` 이후 세트는 read 0 이 실측으로 확인된다. 다만 **캐시 write 단가 한 칸은 아직
  "출처 미확인"** 이라 write 토큰이 정가로 환산돼 있다 — 방향을 모르므로 그 칸까지 "상한"이라고
  적지 않는다.
- **한 계열의 값을 다른 계열의 환산식으로 읽으면 안 된다.** OpenAI 는 캐시 적중분이
  `input_tokens` **안**에 있고 Anthropic 은 **밖**이다. 모르고 더하면 이중 계산이다.
- **단가는 바뀐다.** 위 수치를 다른 기준일에 인용하려면 [단가 표](docs/tracking/pricing.md)를
  먼저 다시 확인한다. 특히 Sonnet 5 행은 도입가 → 정가 전환이 최근에 있었던 자리다.

---

## 실행 방법

### 0. 전제

- Python 3.13(`.python-version`), [uv](https://docs.astral.sh/uv/), Docker
- **`OPENAI_API_KEY`** — 생성·임베딩 공통으로 **1개**만 쓴다
- **`ANTHROPIC_API_KEY`** — L2 판정 전용. **L2 가 켜져 있는데 이 키가 없으면 `POST /inquiries`
  는 503** 이고, 선검사가 처리 진입 시점에 돌아 생성 토큰을 태우기 전에 끝난다.
  `L2_ENABLED=false` 로 끄면 이 키는 필요 없다

```bash
uv sync
cp .env.example .env   # 값을 채운다 (.env 는 커밋되지 않는다)
```

`.env` 에 반드시 채워야 하는 것: `OPENAI_API_KEY`, **`ANTHROPIC_API_KEY`**(L2 를 끄지 않는다면),
`POSTGRES_SUPERUSER_PASSWORD`, `POSTGRES_APP_PASSWORD`, `POSTGRES_RO_PASSWORD`.
나머지는 기본값이 있다(`L2_ENABLED` 기본 `true` · `JUDGE_MODEL` 기본 `claude-sonnet-5`).

### 1. DB 기동

```bash
docker compose up -d --wait
```

Postgres 17 + pgvector 컨테이너 1개가 호스트 **5433** 포트에 뜬다(로컬 Postgres 5432 와 충돌 회피).
최초 기동 시 `db/init/` 이 pgvector 확장과 계정 2개(앱 · read-only)를 만든다.

> `docker compose up` 이 컨테이너를 재생성할 수 있지만 데이터는 named volume(`reply-gate-pgdata`)에 남는다.
> 스키마를 바꿨으면 `docker compose down -v && docker compose up -d --wait` 로 볼륨째 재생성한다.

### 2. 스키마 + 주문 시딩 — **API 키 불필요**

```bash
uv run python -m scripts.seed_orders
# 주문 시딩 완료: 500건
```

`db/schema.sql` 을 적용(멱등)하고 커밋된 픽스처(`db/fixtures/orders.jsonl`)의 합성 주문 500건을 upsert 한다.
시딩 중에 데이터를 새로 만들지 않으므로 몇 번을 돌려도 같은 500건이다. 실존 인물의 데이터는 쓰지 않는다.

### 3. 정책 인덱싱 — **API 키 필요** (임베딩 호출)

```bash
uv run python -m scripts.index_policies
# 정책 조항 적재 완료: 26건 갱신, 0건 삭제, 임베딩 토큰 2577
```

`data/policies/` 의 문서 4개를 **조항 단위**로 쪼개 임베딩하고 pgvector 에 적재한다(조항 26개).
청크 1개 = 조항 1개 = 근거 1개이고, 근거 ID 는 `policy:<문서 slug>:<조항 번호>` 다.
재실행하면 갱신하고, 문서에서 사라진 조항은 지운다. 키가 없으면 호출하지 않고 종료 코드 2로 끝난다.

### 4. 서버 기동

```bash
uv run uvicorn reply_gate.api:app --reload
```

| 엔드포인트 | 역할 | API 키 |
|---|---|---|
| `GET /` | 웹 폼 1장 (외부 리소스를 하나도 불러오지 않는다) | 불필요 |
| `GET /health` | 헬스 체크 | 불필요 |
| `POST /inquiries` | 문의 접수 + 동기 처리 | **필요** (L2 가 켜져 있으면 판정 키까지) |
| `GET /inquiries/{id}` | 처리 기록 조회 (저장된 값에서 같은 응답 골격을 재구성) | 불필요 |

```bash
curl -s http://127.0.0.1:8000/health
# {"status":"ok"}

# 인계 경로 — 존재하지 않는 주문번호
curl -s -X POST http://127.0.0.1:8000/inquiries \
  -H 'Content-Type: application/json' \
  -d '{"content": "이 주문 언제 배송되나요?", "order_no": "ORD-20260101-9999"}'
# {"inquiry_id":"...","status":"escalated", ... ,"escalation_reason":"order_not_found", ... }

# 접수 거부 — 주문번호 형식 오류는 파이프라인에 들어가지 않는다
curl -s -o /dev/null -w '%{http_code}\n' -X POST http://127.0.0.1:8000/inquiries \
  -H 'Content-Type: application/json' \
  -d '{"content": "배송 문의", "order_no": "1234"}'
# 422
```

> 자격 증명이 없는 상태에서 LLM 이 필요한 경로를 부르면 **503**(설정 오류)으로 끝난다.
> 인계 사유(`llm_call_failed`)로 기록하지 않는다 — 키를 안 넣고 돌린 실행이 평가 지표에 섞이면 안 되기 때문이다.

### 5. 검증 — **API 키 불필요**

```bash
uv run pytest              # 1,430 passed
uv run ruff check .        # 0
uv run ruff format --check .   # 182 files
uv run mypy                # strict — 75 files, 0 errors
```

> **`uv run pytest` 는 DB 기동을 전제한다.** `db` 마커가 붙은 통합 테스트(176건)는
> Postgres 에 접속되지 않으면 **사유를 담아 skip** 된다 — 조용히 늘 skip 되는 테스트는 검증이 아니므로,
> skip 사유에 접속 대상 · 원인 · 복구 명령(`docker compose up -d --wait`)이 함께 실린다.
> **전체 녹색을 주장하려면 `docker compose up -d --wait` 를 먼저 실행해야 한다.**

```bash
uv run pytest -m db        # DB 통합 테스트만 — 176 passed, 1,254 deselected
```

DB 통합 테스트는 세션당 1회 스키마 적용 + 주문 시딩을 하고, 각 테스트가 쓴 행은 롤백으로 되돌린다.
실제 외부 API 를 호출하는 테스트는 없다 — 과금 실행은 평가 하네스의 `--live` 가 유일한 입구다.

### 6. 평가 지표 산출

```bash
uv run python -m scripts.evaluate          # 측정 1 (L1 픽스처) — API 키 불필요
uv run python -m scripts.evaluate --live   # 측정 1 + 2(골든셋 30건) + 3(판정 픽스처 11건) — API 키 필요 · 과금
```

리포트는 `.md`(사람용) 와 `.json`(기계용) 으로 나온다. 파일 이름은 **과금 실행 여부**로
갈린다 — 과금 측정이 실제로 돈 실행은 `reports/evaluation-live*.*`, 나머지는
`reports/evaluation.*` 다(규칙 전문은 [operations.md](docs/operations.md)).
실측값은 [사이클 2 실측값](#사이클-2-실측값) · [사이클 3 실측값](#사이클-3-실측값--검색-실험-조건-되돌린-조건이다) ·
[사이클 5 실측값](#사이클-5-실측값--현재-기본값이자-승격-기준선이다) 절에 있다.

**세 장면을 실제로 띄워 돌리는 절차**는 [operations.md](docs/operations.md) 의 시연 절에
있다 — 색인 확인 → 서버 기동 → 세 장면 → `GET /inquiries/{id}` 왕복 → 좀비 확인 순서다.

**검색 전략 비교는 별도 하네스다** — 파이프라인을 태우지 않고 검색 단계만 잰다.

```bash
uv run python -m scripts.compare_retrieval                 # 결정론 대역 — 배관 검증 전용, 실제 수치 아님
uv run python -m scripts.compare_retrieval --live          # 실제 임베딩·리랭크 호출 · 과금
uv run python -m scripts.compare_retrieval --live --embedding-axis   # 임베딩 모델 축까지 · 과금
```

기본 재작성 입력은 **blind 픽스처**이고, `--oracle-rewrite` 를 명시해야 상한 대조군을 돌린다
(선언과 파일 경로가 어긋나면 실행 자체를 거부한다). 리포트는 `reports/retrieval-strategies-*`
로 나가며 파일 이름에 임베딩 모델·차원·재작성 조건·top_k·컷이 박힌다.

---

## 한 것과 하지 않은 것

하지 않은 것을 하지 않았다고 적는 것이 신뢰를 만든다.

### 한 것

사이클 1(L1 까지)에서 한 것:

- 결정론 게이트 **L1** — 사유 4종, LLM 0회, 100% 재현
- **PII allowlist** — 근거 유래 값만 허용, 표기 형식 차이 흡수
- **text-to-SQL 안전장치** — 계정 분리 · 스키마 화이트리스트 · AST 쿼리 검증 · 주문 1건 한정 · LIMIT/타임아웃 강제
- **자체 에이전트 루프** — 재생성 1회 상한을 코드와 DB CHECK 가 함께 강제, 인계 6종
- **RAG 기본** — 정책 문서 조항 단위 청킹 + pgvector 코사인 검색 + 유사도 임계값 필터
- **처리 기록 4테이블** — 시도별 판정 · 초안 원문 · 근거 스냅샷(실행된 쿼리문 + 결과 행 전체) · SQL 실패 내역
- **FastAPI 엔드포인트 4개 + 판정이 먼저 보이는 웹 폼**

**사이클 2(L2)에서 더한 것:**

- **L2 판정 층** — L1 통과분만, 시도당 1회 배치 호출, 사유 2종(`unsupported_claim` ·
  `contradictory_evidence`), 생성과 **다른 provider**(Anthropic)로 self-judging bias 차단
- **fail-closed 배선** — 해석되지 않는 판정 산출은 거부하고 1회 재시도, 재실패하면
  `llm_call_failed`(단계 `l2_judge`)로 인계. 검증 못 한 답변은 내보내지 않는다
- **층별 판정 기록·응답** — 시도별로 L1/L2 판정을 따로 저장·노출, claim 단위 판정과
  모순 근거쌍까지. 웹 폼은 `L2 미실행` 배지에 이유를 함께 찍는다
- **판정 토큰 분리 집계** — 생성·임베딩·판정 세 계열을 끝까지 섞지 않는다
- **측정 3 신설** — 판정 픽스처 11건(`data/judge_fixtures.jsonl`)으로 L2 검출률·오탐률을
  측정 1 과 같은 모양으로 잰다
- **스위치** — `L2_ENABLED=false` 로 층 전체를 끌 수 있다(그러면 Anthropic 의존이 사라진다)

**사이클 3(검색 품질)에서 더한 것:**

- **검색 정답 라벨 30건**(`data/retrieval_labels.jsonl`) — 게이트 앞의 층에도 정답을 박아
  검출·오채택을 재현 가능한 숫자로 만든다. 라벨은 채점 단계에서만 쓴다
- **전략 사다리 비교 하네스** — 벡터 / +재작성 / +하이브리드(BM25+RRF) / +LLM 리랭크를 같은
  채택 축(코사인)으로 재고, 컷 스윕 13점·분모 병기·임베딩 모델 축까지 한 리포트에 싣는다
- **질의 재작성 배선** — 원문과 재작성을 **둘 다** 검색해 합집합, 실패하면 원문으로 폴백
  (인계가 아니다). blind 재작성 픽스처와 oracle 상한 픽스처를 분리 보관
- **검색 토큰 분리 집계** — 계열이 셋에서 **넷**이 됐다. 재작성 호출 토큰은 생성·임베딩·판정
  어디에도 합산하지 않고 폴백 건수·사유와 함께 리포트에 남는다
- **케이스별 실패 귀인** — 합산 지표가 가리지 못하도록 "검색이 못 찾았나 / 생성이 틀렸나 /
  답이 없는 게 정답인가"를 케이스마다 분류한다. 이 절이 아니었으면 사이클 3 의 가장 값진
  회귀(모순 기각 소멸)를 아무도 못 봤다
- **라이브 3회 재실측 + 축별 원복** — 표적은 충족했지만 비악화 가드가 미달해 **컷 한 축만**
  되돌렸다. 실측이 정당화하지 않는 기본값을 남기지 않는다

**사이클 4(채택 축 · 회귀 가드 · 판정 심화 · 비용 서사)에서 더한 것:**

- **질의 단위 기권 게이트** — 채택을 **질의 축 → 항목 축** 두 겹으로 나눴다. 컷 하나로는 못
  지키던 세 방향(정답 조항 보존 · 무근거 기권 · 상충쌍 보존)이 라이브 3회에서 **동시에**
  성립한다. 후보 축 둘은 **손계산이 먼저 반증**했고(무과금), 채택 규칙에는 이번에
  **케이스 단위 하한**이 들어갔다
- **회귀 가드 상설화** — 실행마다 **이중 기준선 두 줄**(승격=구속 / 직전 라이브=경보),
  **비악화 두 겹**(일치 감소 + **근거 부분 손실**), 조건 지문 18항목, "검사가 안 돌았으면
  보류". 사이클 3 이 사람 눈으로 잡았던 회귀를 이제 코드가 잡는다
- **케이스 귀인 3분류** — 정책 검색이 아예 안 돈 케이스 / 주문 단계 사전 인계(기대 경로) /
  `generation_issue` 의 최종 상태 병기([findings 20](docs/tracking/findings.md) 종결)
- **단가 표와 비용 분해** — 기준일이 박힌 단가 표(`docs/tracking/pricing.md`)와 계열별 분해.
  **판정이 건당 비용의 72.3%**, 임베딩은 0.003% 다
- **캐시 계열 토큰 배선** — 판정 캐싱은 **미채택**이지만 배선은 남겼다. 없으면 다음에 누가
  캐싱을 켤 때 판정 입력 토큰이 **77% 줄어든 것처럼** 찍힌다
- **비교만 하고 반영하지 않은 것 둘** — 판정 모델 3종(기본값 무변경)과 청킹 전략(조항 단위 유지)
- **반증 둘** — J11 은 픽스처 교정(0/3)과 프롬프트 강화(1/3, 사전 판정선 미달로 원복)로
  **두 겹으로 반증**됐다. 닫지 못한 것을 닫았다고 적지 않는다

**사이클 5(재등재 · 계측 · 마감)에서 더한 것:**

- **승격 기준선 재등재** — 현재 기본값 조합으로 풀셋 라이브 3회를 사서
  `evaluation-live-l2-{28,29,30}` 을 등재했고, **등재 뒤에** 확인 라이브를 한 번 더 사서
  구속 줄이 **선언 없이 대조를 진행하고 어긋난 항목이 비는 것**을 확인했다. 그 회차의
  판정 자체는 **미달(G06)** 이고 그 사실도 그대로 적었다
- **단계별 지연 계측(구간 아홉)** — L2 판정 8.4~10.1초 · 초안 2.1~2.5초 · **L1 게이트 0.5 ms**.
  **미측정은 0 이 아니라 분모에서 빠진다.** HTTP 응답도 DB 스키마도 바뀌지 않았다
- **계열별 캐시 적중 토큰(생성·검색)** — 계열마다 write/read 한 쌍. OpenAI 는 적중분이 입력
  토큰 **안**, Anthropic 은 **밖**이라 환산식이 계열마다 다르다. 입력 칸에 접지 않는다
- **L1 우회 경로 여섯 개를 닫고 채점표를 27 → 35 로 넓혔다** — 번호 계열의 구분자 변종·결합
  표식이 접기를 비껴가던 자리다. **접기는 패턴별**이고 이메일에는 넓히지 않는다(넓히면 반대
  방향 우회가 열린다). 개인정보 정의는 이제 `gate.pii_shaped` **단독 소유**이고 소비자가 셋이다
- **조회 가드 둘** — 유래 추적을 임시 테이블·파생 테이블 안쪽까지 내렸고(정상 조회를 오기각하던
  자리), **캐스트 대상 타입 허용 목록 13종**을 넣었다(검사되지 않던 자리)
- **자격 증명을 비밀 전용 타입으로** — 설정 객체를 통째 덤프해도 평문이 나오지 않는다.
  평문 추출 자리는 **다섯 줄**이고 AST 검사가 그 개수를 못박는다.
  **다만 접속 문자열은 이 조치 밖이다** — `database_url`·`readonly_database_url` 은 값을
  조립한 새 문자열이라 필드 규칙이 걸리지 않고 비밀번호를 그대로 담는다. 실제로 새는지
  재현하는 것이 먼저라 이번 범위에 넣지 않았고, `tests/test_config_secrets.py` 가
  "의도적으로 열려 있다"를 못박는다([findings 24](docs/tracking/findings.md))
- **조건 지문 18 → 25칸** — 넓히면 그 이전 산출물 전부가 새 항목에서 "미상"이 되어 계보가 한 번
  끊긴다. **재등재 시점이 그것을 감당할 유일한 자리**라 같은 사이클에 넣었다
- **띄워서 돌렸다** — 세 장면을 웹 폼으로 흘려 처리 기록을 남기고 조회 라우트로 다시 열었다.
  **재현 확률과 미재현 회차까지 적었다**(위 데모 절)

> **"구조 검사가 못박는다"의 한계를 함께 적는다.** 이 저장소의 구조 검사는 AST 를 읽되
> **이름과 문면의 블록리스트**로 위반을 찾는다 — 실수는 잘 잡지만 **저자가 피하려고 마음먹으면
> 뚫린다.** 예를 들어 판정 층에 `thinking` 을 **다른 이름의 파라미터로** 보내면 조건 지문이
> 그것을 못 잡고, 개인정보 패턴을 세 가지 알려진 형태(패턴 객체 생성 · 정본 이름 재정의 ·
> 정규화 라이브러리 import)가 아닌 방식으로 다시 만들면 소유권 검사가 비껴간다. 의미 기반
> 검사로 바꾸는 것은 가드 층 재설계이고 **이 프로젝트의 범위 밖으로 확정했다.**
> 문서에 적힌 위반은 전부 잡는다는 것이 지금 말할 수 있는 전부다.

### 하지 않은 것 (범위 밖으로 확정한 것)

**이 프로젝트는 사이클 5 로 닫힌다.** 그래서 아래 표의 "왜 지금 안 했나"는 대부분
**"다음에 한다"가 아니라 "하지 않기로 정했다"** 이다 — 남은 항목의 두 갈래 분류와 각각의
사유는 [미해결 목록](docs/tracking/findings.md)에 있다.

| | 왜 지금 안 했나 |
|---|---|
| **측정 3 목표치** | 미룬 것이 아니라 **두지 않기로 확정했다**([결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md)) — 픽스처 11건이라 한 건이 9.1%p 이고, 실측값 근처에 목표를 박으면 목표가 아니라 현황 기술이 된다. 회귀는 **같은 판 안의** 실측값 비교로 잡는다. |
| **모호 조항 검출** | 심어 둔 모호 3개(“상당한 기간” 등)를 겨냥한 전용 측정이 아직 없다. 상충 2개는 L2 의 `contradictory_evidence` 로 실제 발화했다. |
| **비패턴형 PII**(이름 · 주소) | 정규식으로 잡을 수 없고 **L2 의 대상도 아니다** — L2 가 보는 것은 근거-claim 정합뿐이다. 별도 층이 필요하다. |
| ~~**단계별 지연 계측**~~ | **사이클 5 가 닫았다** — 구간 아홉을 리포트까지 실었다([그 실측](#사이클-5-실측값--현재-기본값이자-승격-기준선이다)). 종료 지점별 차집합으로 추정하던 자리가 실값이 됐고, **L2 판정 한 층이 지연의 절반을 넘는다**는 것이 확인됐다. |
| ~~**승격 기준선 재등재**~~ | **사이클 5 가 닫았다** — 라이브 3회를 새로 사서 `-28`·`-29`·`-30` 을 사람이 등재했고, 확인 라이브에서 지문 25칸이 완전히 일치해 **어긋난 항목이 비었다**. 그 회차의 판정은 미달(G06)이고 그 사실도 함께 적었다. |
| ~~**OpenAI 캐시 계열 토큰 배선**~~ | **사이클 5 가 닫았다** — 계열마다 write/read 한 쌍을 리포트에 싣는다. `-28` 이후 세트는 **read 0** 이 실측으로 확인된다. 남은 것은 캐시 write **단가** 한 칸이고 벤더 출처가 없다. |
| **모순 검출 강화 (J11)** | **두 겹으로 반증됐고 닫지 못했다** — 다리 조항을 픽스처에 넣어도 0/3, 사안 비특정 프롬프트 강화도 1/3(사전 판정선 미달 → 원복), 모델을 올려도 haiku·opus 각 0/3. 남은 것은 입력 판본을 가르는 픽스처 2건인데 **평가 장치 확장이라 별도 승인 사안**이다(측정 3 의 분모 11 이 바뀐다). |
| **RAG 심화 중 남은 것** | 하이브리드 검색(BM25+벡터)·리랭킹·**청킹 전략**은 전부 **측정했고 채택하지 않았다**([이유](#검색-품질--게이트-앞의-층)) — 안 해 본 것이 아니라 이 코퍼스에서 값을 못 한다. BGE-M3 로컬 임베딩 행만 의존성 미설치로 **미측정**이다. |
| **지표 확장** | 사유별 검출률 분해, 건당 비용 추이, 인계율 대시보드 |
| **병렬화 · 비동기 처리** | 정책 검색과 주문 조회는 독립이지만 지금은 순차다. 비동기화는 **검토까지 하고 하지 않기로 정했다**([결정 0020](docs/tracking/decisions/0020-비동기화를-검토하고-이번-사이클에는-하지-않는다.md)) — 헤드라인 지표를 1도 움직이지 않으면서 `status` enum·DB CHECK 셋(볼륨 재생성)·계약의 동치 하나를 연다. 그리고 겨냥이 어긋난다: 진짜 꼬리는 **재생성 1회가 만드는 40~49초**인데 비동기화는 그 시간을 줄이지 않고 감춘다. **선행 조건이던 단계별 계측이 사이클 5 에 생겼고, 그 실측이 같은 결론을 가리킨다** — 순차로 도는 두 조회는 합쳐 30 ms 대이고 지연의 주인은 L2 판정 한 층이다. |
| **DB 접속 문자열의 평문 비밀번호** | 지금까지 확인된 노출 경로는 접속 실패 메시지 하나뿐이고, 고치기 전에 재현이 먼저인데 그 재현이 별도 작업이다. **여전히 미해결**로 남긴다([findings 24](docs/tracking/findings.md)). |
| **운영 기능** | 인증 · 레이트리밋 · 멀티테넌시. 포트폴리오 범위 밖. |

---

## 코드 지도

| 파일 | 역할 |
|---|---|
| [`src/reply_gate/pipeline.py`](src/reply_gate/pipeline.py) | 에이전트 루프 — 파이프라인 순서와 **재생성 1회 상한**을 들고 있는 유일한 곳 |
| [`src/reply_gate/gate.py`](src/reply_gate/gate.py) | **L1 게이트** — 사유 4종 + PII allowlist. LLM 을 import 하지 않는다 |
| [`src/reply_gate/judge.py`](src/reply_gate/judge.py) | **L2 판정** — claim 단위 뒷받침 + 근거쌍 모순, 시도당 1회 배치. 판정 산출 검증은 fail-closed. `gate.py` 를 부르지 않는다 |
| [`src/reply_gate/evidence.py`](src/reply_gate/evidence.py) | 근거 수집 — 의도 해석 · 벡터 검색 · **근거 채택(기권 게이트 → 절대 하한)** · 존재성 선검사 · text-to-SQL 조율 |
| [`src/reply_gate/sql_guard.py`](src/reply_gate/sql_guard.py) | 안전장치 2·3 — sqlglot AST 검증, 거부 규칙 **18종**, 캐스트 대상 타입 허용 목록 |
| [`src/reply_gate/draft.py`](src/reply_gate/draft.py) | 초안 생성 — 근거만 컨텍스트, 기각 사유를 피드백으로 재생성 |
| [`src/reply_gate/contracts.py`](src/reply_gate/contracts.py) | 사이클 간 계약 — 답변 계약 JSON, 근거 ID 체계, 판정·상태 enum |
| [`src/reply_gate/api.py`](src/reply_gate/api.py) | FastAPI 엔드포인트 4개 + 응답 스키마 |
| [`src/reply_gate/policy_index.py`](src/reply_gate/policy_index.py) | 정책 문서 파싱 · 조항 단위 청킹 · 임베딩 적재 · 유사도 검색 (벡터에 임베딩 출처를 박고 다른 공간의 질의를 거부한다) |
| [`src/reply_gate/query_rewrite.py`](src/reply_gate/query_rewrite.py) | **검색용 질의 재작성** — 별도 LLM 호출, 산출은 검색 입력일 뿐이다. 실패하면 원문으로 폴백 |
| [`src/reply_gate/retrieval_strategies.py`](src/reply_gate/retrieval_strategies.py) | 전략 사다리 — 벡터 · BM25 어휘 다리 · RRF 융합 · LLM 리랭크, **기권 게이트의 순수 판정 연산**. 채택 판정은 전 전략이 코사인 하나 |
| [`src/reply_gate/retrieval_eval.py`](src/reply_gate/retrieval_eval.py) | 검색 비교 하네스 — 컷 스윕 · 분모 병기 · 리랭크 폴백 관측 · 임베딩 모델 축 · **기권 게이트 격자** · **청킹 격자** |
| [`src/reply_gate/adoption_axis.py`](src/reply_gate/adoption_axis.py) | **채택 축 손계산** — 커밋된 검색 산출물만 읽는 무과금 오프라인 채점자. 반증된 축과 통계량별 분리 여유를 계산한다 |
| [`src/reply_gate/regression_guard.py`](src/reply_gate/regression_guard.py) | **회귀 판정** — 이중 기준선 두 줄 · 조건 지문 · 비악화 두 겹(일치 감소 + 근거 부분 손실) · 승격 등재 정합성. **승격 참조 파일에 쓰지 않는다** |
| [`src/reply_gate/llm.py`](src/reply_gate/llm.py) | OpenAI 래퍼 + **Anthropic 판정 클라이언트** — 전송 오류 1회 재시도, 구조화 출력 |
| [`src/reply_gate/records.py`](src/reply_gate/records.py) | 처리 기록 저장 · 복원 (층별 판정 · 판정 토큰 포함) |
| [`src/reply_gate/order_ref.py`](src/reply_gate/order_ref.py) | 주문번호 형식의 **단독 소유 모듈** (DB CHECK 제약이 같은 정규식을 쓴다) |
| [`src/reply_gate/templates/form.html`](src/reply_gate/templates/form.html) | 판정이 답변보다 먼저 보이는 웹 폼 |
| [`db/schema.sql`](db/schema.sql) | 주문 · 정책 청크(벡터) · 처리 기록 4테이블 · read-only 권한 |
| [`db/init/01_roles.sh`](db/init/01_roles.sh) | 계정 2개 생성 (안전장치 1) |
| [`data/policies/`](data/policies/) | 정책 문서 4개 · 조항 26개 (미끼 3 · 모호 3 · 상충 2) |
| [`data/judge_fixtures.jsonl`](data/judge_fixtures.jsonl) | **판정 픽스처 11건** — 측정 3 의 유일한 입력 |
| [`data/retrieval_labels.jsonl`](data/retrieval_labels.jsonl) | **검색 정답 라벨 30건** — 검색 recall·precision 의 유일한 정답 입력. 채점자만 본다 |
| [`data/rewritten_queries.jsonl`](data/rewritten_queries.jsonl) | **blind 재작성 픽스처** — 비교의 기본 입력 (같은 폴더의 `_oracle` 은 상한 대조군) |
| [`data/promoted_baseline.json`](data/promoted_baseline.json) | **승격 기준선 참조** — 회귀 판정을 구속하는 유일한 기준선. **사람만 바꾼다**(구조 테스트가 쓰기 경로를 막는다) |
| [`scripts/handcalc_adoption_axis.py`](scripts/handcalc_adoption_axis.py) | 채택 축 손계산 산출물 — **무과금·오프라인**, 커밋된 검색 리포트만 읽는다 |
| [`scripts/seed_orders.py`](scripts/seed_orders.py) · [`scripts/index_policies.py`](scripts/index_policies.py) | 시딩 · 인덱싱 |
| [`tests/`](tests/) | **1,430건** (그중 DB 통합 **176건**) |
