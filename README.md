...
아니다** — 실제로 표적 밖의 G28 은 3회 중 2회만 기권했다.
...
달라 점수 분포가 움직인다(표적 밖의 G28 은 3회 중 2회만 기권했다). ② τ 는 이 임베딩 모델의
...
`-1` … `-12` 에서 산발적으로 흔들리던 픽스처이고, 판정 픽스처를 교정한 뒤(`-13` 이후)로는
27/27 `pass` 였다 — "흔들리는 픽스처"라는 서술은 **그 조건에 묶인 관측**이다.
...
**0.003%** — 200배 어긋난다. 판정은 토큰 69.8% 에 비용 72.3% 로, 출력 단가가 입력의 5배라
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 163 passed, 881 deselected
```
...
| [`tests/`](tests/) | 1,044건 (그중 DB 통합 163건) |
