...
| **범위** | 사이클 2 에 **L2 까지 구현·실측**했고, 사이클 3 은 **게이트 앞(검색 품질)을 측정 가능하게** 만들었으며, 사이클 4 는 **채택 축을 두 겹으로 늘리고 회귀 판정을 하네스 코드로 옮겼다**([검색 품질 절](#검색-품질--게이트-앞의-층) · [회귀 가드](#회귀-가드--실행마다-두-줄로-보고한다)). 하지 않은 것은 [범위 절](#한-것과-하지-않은-것)에 그대로 적어 두었다. |
...
| 검색용 **질의 재작성** | **LLM** | 문의 문장을 정책 문서의 어휘로 옮기는 일. 다만 산출은 **검색 입력일 뿐**이고 무엇이 근거가 되는지는 코드가 정한다 — 유사도 임계값과 `top_k` 가 자르고, 호출이 실패하면 원문 질의로 폴백한다(인계가 아니다). |
...
통째로 싣는다. 그 셋을 빼면 재작성 조건에서 격차가 **+0.0600 → 0.0000** 으로 사라지고 원문
조건에서도 **+0.1000 → +0.0454** 로 절반 이상 사라진다. 같은 예산을 조항으로 환산한
...
**과대 해석 금지**: 이 코퍼스는 조항 26개 · 본문 평균 94자로 **조항이 짧고 자족적이다.**
...
**재생성**이고, 새 축 90건 중 시도 2회를 탄 10건이 36.7~49.1초를 차지한다. 30건 규모에서
