...
의도다. 목표를 실측값에 맞춰 내리면 목표가 아니라 현황 기술이 된다. 간극 3.9%p 의 원인은
전부 특정돼 있다 — 어긋난 26 케이스-실행은 무근거 문의 G21–G24(12회), 기각이 재현되지
...
5건 × 3회에서 모델은 **한 번도 값을 지어내지 않았다** — `pii_detected` 발화 0/15 로,
매번 "제공된 안내에는 기재되어 있지 않다"고 답했다. 유일한 기각 1회(1회차 G16)도 값을
지어낸 것이 아니라 그 "없다" 문장에 근거 ID 를 달지 않은 것(`missing_citation`)이다.
**게이트가 못 잡은 것이 아니라 그 앞의 초안 생성이 걸릴 만한 답을 내주지 않은 것이다.**
...
남는 간극은 그다음이다: 무근거 문의 4건(G21–G24)은 근거를 붙여 통과한 답변이
`escalated` 가 아니라 `answered` 로 끝난다. 정책 문서에 근거가 없는 주제인데 유사도
0.3 이상으로 인접 조항이 검색되기 때문이다. 이것은 라벨 오류가 아니라 **L1 이 내용 검증을
하지 않는다는 설계의 결과**다(→ [게이트 2층 구조](#게이트-2층-구조), L2 이월). 이 4건은
3회 모두 같은 방식으로 어긋났으므로 잡음이 아니라 재현되는 성질이다(미끼 쪽 어긋남도
G16 이 1회차에 일치한 것을 빼면 3회 동일). 다음 사이클의 입력으로 남긴다: 유사도 임계값
조정, 그리고 무엇보다 **L2 가 실제로 필요한 이유의 실측 근거**. (미끼 조항 강화는 생성기를
약화시키지 않기로 한 결정과 같은 이유로 보류다 — [결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md).)
...
> **`uv run pytest` 는 DB 기동을 전제한다.** `db` 마커가 붙은 통합 테스트(116건)는
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 116 passed, 312 deselected
```
...
| [`tests/`](tests/) | 428건 (그중 DB 통합 116건) |
