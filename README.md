...
골든셋 30건 실측에서도 기각 유발 문의 5건의 **기각 재현율은 0%** 였다([첫 실측값](#첫-실측값-2026-08-04-gpt-56-terra--text-embedding-3-small--임계값-03--top-k-5)).
...
**미확정 — 첫 측정값을 보고 확정한다.**
측정하기 전에 목표를 적어 두면 숫자를 맞추러 가는 일이 되므로 하지 않았다.

### 첫 실측값 (2026-08-04, `gpt-5.6-terra` · `text-embedding-3-small` · 임계값 0.3 · top k 5)
...
리포트는 `reports/evaluation.md` 와 `reports/evaluation.json` 두 형식으로 나온다.
...
| 지연 p50 / p95 | 4,510 ms / 9,005 ms |
| 건당 토큰 | 생성 1,280.7 + 임베딩 14.8 = **1,295.5** |

**기각 재현율 0%는 이 사이클의 가장 중요한 실측 결과다.** 미끼 조항 5건 모두에서 모델이
값을 지어내지 않고 "제공된 안내에는 기재되어 있지 않다"고 답했다 — 즉 `pii_detected` 가
발화할 조건 자체가 만들어지지 않았다. **게이트가 못 잡은 것이 아니라 그 앞의 초안 생성이
걸릴 만한 답을 내주지 않은 것이다.** 무근거 문의 4건(G21–G24)이 `answered` 로 끝난 것도
같은 결이다: 유사도 0.3 이상으로 관련 조항이 검색되고, 모델이 그 근거를 딛고 "안내가
어렵다"고 답하므로 L1 이 기각할 구조적 결함이 없다.

두 수치는 라벨 오류가 아니라 **L1 이 내용 검증을 하지 않는다는 설계의 결과**다
(→ [게이트 2층 구조](#게이트-2층-구조), L2 이월). 다음 사이클의 입력으로 남긴다:
...
실측값은 [첫 실측값](#첫-실측값-2026-08-04-gpt-56-terra--text-embedding-3-small--임계값-03--top-k-5) 절에 있다.
