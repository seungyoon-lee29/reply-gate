...
![문의 접수부터 인계까지 — 존재하지 않는 주문번호는 초안 생성에 진입조차 하지 않는다](docs/images/demo-escalation.gif)

위 녹화는 **실제 실행**이다(로컬 · 문의 1건 · 2026-08-26). 존재성 선검사가 먼저 걸려
**주문 조회에도 초안 생성에도 진입하지 않았고**, 그래서 화면 하단 처리 지표의 **판정 토큰이
`0`** 이다 — 인계 사유 `order_not_found` 는 LLM 이 정한 것이 아니라 코드가 정했다.
...
### 3-0) 통과 — 세 claim 이 각각 근거를 달고 L2 까지 지난다

기각만 보여주면 게이트가 무엇을 **통과시키는지**를 알 수 없다. 골든셋 G04 를 웹 폼에 그대로 넣은
실행이다(2026-08-27 · 현재 기본값).

![적립금 문의가 시도 1 에서 L1 pass · L2 pass 로 통과하고 claim 마다 근거 ID 가 붙는다](docs/images/demo-pass.gif)

시도 1 에서 끝났다 — **L1 pass · L2 pass**, claim 셋이 각각 판정을 받았고(`인용 근거 원문과
동일한 내용으로 뒷받침됨`), 근거쌍 모순은 없었다. 확정 답변의 문장마다 `policy:support:4-6` 이
붙어 있다. **이 장면과 아래 두 장면의 차이는 입력뿐이고, 판정하는 코드는 같다.**
문의별 응답 본문은 [데모 나머지 장면](docs/demo.md)의 2) 절에 있다.
...
**현재 기본값에서 재현한 회차** — 위 트랜스크립트는 기권 게이트 배선 **전** 기록이라 같은 문의로는
더 이상 재현되지 않는다. 같은 실행 경로(**L1 통과분을 L2 가 기각한다**)를 현재 기본값에서 그대로
보여주는 것이 골든셋 G01 이고, 라이브 3회 모두 발화했다. 아래는 그 회차를 웹 폼으로 흘린
녹화다(2026-08-27). 사유는 `unsupported_claim` 이 아니라 `contradictory_evidence` 다 —
**둘 다 L2 만 낼 수 있는 사유다.**

![시도 1 이 L1 을 통과하고 L2 에서 contradictory_evidence 로 기각된 뒤, 재생성한 시도 2 가 모순을 명시해 통과한다](docs/images/demo-l2-reject.gif)

L2 가 짚은 것은 `policy:refund:2-1 ↔ policy:refund:2-2` 다 — **2-1 은 변심 환불 신청 기한을 7일,
2-2 는 반품 신청 기한을 14일**로 적어 같은 사안에 다른 기간을 제시한다. 시도 1 의 claim 은 **개별로는
전부 pass** 였고(각자 인용한 조항이 뒷받침한다), 기각된 것은 **초안이 그 모순을 명시하지 않은 채
한쪽만 딛고 답했기 때문**이다. 재생성한 시도 2 는 *"신청 기한 기준이 서로 다르게 안내되고 있습니다"*
라고 두 기준을 모두 밝혔고 그래서 통과했다 — 같은 모순 근거쌍이 기록에는 그대로 남아 있다.
**기록됐다고 곧 기각은 아니다**는 화면의 문장이 이 자리에서 실측으로 성립한다.
...
![API 키를 지운 셸에서 L1 게이트만 호출해 pii_detected · pass · missing_citation+invalid_citation 을 얻는다](docs/images/demo-l1-standalone.gif)

위 카드는 **실제 실행의 명령과 출력**이다(`env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY` 로 키를 지운
셸에서 돌렸고, DB 도 서버도 뜨지 않았다). 터미널 화면 녹화가 아니라 그 트랜스크립트를 그려 낸
것이고, 아래에 같은 명령과 출력을 복사할 수 있게 그대로 둔다.
