...
> **문제정의 — 실측 4사이클이 다듬은 현재형** ([전문](docs/problem.md))
>
> LLM CS 답변의 실패는 "그럴듯한 값을 지어낸다"로 시작했지만, 이 저장소의 실측은 그 서술을
> 수정했다: 요즘 상위 모델은 값을 거의 지어내지 않는다(**미끼 75 케이스-실행에서 `pii_detected`
> 발화 0회** — 네 사이클 **풀셋 라이브 15회** 전부).
...
| **범위** | 사이클 2 에 **L2 까지 구현·실측**했고, 사이클 3 은 **게이트 앞(검색 품질)을 측정 가능하게** 만들었으며, 사이클 4 는 **채택 축을 두 겹으로 늘리고 회귀 판정을 사람 눈에서 코드(`regression_guard.py`)로 옮겼다**([검색 품질 절](#검색-품질--게이트-앞의-층) · [회귀 가드](#회귀-가드--실행마다-두-줄로-보고한다)). 하지 않은 것은 [범위 절](#한-것과-하지-않은-것)에 그대로 적어 두었다. |
| **비용** | 문의 1건당 **$0.014819** (현재 기본값 · 3회 90건 실측 · 기준일 2026-08-19). **판정이 그중 72.3%** 이고 임베딩은 0.003% 다 — [비용 절](#비용--계열마다-단가가-다르다). |
...
| **사이클 4 T10** (현재 기본값 · 게이트 τ=0.06) | **13.3% (2/15)** | G18 `contradictory_evidence` 1회 + G16 `missing_citation` 1회 |
...
아니다.** `pii_detected` 는 사이클 4 의 여섯 번을 포함해 **풀셋 라이브 15회 75 케이스-실행
전부에서 한 번도 발화하지 않았다.**
...
**현재 기본값에서 웹 폼으로 흘린 기록 1회** (2026-08-19, 위 3-2 와 **같은 문의**):

```json
{
  "inquiry_id": "8e0e821f-3c47-4a21-a44f-22bb4db70732",
  "status": "escalated",
  "answer": null,
  "claims": [],
  "citations": [],
...
(입력 213 · 출력 12)과 재작성 호출(192/22)뿐이고, 지연은 **4.6초**다 — 위 3-2 의 19.5초와
...
**이것도 1회 실행이다.** 같은 네 건(G21–G24)이 라이브 3회에서 **12/12** 같은 경로로 끝났다는
것이 지금 가진 전부이고, 재작성 산출이 실행마다 달라 점수 분포가 움직이므로 **"항상"이
...
그래서 지금 L1 의 PII 검사 범위는 사이클 1·2 실측 때보다 넓다 — 같은 픽스처 27건에서
검출률·오탐률은 그대로 **100.0% / 0.0%** 다.
...
**측정 1 은 네 사이클 열다섯 번의 풀셋 라이브 전부에서 같은 값이다**(100.0% / 0.0%).
...
| 지표 | 목표 | 사이클 1 (L2 꺼짐) | 사이클 2 (L2 켜짐) | 사이클 3 (컷 0.50, 원복됨) | 사이클 4 T2 (승격 기준선) | **사이클 4 T10** (현재 기본값) | 판정 |
|---|---|---|---|---|---|---|---|
...
것은 목표 미달이 아니라 **회귀**다. **네 사이클 열다섯 번의 풀셋 라이브 전부에서 100.0% /
0.0% 로 무변경**이다.
...
**조건이 다른 실측끼리는 대조하지 않는다.** 실행 조건 **지문** 18개 항목(라벨 버전 · 검색 정답
라벨 버전 · 컷 · **기권 게이트 통계량·τ** · 재작성 · 임베딩 모델·차원 · `top_k` · 생성/판정
모델 · effort · 판정 프롬프트·픽스처 버전 · 캐싱 · 측정 범위 …)이 대조 가능성을 가른다.
...
**지금 어긋나 있는 것을 숨기지 않는다.** 승격 기준선은 기권 게이트가 배선되기 **전**에
등재됐으므로, 현재 기본값으로 도는 모든 실측은 `abstention_gate_statistic`·`abstention_tau`
두 항목에서 기준선과 어긋난다(판정 픽스처 교정이 하나 더 얹혔다). 재등재는 풀셋 라이브 3회를
새로 사는 일이라 **사용자 결정으로 다음 사이클에 미뤘다**([findings 23](docs/tracking/findings.md)).
...
| 지표 (3회 합) | **T2 — 게이트 미배선** (승격 기준선) | **T10 — 게이트 τ=0.06** (현재 기본값) |
|---|---|---|
...
### 계열별 분해 — 현재 기본값 라이브 3회 (문의 90건)

| 계열 | 모델 | 토큰 | 토큰 점유율 | 달러 | **달러 점유율** |
|---|---|---:|---:|---:|---:|
| 생성 (의도 해석 + SQL 생성 + 초안) | `gpt-5.6-terra` | 106,038 | 26.5% | $0.332116 | 24.90% |
| 임베딩 (문의·재작성 질의) | `text-embedding-3-small` | 2,317 | 0.6% | $0.000046 | **0.003%** |
| **판정 (L2)** | `claude-sonnet-5` | **279,594** | **69.8%** | **$0.964452** | **72.31%** |
| 검색 (질의 재작성) | `gpt-5.6-terra` | 12,464 | 3.1% | $0.037108 | 2.78% |
| **합계** | | **400,413** | 100% | **$1.333722** | 100% |

회당 $0.444574 · **문의 1건당 $0.014819**.

**점유율이 계열마다 어긋나는 것이 이 표의 요지다.** 임베딩은 토큰의 0.6% 인데 비용의
**0.003%** — 반올림 전 값(0.5787% · 0.003475%)으로 재면 **166.5배** 어긋난다. 판정은 토큰
69.8% 에 비용 72.3% 로, 출력 단가가 입력의 5배라
출력이 많은 계열이 비용 쪽으로 밀린다. **절대액(3회 $1.33)은 이 규모에서 의사결정 요인이
...
| **기권 게이트 τ=0.06 · 컷 0.30**(현재 기본값) | 승격 기준선 (같은 조건 · 게이트만 미배선) | **−12.8%** | **−13.0%** | 대가 없이 얻었다 |
...
- **생성·검색 계열의 달러는 전부 상한이다.** `llm.py` 의 OpenAI 래퍼가 캐시 적중분
  (`cached_tokens`)을 읽지 않아, 그중 얼마가 캐시 단가로 과금됐는지 이 저장소는 모른다.
  Anthropic 쪽에 넣은 배선과 **같은 자리가 생성 계열에 비어 있다**
  ([findings 21](docs/tracking/findings.md)). 실제 청구액은 위 값보다 같거나 적다.
...
```bash
uv run pytest              # 1044 passed
uv run ruff check .
uv run ruff format --check .
uv run mypy                # strict
```

> **`uv run pytest` 는 DB 기동을 전제한다.** `db` 마커가 붙은 통합 테스트(163건)는
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 163 passed, 908 deselected
```
...
실측값은 [사이클 2 실측값](#사이클-2-실측값) · [사이클 3 실측값](#사이클-3-실측값--검색-실험-조건-되돌린-조건이다) 절에 있다.
...
| **단계별 지연 계측** | **다음 사이클의 1순위다.** 리포트의 지연 필드는 셋뿐(`latency_ms`·p50·p95)이라 어느 단계가 몇 초인지 **한 번도 측정된 적이 없다.** 지금 말할 수 있는 것은 종료 지점별 차집합뿐이다 — 접수~검색 **상한 4.5초**, 시도 1회 종결 중앙값 11.6~12.1초, 그 사이 7~8초대가 초안+판정 몫(둘로 못 가른다). 계약 무변경이라 싸고, 비동기화 판단의 선행 조건이다([findings 22](docs/tracking/findings.md)). |
| **승격 기준선 재등재** | 기권 게이트가 승격 **뒤에** 채택돼 두 항목이 영구히 어긋나 있다(판정 픽스처 교정이 하나 더 얹혔다). 재등재는 **풀셋 라이브 3회를 새로 사는 일**이고 승격은 사람만 한다 — 사용자 결정으로 다음 사이클에 미뤘다([findings 23](docs/tracking/findings.md)). |
| **OpenAI 캐시 계열 토큰 배선** | 생성·검색 계열의 달러가 전부 **상한**이다 — 래퍼가 `cached_tokens` 를 읽지 않는다. Anthropic 쪽에 넣은 배선과 같은 자리가 비어 있다([findings 21](docs/tracking/findings.md)). |
...
| **병렬화 · 비동기 처리** | 정책 검색과 주문 조회는 독립이지만 지금은 순차다. 비동기화는 **검토까지 하고 하지 않기로 정했다**([결정 0020](docs/tracking/decisions/0020-비동기화를-검토하고-이번-사이클에는-하지-않는다.md)) — 헤드라인 지표를 1도 움직이지 않으면서 `status` enum·DB CHECK 셋(볼륨 재생성)·계약의 동치 하나를 연다. 그리고 겨냥이 어긋난다: 진짜 꼬리는 **재생성 1회가 만드는 40~49초**인데 비동기화는 그 시간을 줄이지 않고 감춘다. |
...
| [`tests/`](tests/) | 1,071건 (그중 DB 통합 163건) |
