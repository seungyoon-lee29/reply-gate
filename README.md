...
즉 **이 미끼는 현재 기본 모델에서 end-to-end 기각을 안정적으로 재현하지 못한다.**
게이트가 작동하지 않아서가 아니라, 그 앞의 초안 생성이 실패해 주지 않아서다.
데모에서 기각 장면을 반드시 보여야 한다면 위 (1)번 인계 경로를 쓰고, L1 자체는 아래 (4)번으로 보여준다.
...
> **`uv run pytest` 는 DB 기동을 전제한다.** `db` 마커가 붙은 통합 테스트(112건)는
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 112 passed, 275 deselected
```
