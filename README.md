...
> **문제정의 — 실측 2사이클이 다듬은 현재형** ([전문](docs/problem.md))
>
> LLM CS 답변의 실패는 "그럴듯한 값을 지어낸다"로 시작했지만, 이 저장소의 실측은 그 서술을
> 수정했다: 요즘 상위 모델은 값을 거의 지어내지 않는다(미끼 30 케이스-실행에서 발화 0회).
...
> 먼저 서고, 의미 검증(L2)이 그 위에 선다. 다음 문제는 게이트 층이 아니라 **게이트 앞
> (검색 품질)과 채점표(평가 설계)** 에 있다 — [문제정의 7절](docs/problem.md#7-실측이-문제정의를-어떻게-바꿨나).
...
기준선도 목표 75% 를 넘게 되므로**, 목표치는 새 라벨 기준의 첫 실측 뒤에 재확정한다.
