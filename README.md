...
> **문제정의 — 실측 3사이클이 다듬은 현재형** ([전문](docs/problem.md))
>
> LLM CS 답변의 실패는 "그럴듯한 값을 지어낸다"로 시작했지만, 이 저장소의 실측은 그 서술을
> 수정했다: 요즘 상위 모델은 값을 거의 지어내지 않는다(**미끼 45 케이스-실행에서 `pii_detected`
> 발화 0회** — 세 사이클 9회 실행 전부).
...
> 무근거 문의를 걸러내는 컷이 정상 문의의 근거와 모순 판정에 필요한 상충 조항까지 함께
> 자른다. 그래서 **인접 근거를 딛는 답변은 지금도 L2 가 잡는다**([그 장면](#3-2-l2-가-잡는-장면--근거를-달았는데-그-근거가-답을-뒷받침하지-않는다) · [검색 품질 절](#검색-품질--게이트-앞의-층) · [문제정의 7절](docs/problem.md#7-실측이-문제정의를-어떻게-바꿨나)).
...
| **범위** | 사이클 2 에 **L2 까지 구현·실측**했고, 사이클 3 은 **게이트 앞(검색 품질)을 측정 가능하게 만들고 이긴 한 단만 배선**했다([검색 품질 절](#검색-품질--게이트-앞의-층)). 하지 않은 것은 [범위 절](#한-것과-하지-않은-것)에 그대로 적어 두었다. |
...
가 성립하지 않았다. 임계값은 되돌렸지만([검색 품질 절](#검색-품질--게이트-앞의-층))
되돌린 조건의 라이브 실측은 아직 없으므로 **"복구됐다"고 쓰지 않는다.**
...
아래는 **현재 기본값**(재작성 켜짐 · 임계값 0.3 · L2 켜짐)에서 웹 폼으로 흘린 실제 기록이다.
...
**재현 횟수를 정확히 적는다.** 위 기록은 현재 기본값에서 **1회** 관측이다(2026-08-14,
...
상당 부분 이 네 건에서 나온다. 확률 층이 낀 장면에 "항상"을 붙이지 않는 이유는
[아래](#검색-품질--게이트-앞의-층)에 하나 더 있다.
...
flowchart TD
    ...
    VEC["3a. 정책 근거 수집 [코드]<br/>질의 재작성 [LLM] → 원문·재작성 임베딩 → pgvector 코사인 검색 ×2<br/>합집합(더 큰 유사도) → top_k=5 · 유사도 임계값 0.3<br/>재작성 실패 = 원문으로 폴백(인계 아님)"]
    VEC --> MERGE
...
| 근거 **채택** | **코드** | 어떤 조항이 근거가 되는지는 프롬프트가 아니라 임계값·상한이 정한다. 이 축을 LLM 에게 넘기면 "근거가 없다"를 말할 주체가 사라진다([검색 품질 절](#검색-품질--게이트-앞의-층)). |
...
**사이클 3 이 손댄 곳은 게이트 앞(검색)이고, 두 층의 계약 — 사유 4종·2종, 층 경계, 판정
규칙 — 은 그대로다.** 다만 같은 사이클의 적대리뷰가 찾은 **결함 수정 3건이 두 층 안으로
들어갔다**(아래 [L1 절](#l1-게이트)) — 검사 범위를 넓히고 오탐 경로를 없앤 것이지
판정을 느슨하게 한 것이 아니다. 측정 1 은 세 사이클 내내, 측정 3 은 신설된 사이클 2 이후
여섯 실측 내내 같은 값이다.
...
- **원복한 조합(재작성 + 컷 0.30)의 라이브 실측은 없다.** 컷 0.30 은 사이클 1·2 가 6회,
  재작성은 T10 이 3회 실측했지만 **둘을 합친 조건은 재지 않았다.** 위 오프라인 수치가 그
  조건의 유일한 근거다.
- **기권은 결정론이 아니다.** 11/12 의 유일한 실패(3회차 G21)는 재작성 산출이 실행마다
  조금씩 달라 5조항이 컷 위로 올라온 경우다. 확률 층이 낀 단계에 "항상"을 붙이지 않는다.
- **다음 사이클의 문제는 채택 축이다.** 컷이라는 단일 축으로 양방향을 못 막는다는 것이 두 번
  실측됐다. 질의별 적응 컷·상대 마진·상충쌍 보존이 후보이고, 채택 규칙에는 **"라벨이 있는
  케이스가 채택 근거 0건이 되면 안 된다"** 는 케이스 단위 하한을 함께 건다
...
| 지표 | 목표 | 사이클 1 (L2 꺼짐) | 사이클 2 (L2 켜짐) | **사이클 3** (검색 실험 조건) | 판정 |
|---|---|---|---|---|---|
...
것은 목표 미달이 아니라 **회귀**다. 사이클 2·3 에서도 3회 모두 100.0% / 0.0% 로 **무변경**이고,
측정 3 도 사유 하나가 어긋나는 방식까지 사이클 2 신설 이후 여섯 실측 모두 같다.
...
**둘 중 ②만 리포트가 매 실행 계산하고, ①은 아직 사람이 이전 리포트와 대조해 판정한다** —
하네스 코드로 상설화하는 것이 다음 사이클 몫이다.

**측정 3 에는 목표치를 두지 않기로 확정했다.** 첫 실측이 나왔다고 그 값 근처에 목표를
박으면 목표가 아니라 현황 기술이 된다. 회귀는 3회 동일했던 실측값과의 비교로 잡는다.
...
```bash
uv run pytest              # 809 passed
uv run ruff check .
uv run ruff format --check .
uv run mypy                # strict
```

> **`uv run pytest` 는 DB 기동을 전제한다.** `db` 마커가 붙은 통합 테스트(162건)는
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 162 passed, 647 deselected
```
...
| | 왜 지금 안 했나 |
|---|---|
| **측정 3 목표치** | 미룬 것이 아니라 **두지 않기로 확정했다**([결정 0006](docs/tracking/decisions/0006-지표-목표치를-실측-뒤에-확정한다.md)) — 픽스처 11건이라 한 건이 9.1%p 이고, 실측값 근처에 목표를 박으면 목표가 아니라 현황 기술이 된다. 회귀는 세 사이클 동일했던 값과의 비교로 잡는다. |
...
| **채택 축을 하나 더 만드는 일** | 사이클 3 이 남긴 가장 큰 숙제다. 유사도 임계값 하나로는 "못 찾는다"와 "잘못 찾는다"를 동시에 못 막는다는 것이 **두 번 실측됐다.** 질의별 적응 컷·상대 마진·상충쌍 보존이 후보이고, 채택 규칙에 케이스 단위 하한("라벨이 있는 케이스가 근거 0건이면 안 된다")을 함께 건다. |
| **회귀 판정의 상설화** | 합산 일치율 뒤에서 케이스가 죽는 것을 이번엔 귀인 절이 사람 눈으로 잡았다. 케이스 단위 판정이 하네스 코드에 들어가야 한다. |
| **RAG 심화 중 남은 것** | 하이브리드 검색(BM25+벡터)과 리랭킹은 **측정했고 채택하지 않았다**([이유](#검색-품질--게이트-앞의-층)) — 안 해 본 것이 아니라 이 코퍼스에서 값을 못 한다. **청킹 전략 비교**(조항 단위 vs 고정 크기)는 여전히 미측정이고, BGE-M3 로컬 임베딩 행도 의존성 미설치로 미측정이다. |
| **모순 검출 강화** | L2 판정 픽스처에서 3회 모두 같은 방식으로 어긋나는 1건(J11)이 남아 있다 — 모순 검출이 뒷받침 판정보다 약하다는 재현되는 신호다. |
...
| [`src/reply_gate/retrieval_strategies.py`](src/reply_gate/retrieval_strategies.py) | 전략 사다리 — 벡터 · BM25 어휘 다리 · RRF 융합 · LLM 리랭크. **채택 판정은 전 전략이 코사인 하나** |
| [`src/reply_gate/retrieval_eval.py`](src/reply_gate/retrieval_eval.py) | 검색 비교 하네스 — 컷 스윕 · 분모 병기 · 리랭크 폴백 관측 · 임베딩 모델 축 |
...
| [`tests/`](tests/) | 809건 (그중 DB 통합 162건) |
