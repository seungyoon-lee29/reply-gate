...
**재현 확률.** L2 기각 자체는 사이클 4 라이브에서 3/3 이었지만 **재기각 인계까지 가는 것은
1/3** 이다. 이번 시연에서는 **2회차에 나왔고**(상한 3회) 재생성은 통과했다.
...
`-1` … `-12` 에서 산발적으로 흔들리던 픽스처이고, 판정 픽스처를 교정한 뒤(`-13` … `-27`)
15세트에서는 **14/15 가 `pass`** 였다 — 남은 한 칸이 `-25`(판정 프롬프트 캐싱 켬,
...
귀인은 이번 사이클의 변경을 가리키지 않는다(제품 코드 무변경 · 15세트 만의 첫 이탈 ·
이탈 지점이 **의도 분류**). **두 사실을 함께 적는다 — 한쪽만 적으면 읽는 사람이 속는다.**
...
>
> **지금 말할 수 있는 것은 관측뿐이다** — 커밋된 라이브 측정 3 **27세트 중 26세트**에서
...
- **그 회차의 구속 줄 판정은 미달이다.** 케이스 **G06** 이 기준선 3/3 → 이번 0/1 이다.
  ...
  커밋된 15세트 전부 `answered` 였고 이번이 첫 이탈, 이탈 지점은 **의도 분류**(`policy` →
  `both` 로 가서 주문번호가 없다는 이유로 `missing_order_ref`)다. 사용자 판단은
  **"그대로 받고 정직하게 기록한다"** 였고 재구매도 가드 수정도 하지 않았다.
...
```bash
uv run pytest -m db        # DB 통합 테스트만 — 176 passed, 1,252 deselected
```
...
| [`tests/`](tests/) | **1,428건** (그중 DB 통합 **176건**) |
