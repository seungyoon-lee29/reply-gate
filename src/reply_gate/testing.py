...
class LexicalEmbeddingClient:
    ...
    (적재 → 유사도 정렬 → 상위 `top_k` 절단 → 기권 게이트 → 임계값 필터)을 끝까지 검증할 수 있다.

    **유사도의 절대 크기와 분포는 실제 모델과 다르다.** 그래서 이 대역으로 도는 실행의
    컷(`scripts.evaluate.STUB_SIMILARITY_THRESHOLD`)도, 기권 게이트의 τ 도 **이 조건의
    값이 아니다** — τ 는 임베딩 모델에 묶인 인자이고 모델을 넘어 이전되지 않는다
    (`docs/tracking/decisions/0014`). 대역 실행이 배관을 확인해 주는 것과 그 실행의 수치가
    제품 수치인 것은 다른 이야기다.
...
def build_stub_pipeline(
    ...
    **채택 축(컷 · `top_k` · 기권 게이트)은 여기 다시 적지 않는다** — 수집기가 `settings`
    에서 직접 조립하므로 두 조립기가 같은 설정을 받으면 같은 축으로 돈다. 축을 여기에
    한 번 더 적으면 그 순간부터 대역이 제품과 다른 축을 재기 시작한다.
    """
