...
(docs/architecture.md "대표 흐름" 3단계, `.dryforge/spec.md` §5-2).
...
**실패는 폴백이지 인계가 아니다**(`.dryforge/spec.md` §8-1). fail-closed 원칙이 지키는 것은
"검증하지 못한 답변을 내보내지 않는다"이고, 검색은 검증이 아니라 재료 수집이다. 재작성을
얻지 못하면 원문 질의로 검색을 계속하며 — 그 결과는 사이클 2 까지의 제품 동작과 정확히
같고 뒤에 L1·L2 두 층이 그대로 서 있다. 다만 **조용히 지나가지 않는다**: 폴백 사유와 실비용
토큰이 호출자에게 그대로 올라가 처리 기록과 평가 리포트에 남는다.
"""
