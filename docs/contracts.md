...
**보기 모드 토글 — `고객이 보는 화면`, 기본은 꺼짐.** 켜면 게이트 판정 · 수집된 근거 ·
처리 지표 · claim 목록을 숨기고 **확정 답변만** 남기며, 인계는 사유 코드(`no_evidence`)
대신 안내 문장으로 바뀐다(고객에게 나갈 화면에 내부 코드를 싣지 않는다).
**기본값이 꺼짐인 것이 계약이다** — 켜짐이 기본이면 첫 화면이 답변 한 문단이 되어 위의
표시 순서가 조용히 무너진다. `tests/test_api.py` 가 기본값을 못박고, 음성 대조가 그 검사가
실제로 문다는 것을 함께 증명한다.

토글은 **표시 계층만** 바꾼다 — 응답 스키마도, 저장되는 처리 기록도, 호출 횟수도 그대로다.
모드를 바꿔도 재요청하지 않는다(마지막 응답을 다시 칠한다 — 과금이 없다).
