...
| `VECTOR_SIMILARITY_THRESHOLD` | `0.3` | **T10 이 0.50 을 반증해 되돌린 값이다.** 0.50 은 무근거 4건을 검색 단계에서 기권시키지만 같은 컷이 G04 의 정답 조항(0.3571)과 G18 의 상충 조항(0.4676)까지 잘라 정상 문의가 인계되고 L2 모순 기각이 사라진다(`docs/tracking/status.md` "T10 라이브 재실측") |
