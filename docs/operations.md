...
`mypy` 75 파일 0 · `pytest` **1,428 통과** · `pytest -m db` **176 통과 / skip 0**.
...
| 측정 | 무엇을 재나 | 언제 도나 |
|---|---|---|
| 측정 1 | L1 게이트 단위 정확도 (L1 픽스처 **35건**) | **항상** — LLM 호출 0회, 키도 DB 도 불필요 |
...
| 2 | 3회 | 2회차에 재현 (1회차는 1시도 통과) | L2 기각 자체는 높고, 재기각 인계까지는 1/3 |
| 3 | 3회 | 1회차에 재현 | 12/12 |
