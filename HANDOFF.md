...
- **외부 API: 임베딩·합성데이터·초안 생성은 OpenAI, L2 judge만 다른 계열.** judge를 생성과 같은 계열로 두면 self-bias + blind spot 중첩으로 검출률이 실제보다 좋게 나온다 — 대표 지표가 오염된다. judge 계열 선택은 미결. 단가·비용 추정은 `기획-입력.md` "외부 API provider" 절에 2026-08-04 기준으로 기록돼 있다.
- **데모 첫 3초 = 답변이 기각되는 순간.** "CS 챗봇"으로 소개하지 않는다.

## 저장소 상태

- `main`, 문서 커밋만 존재. **`go` 실행 전 `git status`로 working tree clean 확인 필수** — provider 결정 반영분(2026-08-04)이 커밋돼 있어야 한다
- 파일: `기획-입력.md`, `.gitignore`(Python 기준; `.dryforge/`는 버전관리에 포함), `HANDOFF.md`(이 파일)
...
- 지표 목표치, 골든셋 규모
- L2 judge 모델 계열 ("OpenAI 아님"까지만 확정)

위 넷은 `/ready` 대화에서 결정하면 된다.
